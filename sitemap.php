...
// Caching logic using CMSGO_CONTENT to prevent DB/server overload
$cache_dir = CMSGO_CONTENT . '/cache';
...
$cmsgo['sitemap_set_base'] = true;
$cmsgo['sitemap_set_default'] = true;

if (is_file(CMSGO_ROOT . '/include/config/sitemap.custom.php')) {
    include CMSGO_ROOT . '/include/config/sitemap.custom.php';
}

if (function_exists('cmsgo_getCustomSitemap')) {
    $_addURL = cmsgo_getCustomSitemap($struct);
} else {
    $_addURL = [];
}

if ($cmsgo['sitemap_set_default']) {
    ...
    $sql_host .= 'FROM ' . DB_PREPEND . 'cmsgo_articlecontent ac ';
    $sql_host .= 'INNER JOIN ' . DB_PREPEND . 'cmsgo_article a ON ac.acontent_aid = a.article_id ';
    ...
    if ($news_host_aid > 0) {
        ...
        $sql_news .= 'FROM ' . DB_PREPEND . 'cmsgo_content WHERE ';
...
if (is_file(CMSGO_ROOT . '/include/config/sitemap.custom.ini')) {
    $customSitemapLinks = parse_ini_file(CMSGO_ROOT . '/include/config/sitemap.custom.ini', true);
...
if ($cmsgo['sitemap_set_base']) {
    // just return the main URL
    echo '  <url>' . LF;
    echo '      <loc>' . CMSGO_URL . '</loc>' . LF;
