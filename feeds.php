...
require_once CMSGO_ROOT.'/include/inc_ext/feedcreator.class.php';
...
$feeds          = array();
$feeds          = @parse_ini_file(CMSGO_ROOT.'/include/config/feeds.ini.php', true);
...
if(empty($feeds) || (is_array($feeds) && count($feeds) == 0)) {
    ...
    $feeds['default'] = array(
        ...
        "description"       => "",
        "link"              => CMSGO_URL,
        "syndicationURL"    => CMSGO_URL.'feeds.php',
        "imagesrc"          => "",
        "imagetitle"        => "",
        "imagelink"         => "",
        "imagedescription"  => "",
...
if(isset($result[0]['article_title'])) {
    foreach($result as $data) {
        ...
        $item->link             = CMSGO_URL.'index.php?'.setGetArticleAid( $data );
        ...
        $item->guid             = CMSGO_URL.'index.php?'.setGetArticleAid( $data );
        $rss->addItem($item);
...
function combinedParser($string, $charset='utf-8', $allowed_tags='') {
    ...
    if(!empty($string) && CMSGO_CHARSET != $charset) {
        $string = makeCharsetConversion($string, CMSGO_CHARSET, $charset);
    } else {
...
function getFeedStructureID($value) {
    ...
    if($value !== '' && !is_num($value)) {
        ...
        $sql  = "SELECT acat_id FROM ".DB_PREPEND."cmsgo_articlecat WHERE acat_aktiv=1 AND ";
