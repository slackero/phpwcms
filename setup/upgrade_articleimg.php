...
// get all articles
if($all = _dbQuery("SELECT article_id, article_image, article_title, article_tstamp FROM ".DB_PREPEND."cmsgo_article")) {

	foreach($all as $key => $value) {

		$all[$key]['article_image']	= unserialize($all[$key]['article_image'], ['allowed_classes' => false]);
		if(isset($all[$key]['article_image']['prev']) && !empty($all[$key]['article_image']['id'])) {
			...
			$file = _dbQuery("SELECT f_id, f_hash, f_ext FROM ".DB_PREPEND."cmsgo_file WHERE f_id=".$all[$key]['article_image']['id']." LIMIT 1");
