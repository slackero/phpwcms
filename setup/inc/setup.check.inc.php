...
if(!empty($step)) {
    ...
    if ($step == 1 && $do) {
        ...
        $cmsgo['db_port'] = empty($_POST['db_port']) || !intval($_POST['db_port']) ? 3306 : intval($_POST['db_port']);
        ...
        if(isset($_POST['dbsavesubmit'])) {
            ...
            if(!empty($cmsgo['db_pers']) && !str_starts_with($db_host, 'p:')) {
                $db_host = 'p:'.$db_host;
            }
    ...
    if($step == 4 && $do) {
        ...
        $cmsgo['file_maxsize']     = ($cmsgo['file_maxsize']) ?: 2097152;
        $cmsgo['content_width']    = ($cmsgo['content_width']) ?: 538;
        $cmsgo['img_list_width']   = ($cmsgo['img_list_width']) ?: 100;
        $cmsgo['img_list_height']  = ($cmsgo['img_list_height']) ?: 75;
        $cmsgo['img_prev_width']   = ($cmsgo['img_prev_width']) ?: 538;
        $cmsgo['img_prev_height']  = ($cmsgo['img_prev_height']) ?: 400;
        $cmsgo['max_time']         = ($cmsgo['max_time']) ?: 1800;
