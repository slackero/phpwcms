...
<form action="setup.php?step=1" method="post" autocomplete="off">
        <table border="0" cellpadding="0" cellspacing="0" summary="">
          <tr>
            ...
            <td width="270"><input name="db_host" type="text" class="v12" id="db_host" value="<?php echo html_specialchars($cmsgo["db_host"]) ?>" placeholder="localhost" size="30" style="width:300px" /></td>
          ...
          <tr>
            ...
            <td><table border="0" cellpadding="0" cellspacing="0" summary="">
                <tr>
                <td><input name="db_pers" type="checkbox" id="db_pers" value="1" <?php echo ($cmsgo["db_pers"]) ? "checked" : ""; ?> /></td>
...
?>
    ...
    <tr>
            ...
            <td><select class="custom-select" name="charset">
            ...
            foreach($available_languages as $key => $value) {
                ...
                if($cmsgo['db_version'] < 40100 && !in_array($mysql_charset_map[$_lang_charset], $_availableCharsets)) continue;

                echo '<option value="'.$key.'"';

                if($key == strtolower(str_replace('-', '', $cmsgo['default_lang'])  .'-'. $cmsgo['charset'])) {
        ...
        if($result = mysqli_query($db, "SHOW COLLATION")) {
            ...
            while($row = mysqli_fetch_assoc($result)) {
                ...
                if($row['Charset'] == 'utf8' && in_array($row['Charset'], $mysql_charset_map)) {
                    if($cmsgo['db_collation']==$row['Collation']) {
...
?>
        <tr>
            ...
            <td><select class="custom-select" name="collation">
<?php
...
<?php
    ...
    if(!empty($sql_data)) {
        ...
        foreach($sql_data as $key => $value) {
            ...
            if($cmsgo['db_version'] > 40100 && strpos(strtoupper(trim($value)), 'INSERT') !== 0) {
                $sql_data[$key] .= ' DEFAULT';
                $sql_data[$key] .= ' CHARACTER SET '.$cmsgo['db_charset'];
                $sql_data[$key] .= ' COLLATE '.$cmsgo['db_collation'];
            }
...
?>
    <tr>
        ...
        <td><input name="admin_name" type="text" id="admin_name" class="v12" style="width:300px" value="<?php echo empty($cmsgo["admin_name"]) ? "Webmaster" : html_specialchars($cmsgo["admin_name"]) ?>" size="30" /></td>
    ...
    <tr>
        ...
        <td><input name="admin_user" type="text" id="admin_user" class="v12" style="width:300px" value="<?php echo empty($cmsgo["admin_user"]) ? "webmaster" : html_specialchars($cmsgo["admin_user"]) ?>" size="30" /></td>
...
?>
    ...
    <tr>
        ...
        <td><input name="admin_email" type="text" id="admin_email" class="v12" style="width:300px" value="<?php echo html_specialchars($cmsgo["admin_email"]) ?>" size="30" /></td>
...
<?php
        ...
        $user_check = _dbQuery('SELECT * FROM '.$_db_prepend."cmsgo_user WHERE usr_login='".mysqli_real_escape_string($db, $cmsgo['admin_user'])."'");

        if($user_check !== false && count($user_check)) {

            //hm user still exists - so try to update
            $sql  = "UPDATE ".$_db_prepend."cmsgo_user SET ";
            $sql .= "usr_login      = '".mysqli_real_escape_string($db, $cmsgo['admin_user'])."', ";
            $sql .= "usr_pass       = '".mysqli_real_escape_string($db, $cmsgo["admin_pass"])."', ";
            $sql .= "usr_email      = '".mysqli_real_escape_string($db, $cmsgo["admin_email"])."', ";
            $sql .= "usr_admin      = 1, ";
            $sql .= "usr_aktiv      = 1, ";
            $sql .= "usr_name       = '".mysqli_real_escape_string($db, $cmsgo['admin_name'])."', ";
            $sql .= "usr_lang       = '".mysqli_real_escape_string($db, $cmsgo['default_lang'])."', ";
            $sql .= "usr_wysiwyg    = 2, ";
            $sql .= "usr_fe         = 2 ";
            $sql .= "WHERE usr_login='".mysqli_real_escape_string($db, $cmsgo['admin_user'])."' LIMIT 1";
        ...
        if(!empty($create_user)) {
            ...
            echo '<strong>Done!</strong> Account for user <b>'.html_specialchars($cmsgo['admin_user']).'</b> was created.';
        ...
        if(!empty($update_user)) {
            ...
            // update
            echo '<img src="../img/famfamfam/icon_accept.gif" alt="Juchu" class="icon1" />';
            echo '<strong>Done!</strong> Account of user <b>'.html_specialchars($cmsgo['admin_user']).'</b> was updated.';
            echo '<input type="hidden" name="user_account" value="1" />';
        ...
        if($user_check === false) {
            ...
            echo '<p>Account for user <b>'.html_specialchars($cmsgo['admin_user']).'</b> was not created or updated.<br />Click <b>continue</b> to try again.</p>';
