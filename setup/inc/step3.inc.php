...
?>
<h2 class="h4 text-primary fw-normal mb-3">5. Site URL &amp; Email Transport</h2>
...
<form action="setup.php?step=3" method="post">
            ...
            <div class="form-group row mb-0">
                ...
                <div class="col-sm-6">
                    <div class="form-password">
                        <input name="smtp_pass" type="password" class="form-control" id="smtp_pass" value="<?php echo html_specialchars($phpwcms['SMTP_PASS'] ?? '') ?>" autocomplete="new-password" />
    ...
    <?php
    $_wl_key = !empty($phpwcms['whitelabel_key']) ? $phpwcms['whitelabel_key'] : (!empty($_SESSION['whitelabel_key']) ? $_SESSION['whitelabel_key'] : '');
    $_wl_payload = !empty($_wl_key) ? setup_validate_whitelabel_key($_wl_key) : false;
    if ($_wl_payload !== false):
        $_brand_name_val = (!empty($phpwcms['brand_name']) && $phpwcms['brand_name'] !== 'phpwcms')
            ? $phpwcms['brand_name']
            : (!empty($_SESSION['brand_name']) && $_SESSION['brand_name'] !== 'phpwcms'
                ? $_SESSION['brand_name']
                : (!empty($_wl_payload['brand_name']) && $_wl_payload['brand_name'] !== '*' ? $_wl_payload['brand_name'] : ''));
    ?>
    <div class="card mb-4 border">
        <div class="card-header bg-light fw-bold">Whitelabel Branding</div>
        <div class="card-body">
            <div class="form-group row">
                <label for="brand_name" class="col-sm-3 col-form-label fw-bold">Brand Name</label>
                <div class="col-sm-6">
                    <input name="brand_name" type="text" class="form-control" id="brand_name" value="<?php echo html_specialchars($_brand_name_val) ?>" placeholder="phpwcms" maxlength="100" />
                </div>
                <div class="col-sm-3 form-text text-muted small align-self-center">Custom product name replacing phpwcms</div>
            </div>
            <div class="form-group row">
                <label for="brand_url" class="col-sm-3 col-form-label fw-bold">Brand Website URL</label>
                <div class="col-sm-6">
                    <input name="brand_url" type="url" class="form-control" id="brand_url" value="<?php echo html_specialchars($phpwcms['brand_url'] ?? '') ?>" placeholder="https://example.com" />
                </div>
                <div class="col-sm-3 form-text text-muted small align-self-center">Custom website URL</div>
            </div>
            <div class="form-group row mb-0">
                <label for="brand_copyright" class="col-sm-3 col-form-label fw-bold">Brand Copyright</label>
                <div class="col-sm-6">
                    <input name="brand_copyright" type="text" class="form-control" id="brand_copyright" value="<?php echo html_specialchars($phpwcms['brand_copyright'] ?? '') ?>" placeholder="<?php echo html_specialchars($_wl_payload['licensee'] ?? '') ?>" />
                </div>
                <div class="col-sm-3 form-text text-muted small align-self-center">Custom copyright text (defaults to licensee)</div>
            </div>
        </div>
    </div>
    <?php endif; ?>
