...
function write_conf_file($val) {
    ...
    $conf_file .= "\$cmsgo['db_port'] = " . (empty($val['db_port']) || !intval($val['db_port']) ? 3306 : (int)$val['db_port']) . ";\n";
    ...
    $conf_file .= "\$cmsgo['site_ssl_mode'] = 0; // turns the SSL Support of WCMS on (1) or off (0), default value 0\n";
    ...
    $conf_file .= "\$cmsgo['content_width'] = " . intval($val['content_width']) . "; //max width of the article content column - important for rendering multi column images\n";
    $conf_file .= "\$cmsgo['img_list_width'] = " . intval($val['img_list_width']) . "; //max with of the list thumbnail image\n";
    $conf_file .= "\$cmsgo['img_list_height'] = " . intval($val['img_list_height']) . "; //max height of the list thumbnail image\n";
    $conf_file .= "\$cmsgo['img_prev_width'] = " . intval($val['img_prev_width']) . "; //max width of the large preview image\n";
    $conf_file .= "\$cmsgo['img_prev_height'] = " . intval($val['img_prev_height']) . "; //max height of the large preview image\n";
    $conf_file .= "\$cmsgo['max_time'] = " . intval($val['max_time']) . "; //logout after max_time/60 seconds\n";
    $conf_file .= "\$cmsgo['responsive'] = 1; // 0 max. image width = \$cmsgo['content_width'], 1 = as given\n";
    $conf_file .= "\$cmsgo['preserve_image_name'] = 0; // keep file name for resized versions of the image\n";
    ...
    $conf_file .= "\$cmsgo['wysiwyg_editor'] = 2; //0 = no wysiwyg editor, 1 = CKEditor (legacy), 2 = TinyMCE 8\n";
    ...
    $conf_file .= "\$cmsgo['allow_cntPHP_rt'] = 0; //allow PHP replacement tags and includes in content parts\n";
    $conf_file .= "\$cmsgo['GETparameterName'] = 'id'; //must have a minimum of 2 chars \n";
    $conf_file .= "\$cmsgo['BOTS'] = array('googlebot', 'msnbot', 'bingbot', 'baiduspider', 'yandex', 'sosospider', 'ia_archiver', 'altavista', 'slurp', 'yahoo', 'jeeves', 'teoma', 'lycos', 'crawler'); //don't start session \n";
    $conf_file .= "\$cmsgo['mode_XHTML'] = 3; // Doctype: 1 = XHTML 1.0 Transitional, 0 = HTML 4.01 Transitional, 2 = XHTML 1.0 Strict, 3 = HTML5 \n";
    $conf_file .= "\$cmsgo['header_XML'] = 0; // Content Type: 1 = application/xhtml+xml, 0 = text/html \n";
    $conf_file .= "\$cmsgo['IE7-js'] = 0; // load IE7-js - fix for HTML/CSS/PNG bugs in IE\n";
    ...
    $conf_file .= "\$cmsgo['gd_memcheck_off'] = 0; // disable GD php memory check before resize an image \n";
    $conf_file .= "\$cmsgo['enable_chat'] = 0; // enable or disable chat function, by default it is disabled - not recommend anymore to use it \n";
    $conf_file .= "\$cmsgo['enable_messages'] = 0; // enable or disable internal messags, by default it is disabled - not recommend anymore to use it \n";
    ...
    $conf_file .= "\$cmsgo['video-js'] = ''; // can be stored locally too 'template/lib/video-js/ (https://vjs.zencdn.net/8.23.4/)\n";
    ...
    $conf_file .= "\$cmsgo['wkhtmltopdf_path'] = ''; // used for generating PDF, use full path including application name '/usr/bin/wkhtmltopdf'\n";
    $conf_file .= "\$cmsgo['render_clean_html'] = 0; // clean up HTML source a bit, experimental can have unexpected side effects\n";
    $conf_file .= "\$cmsgo['browser_check'] = array('fe' => false, 'be' => false, 'vs' => '', 'insecure' => true, 'required' => ''); // enable Browser Update check in frontend and/or backend, use 'vs' to which browser version, see http://www.browser-update.org/index.html#install\n";
    $conf_file .= "\$cmsgo['usergroup_support'] = false; // set true or false to support/disable this feature, is experimental\n";
    $conf_file .= "\$cmsgo['force301_id2alias'] = false; // send 301 HTTP Redirect when article/structure has alias but ID is given\n";
    $conf_file .= "\$cmsgo['force301_2struct'] = false; // send 301 HTTP Redirect to structure level when only 1 article is inside\n";
    $conf_file .= "\$cmsgo['allow_empty_alias'] = false; // do not auto-create (default) alias when alias field is empty\n";
    ...
    $conf_file .= "\$cmsgo['set_category_active'] = 1; // activate (1) or disable (0) category/structure level by default on create\n";
    $conf_file .= "\$cmsgo['set_file_active'] = 1; // activate (1) or disable (0) files and folders by default on create\n";
    $conf_file .= "\$cmsgo['set_news_active'] = 1; // activate (1) or disable (0) news by default on create\n";
    $conf_file .= "\$cmsgo['log_404error'] = false; // log each 404 for redirect edit\n";
    $conf_file .= "\$cmsgo['set_sociallink'] = array('article' => false, 'articlecat' => false, 'news' => false, 'shop' => false, 'render' => true); // TRUE/FALSE to enable status for article/articlecat/news/shop by default, render TRUE/FALSE to enable/disable in frontend\n";
    $conf_file .= "\$cmsgo['header_comment'] = '';\n";
    $conf_file .= "\$cmsgo['cnt_sort'] = 'a-z'; // not set or empty or false like before; 'a-z' or reverse 'z-a'\n";
    $conf_file .= "\$cmsgo['cmsimage_redirect'] = false; // redirect to the resized/cropped image if true\n";
    $conf_file .= "\$cmsgo['disable_next_prev'] = false; // https://support.google.com/webmasters/answer/1663744\n";
    $conf_file .= "\$cmsgo['allowed_upload_ext'] = 'jpg,jpeg,png,gif,tif,tiff,bmp,pic,psd,eps,ai,svg,pdf,ps,doc,docx,xls,xlsx,ppt,pptx,odt,odm,odg,ods,odp,odf,odc,odb,sxw,sxc,sxi,csv,txt,rtf,html,xml,ini,sql,db,zip,rar,7z,s7z,dmg,bz2,gz,tar,tgz,mkv,webm,vob,ogg,ogv,mov,qt,wmv,mpg,mpeg,mp3,mp4,m4p,flv,f4v,f4p,f4a,f4b';\n";
    $conf_file .= "\$cmsgo['enable_inline_php'] = false; // disable [PHP] {PHP…} … by default\n";
    ...
    $conf_file .= "\$cmsgo['remove_empty_get_vars'] = true; // If true all GET parameters without a value except the alias will be deleted\n";
