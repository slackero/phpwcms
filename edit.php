...
if(isset($_POST['form_aktion']) && $_POST['form_aktion'] == 'login' && $json_check === 1) {
    ...
    $sql_query  = "SELECT * FROM " . DB_PREPEND . "cmsgo_user WHERE usr_login=" . _dbEscape($wcs_user) . " AND usr_aktiv=1 AND (usr_fe=1 OR usr_fe=2)";
            ...
            if ($plain_pass !== '') {
                if (str_starts_with($db_pass, '$')) {
                    if (password_verify($plain_pass, $db_pass)) {
                        ...
                        $md5_pass = md5(makeCharsetConversion($plain_pass, CMSGO_CHARSET, 'utf-8'));
                        if (password_verify($md5_pass, $db_pass)) {
                            $login_passed = 1;
                            $new_hash = password_hash($plain_pass, PASSWORD_DEFAULT);
                            _dbQuery('UPDATE ' . DB_PREPEND . 'cmsgo_user SET usr_pass=' . _dbEscape($new_hash) . ' WHERE usr_id=' . (int)$result[0]['usr_id'], 'UPDATE');
                        }
                    }
                } else {
                    $md5_pass = md5(makeCharsetConversion($plain_pass, CMSGO_CHARSET, 'utf-8'));
