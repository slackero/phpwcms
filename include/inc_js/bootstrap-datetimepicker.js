    ...
    var DateTimePicker = function($, moment) {
        ...
        var trim = function trim(str) {
            ...
            Event = {
                ...
                CLICK_DATA_API: 'click' + EVENT_KEY + DATA_API_KEY, //emitted
        ...
        var DateTimePicker = /*#__PURE__*/function() {
            ...
            function DateTimePicker(element, options) {
                ...
                // Custom modification (2026-06-26): initialise toggle-cooldown timestamp so show()
                // always has a defined numeric value to compare against.
            ...
            _proto._setValue = function _setValue(targetMoment, index) {
                // Custom modification (2026-06-26): safeguard against -1 index if value is deleted/cleared
                if (index === -1) {
                    index = 0;
                }
    ...
    var TempusDominusBootstrap4 = function($) {
        ...
        var TempusDominusBootstrap4 = /*#__PURE__*/function(_DateTimePicker) {
            ...
            _proto2._fillDate = function _fillDate() {
                ...
                // Custom modification (2026-06-26): cache $('body') to avoid four separate DOM lookups.
                var $body = $('body');
            ...
            _proto2._fillTime = function _fillTime() {
                ...
                var timeComponents = this.widget.find('.timepicker span[data-time-component]'), // Custom modification (2026-06-26): fall back to current browser moment when input is empty/cleared,
                    // so the timepicker UI renders valid hour/minute/second values instead of being blank.
                    lastPickedDate = this._getLastPickedDate() || this.getMoment();

                if (!this.use24Hours) {
                    toggle = this.widget.find('.timepicker [data-action=togglePeriod]');
                    // Custom modification (2026-06-26): newDate ternary is now always truthy;
                    // lastPickedDate is guaranteed non-null via the getMoment() fallback above.
                    newDate = lastPickedDate.clone().add(lastPickedDate.hours() >= 12 ? -12 : 12, 'h');
                    toggle.text(lastPickedDate.format('A'));
                ...
                // Custom modification (2026-06-26): removed redundant `lastPickedDate &&` guards —
                // lastPickedDate is now guaranteed non-null via the `|| this.getMoment()` fallback above.
                timeComponents.filter('[data-time-component=hours]').text(lastPickedDate.format('' + (this.use24Hours ? 'HH' : 'hh')));
            ...
            _proto2._doAction = function _doAction(e, action) {
                var lastPicked = this._getLastPickedDate() || this.getMoment();
            ...
            _proto2.hide = function hide() {
                ...
                this.widget.remove();
                this.widget = false;
                // Custom modification (2026-06-26): record when the picker was hidden so show() can detect
                // an immediate reopen caused by blur -> button click event sequencing.
            ...
            _proto2.show = function show() {
                // Custom modification (2026-06-26): prevent the picker immediately reopening after a toggle-button
                // click. When the user clicks the calendar/clock icon while the picker is open, the
                // input blur fires first (hiding the picker) and then the click fires (calling show()).
                // The 250ms cooldown stops show() from reopening in that scenario.
                if (this.lastHiddenTime && Date.now() - this.lastHiddenTime < 250) {
                    this.lastHiddenTime = 0;
                    return;
                }
                var currentMoment,
                ...
                // Custom modification (2026-06-26): combined into one selector to avoid three separate DOM queries.
                this.widget.find('.timepicker-hours, .timepicker-minutes, .timepicker-seconds').hide();
