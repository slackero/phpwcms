...
$content['images']              = [];
$content['opengraph']           = ['support' => true, 'type' => 'website', 'render' => !empty($cmsgo['set_sociallink']['render'])]; // will hold all relevant open graph information
$pagelayout                     = [];
...
$cmsgo['preview_mode']          = false;

// reset force redirect in case it is a POST
if(!empty($_POST)) {
    $cmsgo['force301_id2alias']   = false;
    $cmsgo['force301_2struct']    = false;

    // handle preview mode
} elseif(isset($_GET['cmsgo-preview'])) {
    $cmsgo['force301_id2alias']   = false;
    $cmsgo['force301_2struct']    = false;
    unset($_GET['cmsgo-preview'], $_getVar['cmsgo-preview']);
    checkLoginCount();
    if(empty($_SESSION['wcs_user'])) {
        headerRedirect(abs_url());
    }
    $cmsgo['preview_mode']        = true;
}

define('PREVIEW_MODE', $cmsgo['preview_mode']);
...
if(isset($_GET['id'])) {
    ...
    if(!isset($content['struct'][ $aktion[0] ])) {
        ...
        if($aktion[1]) {
            $sql  = 'SELECT article_id, article_cid FROM '.DB_PREPEND.'cmsgo_article WHERE ';
    ...
    if($content['404error']['status'] === false && !empty($cmsgo['force301_id2alias']) && !empty($content['struct'][ $aktion[0] ]['acat_alias'])) {
    ...
    if($_GET['aid']) {
        ...
        $sql  = 'SELECT article_cid, article_alias FROM '.DB_PREPEND.'cmsgo_article WHERE ';
        ...
        if(isset($result[0]['article_cid'])) {
            ...
            // Force 301 Redirect when alias is available
            if(!empty($cmsgo['force301_id2alias']) && !$content['aId_CpPage'] && !empty($result[0]['article_alias'])) {
...
if(isset($_GET['print'])) {
    ...
    $aktion[2] = 1;
    define('PRINT_PDF', $_GET['print'] == 2 && !empty($cmsgo['wkhtmltopdf_path']));
...
if($content['404error']['status'] === true) {
    ...
    if($content['404error']['alias'] == 'r404' || (isset($_SERVER['REDIRECT_STATUS']) && $_SERVER['REDIRECT_STATUS'] == 404)) {
        ...
        // try to detect and remove $cmsgo[root]
        if(!empty($_SERVER['REDIRECT_URL'])) {

            $content['404error']['redirect_url'] = trim($_SERVER['REDIRECT_URL'], '/');

            if($cmsgo['root'] && $content['404error']['redirect_url']) {
                $content['404error']['redirect_url'] = substr_replace($content['404error']['redirect_url'], '', 0, strlen($cmsgo['root']));
            }
        }
    }
    ...
    if($content['404error']['redirect_url']) {
        ...
        if($cmsgo['rewrite_url'] && $cmsgo['rewrite_ext']) {

            $content['404error']['rewrite_ext_length'] = strlen($cmsgo['rewrite_ext']);

            if(substr($content['404error']['redirect_url'], $content['404error']['rewrite_ext_length'] * -1) === $cmsgo['rewrite_ext']) {
    ...
    if($content['404error']['status']) {
        ...
        $content['404error']['result'] = _dbGet('cmsgo_redirect', '*', $content['404error']['where']);

        if(isset($content['404error']['result'][0])) {

            $content['404error']['result'] = $content['404error']['result'][0];

            _dbUpdate('cmsgo_redirect', ['views' => intval($content['404error']['result']['views']) + 1], 'rid='.$content['404error']['result']['rid']);
            ...
            if($content['404error']['result']['active'] == 1) {
                ...
                if(empty($content['404error']['result']['type'])) {
                    ...
                    $content['404error']['result']['target'] = CMSGO_URL . $content['404error']['result']['target']['link'];
        ...
        } elseif(!empty($cmsgo['log_404error'])) {

            // Store failed page access
            _dbInsert('cmsgo_redirect', [
...
$cmsgo['output_action'] = false;
if(!empty($_GET['cmsgo_output_action']) || !empty($_POST['cmsgo_output_action'])) {

    // split by function - value: F-function1|function2|function3--S-SECT1|SECT2|SECT3
    $cmsgo['output_action'] = explode('--', clean_slweg( empty($_GET['cmsgo_output_action']) ? $_POST['cmsgo_output_action'] : $_GET['cmsgo_output_action'] ));
    unset(
        $_GET['cmsgo_output_action'],
        $_POST['cmsgo_output_action'],
        $_getVar['cmsgo_output_action']
    );

    if(is_array($cmsgo['output_action'])) {
        $cmsgo['output_function'] = [];
        $cmsgo['output_section']  = [];

        foreach($cmsgo['output_action'] as $value) {
            $value = trim($value);
            ...
            if(is_array($value)) {
                foreach($value as $_value) {
                    $_value = trim($_value);
                    if($_value != '') {
                        $cmsgo[$output_key][$_value] = $_value;
                    }
                }
            }
        }

        $cmsgo['output_action'] = count($cmsgo['output_function']) || count($cmsgo['output_section']);
    } else {
        $cmsgo['output_action'] = false;
    }
}
...
if(!CMSGO_SSL && (!empty($cmsgo['site_ssl_mode']) || !empty($content['struct'][ $content['cat_id'] ]['acat_ssl']))) {
    ...
    if($query_string && !CMSGO_REWRITE) {
        $query_string = 'index.php' . $query_string;
    }
    headerRedirect($cmsgo['site_ssl_url'] . $query_string, 301);
}
...
if(!empty($content['struct'][ $content['cat_id'] ]['acat_template'])) {
    ...
    $sql  = 'SELECT template_var FROM ' .DB_PREPEND. 'cmsgo_template WHERE template_trash=0 AND ';
...
if(!isset($block)) {
    ...
    $sql  = 'SELECT template_var FROM ' .DB_PREPEND. 'cmsgo_template ';
...
// template defaults
$template_default['classes'] = isset($template_default['classes']) ? array_merge($cmsgo['default_template_classes'], $template_default['classes']) : $cmsgo['default_template_classes'];
$template_default['attributes'] = isset($template_default['attributes']) ? array_merge($cmsgo['default_template_attributes'], $template_default['attributes']) : $cmsgo['default_template_attributes'];
...
if(!empty($block['overwrite'])) {
    ...
    @include CMSGO_TEMPLATE.'inc_settings/template_default/'.$block['overwrite'];
}
...
if(isset($template_default['search_highlight']['prefix'])) {
    $cmsgo['search_highlight'] = array_merge($cmsgo['search_highlight'], $template_default['search_highlight']);
}

// load frontend JavaScript lib file
require CMSGO_ROOT.'/include/inc_front/js.inc.php';
...
$sql  = 'SELECT pagelayout_var FROM ' .DB_PREPEND. 'cmsgo_pagelayout WHERE pagelayout_trash=0 ';
...
if(empty($pagelayout)) {
    ...
    die('There is no pagelayout available. Please <a href="'.
        CMSGO_URL.get_login_file().'">login</a> to the admin section and <a href="'.
        CMSGO_URL.'cmsgo.php?do=admin&amp;p=8">create one here</a>!');
}
// Pagetitle
...
$cmsgo['donottrack'] = !empty($block['donottrack']) && isset($_SERVER['HTTP_DNT']) && $_SERVER['HTTP_DNT'] === '1';
$cmsgo['cookie_consent'] = false;
$cmsgo['cookie_consent_dismiss'] = false;
if (empty($block['require_consent']['cookie_name'])) {
    if (empty($cmsgo['cookie_consent_name'])) {
        $cmsgo['cookie_consent_name'] = 'cookieconsent_dismissed';
    }
} else {
    $cmsgo['cookie_consent_name'] = $block['require_consent']['cookie_name'];
}
if (empty($block['require_consent']['cookie_value'])) {
    if (empty($cmsgo['cookie_consent_value'])) {
        $cmsgo['cookie_consent_value'] = 'yes';
    }
} else {
    $cmsgo['cookie_consent_value'] = $block['require_consent']['cookie_value'];
}
if (!empty($block['require_consent']['enable'])) {
    // Todo: make decision based on cookie consent version v2 or v3
    if (isset($_COOKIE[$cmsgo['cookie_consent_name']])) {
        $cmsgo['cookie_consent'] = true;
        if (strpos($_COOKIE[$cmsgo['cookie_consent_name']], $cmsgo['cookie_consent_value']) !== false) {
            $cmsgo['cookie_consent_dismiss'] = true;
        } else {
            $cmsgo['donottrack'] = true;
        }
    } else {
        $cmsgo['donottrack'] = true;
    }
...
// default dir: "cmsgo_template/inc_script/frontend_init"; only *.php files are allowed there
if($cmsgo['allow_ext_init']) {
    if(count($custom_includes = get_tmpl_files(CMSGO_TEMPLATE.'inc_script/frontend_init', 'php'))) {
        foreach($custom_includes as $value) {
            include_once CMSGO_TEMPLATE.'inc_script/frontend_init/'.$value;
        }
    }
}
// include custom frontend init scripts based on module definitions
if(count($cmsgo['modules_fe_init'])) {
    foreach($cmsgo['modules_fe_init'] as $value) {
        include_once $value;
    }
}
...
if(!PERMIT_ACCESS && !_getFeUserLoginStatus()) {
    ...
    if(!empty($block['feloginurl'])) {
        $template_default['login_form_url'] = str_replace('{SITE}', CMSGO_URL, $block['feloginurl']);
    } elseif(empty($template_default['login_form_url'])) {
        $template_default['login_form_url'] = CMSGO_URL;
    }
...
if($content['set_canonical'] && !empty($cmsgo['force301_2struct']) && !$content['aId_CpPage'] && empty($content['struct'][ $aktion[0] ]['acat_disable301']) && !empty($content['struct'][ $aktion[0] ]['acat_alias']) && (!defined('CMSGO_ALIAS') || CMSGO_ALIAS != $content['struct'][ $aktion[0] ]['acat_alias'])) {
    headerRedirect(abs_url([], [], $content['struct'][ $aktion[0] ]['acat_alias'], 'urlencode'), 301);
...
if(empty($cmsgo['enable_deprecated'])) {
    $cmsgo['enable_deprecated'] = false;
} else {
    include_once CMSGO_ROOT. '/include/inc_front/deprecated.inc.php';
}

// check if current category should be cached
if($content['struct'][$content['cat_id']]['acat_timeout'] != '') {
    $cmsgo['cache_timeout'] = $content['struct'][$content['cat_id']]['acat_timeout'];
}
...
if(!empty($cmsgo['use_content_lang']) && !empty($content['struct'][$content['cat_id']]['acat_lang']) && $content['struct'][$content['cat_id']]['acat_lang'] !== $cmsgo['DOCTYPE_LANG']) {
    $cmsgo['DOCTYPE_LANG'] = $content['struct'][$content['cat_id']]['acat_lang'];
    $cmsgo['default_lang'] = $content['struct'][$content['cat_id']]['acat_lang'];
}
...
if($aktion[1]) {
    ...
    include_once CMSGO_ROOT. '/include/inc_front/content.article.inc.php';
...
if($content['overwrite_canonical']) {
    ...
    if($_test_canonical_schema !== 'http') {
        ...
        if($_test_canonical_schema === '{SIT') {
            $content['overwrite_canonical'] = str_replace('{SITE}', CMSGO_URL, $content['overwrite_canonical']);
        } else {
            $content['overwrite_canonical'] = CMSGO_URL . $content['overwrite_canonical'];
        }
    }
...
if($aktion[2] == 0) {
    ...
    $content['all'] = is_file(CMSGO_TEMPLATE.'inc_default/'.$_print_tmpl.'.tmpl') ? @file_get_contents(CMSGO_TEMPLATE.'inc_default/'.$_print_tmpl.'.tmpl') : '{CONTENT}<hr />{CURRENT_URL}';
    if(PRINT_PDF) {
...
$content['all'] = str_replace('{SITE}', CMSGO_URL, $content['all']);
...
if(empty($cmsgo['parse_html_mode']) || substr($cmsgo['parse_html_mode'], 0, 6) === 'before') {
...
if(!defined('CMSGO_ALIAS')) {
    if(empty($content['struct'][ $content['cat_id'] ]['acat_alias'])) {
        define('CMSGO_ALIAS', empty($aktion[1]) ? 'id='.$content['cat_id'] : 'aid='.$aktion[1]);
    } else {
        define('CMSGO_ALIAS',  $content['struct'][ $content['cat_id'] ]['acat_alias']);
    }
}

// try to include custom functions and replacement tags or what you want to do at this point of the script
// default dir: "cmsgo_template/inc_script/frontend_render"; only *.php files are allowed there
if($cmsgo['allow_ext_render']) {
    if(count($custom_includes = get_tmpl_files(CMSGO_TEMPLATE.'inc_script/frontend_render', 'php'))) {
        foreach($custom_includes as $value) {
            include_once CMSGO_TEMPLATE.'inc_script/frontend_render/'.$value;
        }
    }
}
if(count($cmsgo['modules_fe_render'])) {
    foreach($cmsgo['modules_fe_render'] as $value) {
        include_once $value;
    }
}

// parse replacements to HTML before frontend render
if(!empty($cmsgo['parse_html_mode']) && substr($cmsgo['parse_html_mode'], -5) === 'after') {
    $content['all'] = html_parser($content['all']);
...
$content['all'] = str_replace('{LAZY_LOADING}', CMSGO_LAZY_LOADING, $content['all']);
...
if($content['opengraph']['render']) {
    ...
    if(empty($content['opengraph']['url'])) {
        set_meta('og:url', abs_url([], ['cmsgo_output_action', 'print', 'cmsgo-preview', 'unsubscribe', 'subscribe']), 'property');
    } else {
...
if(strpos($content['all'], '[PRINT]') !== false) {
    $content['all'] = str_replace('[PRINT]', '<a href="'.rel_url(['print'=>1], [], CMSGO_ALIAS).'" class="'.$template_default['classes']['link-print'].'" target="_blank" rel="nofollow">', $content['all']);
    $content['all'] = str_replace('[/PRINT]', '</a>', $content['all']);
}
if(strpos($content['all'], '[PRINT_PDF]') !== false) {
    $content['all'] = str_replace('[PRINT_PDF]', '<a href="'.rel_url(['print'=>2], [], CMSGO_ALIAS).'" class="'.$template_default['classes']['link-print-pdf'].'" target="_blank" rel="nofollow">', $content['all']);
...
if(!empty($_CpPaginate)) {
    ...
    if(strpos($content['all'], '{CP_PAGINATE}')) {
        ...
        foreach($content['CpPages'] as $key => $value) {
            ...
            $content['CpPaginateNavi'][ $key ] .= '<a href="' . rel_url([], [], $key ? 'aid='.$aktion[1].'-'.$key : CMSGO_ALIAS) . '" class="';
    ...
    if(in_array($content['CpPages'][ $content['aId_CpPage'] ] - 1, $content['CpPages'])) {
        ...
        $value = abs_url([], [], $key ? 'aid='.$aktion[1].'-'.$key : CMSGO_ALIAS);
        $content['all'] = render_cnt_template($content['all'], 'CP_PAGINATE_PREV', $value);

        if(empty($cmsgo['disable_next_prev'])) {
    ...
    if(strpos($content['all'], '[CP_PAGINATE_MENU') !== false) {
        ...
        foreach($content['CpPageTitles'] as $key => $value) {
            ...
            $content['CpItem'] = '<a href="' . rel_url([], [], $key ? 'aid='.$aktion[1].'-'.$key : CMSGO_ALIAS) . '"';
...
if (!$cmsgo['cookie_consent']) {
    ...
    if (!empty($block['cc_v3']['enable'])) {
        ...
        $cc_v3_translations = ['en', 'de'];
        if (!in_array($cmsgo['lang'], $cc_v3_translations)) {
            $cc_v3_translations[] = $cmsgo['lang'];
        }
        foreach ($cc_v3_translations as $cc_v3_lang) {
            if (is_file(CMSGO_TEMPLATE . 'lib/cookieconsent3/lang/' . $cc_v3_lang . '.json')) {
                $cc_v3_config_lang = json_decode(
                    file_get_contents(CMSGO_TEMPLATE . 'lib/cookieconsent3/lang/' . $cc_v3_lang . '.json'),
                    true
                );
        ...
        if (!empty($block['cc_v3']['sections'])) {
            ...
            // =================
            // General
            if (!empty($block['cc_v3']['sections']['general']['active'])) {

            }
            // Necessary
            if (!empty($block['cc_v3']['sections']['necessary']['active'])) {
                $cc_v3_config['categories']['necessary'] = ['readOnly' => true];
            }
            // Functionality
            if (!empty($block['cc_v3']['sections']['functionality']['active'])) {
                $cc_v3_config['categories']['functionality'] = ['readOnly' => false];
            }
            // Analytics
            if (!empty($block['cc_v3']['sections']['analytics']['active'])) {
                $cc_v3_config['categories']['analytics'] = ['readOnly' => false];
            }
            // Marketing
            if (!empty($block['cc_v3']['sections']['marketing']['active'])) {
                $cc_v3_config['categories']['marketing'] = ['readOnly' => false];
            }
            // Social Media
            if (!empty($block['cc_v3']['sections']['social']['active'])) {
                $cc_v3_config['categories']['social'] = ['readOnly' => false];
            }
            // More information
            if (!empty($block['cc_v3']['sections']['more']['active'])) {

            }
        }

        $block['custom_htmlhead']['cookieconsent.js'] .= '<script' . SCRIPT_ATTRIBUTE_TYPE . '>' . LF . SCRIPT_CDATA_START . LF;
        $block['custom_htmlhead']['cookieconsent.js'] .= '// language: ' . $cmsgo['lang'] . LF;
        $block['custom_htmlhead']['cookieconsent.js'] .= 'CookieConsent.run(' . json_encode($cc_v3_config) . ');';
    ...
    elseif (!empty($block['cookie_consent']['enable'])) {
        ...
        } elseif (!CMSGO_USE_CDN && strpos($block['cookie_consent']['theme'], '.css') === false) {
            $block['cookie_consent']['options']['theme'] = CMSGO_URL . TEMPLATE_PATH . 'lib/cookieconsent2/' . $block['cookie_consent']['theme'] . '.css';
        } else {
        ...
        if (CMSGO_USE_CDN) {
            $block['custom_htmlhead']['cookieconsent.js'] .= '  <script src="' . CMSGO_HTTP_SCHEMA . '://cdnjs.cloudflare.com/ajax/libs/cookieconsent2/1.0.10/cookieconsent.min.js"></script>';
        } else {
            $block['custom_htmlhead']['cookieconsent.js'] .= '  <script src="' . CMSGO_URL . TEMPLATE_PATH . 'lib/cookieconsent2/cookieconsent.min.js"></script>';
        }
    }
}
  ...
  <script" . SCRIPT_ATTRIBUTE_TYPE . ">
    ...
    function gaOptout() {
        ...
        $block['tracking_ga']['config'] = [
            "cookie_comain: '" . ($cmsgo['session_cookie_params']['domain'] ?? CMSGO_DOMAIN) . "'"
        ];
...
if(!empty($GLOBALS['cmsgo']['detect_pixelratio']) && $cmsgo['USER_AGENT']['pixelratio'] == 1 && empty($_COOKIE['cmsgo_pixelratio'])) {
    $block['custom_htmlhead']['pixelratio']  = '  <script'.SCRIPT_ATTRIBUTE_TYPE.'>' . LF;
    $block['custom_htmlhead']['pixelratio'] .= '    var cmsgo_pixelratio = typeof window.devicePixelRatio === "undefined" ? 1 : window.devicePixelRatio;' . LF;
    $block['custom_htmlhead']['pixelratio'] .= '    document.cookie = "cmsgo_pixelratio="+cmsgo_pixelratio+";expires='.date('r', now()+2592000).'";' . LF; // 30 days
...
if(strpos($content['all'], 'index.php?aid=') || strpos($content['all'], 'index.php?id=')) {
    ...
    if(strpos($content['all'], CMSGO_URL.'index.php?')) {
        $all_reset_url = true;
        $content['all'] = str_replace(CMSGO_URL.'index.php?', '###CMSGO_URL###index.php?', $content['all']);
    } else {
        $all_reset_url = false;
    }
    ...
    if( isset($match[1]) && isset($match[2]) ) {
        ...
        if(count($all_id)) {
            $sql_id   = "SELECT 'id' AS alias_type, acat_id AS id, 0 AS aid, acat_alias AS alias FROM ".DB_PREPEND.'cmsgo_articlecat ';
            $sql_id  .= 'WHERE acat_id IN (' . implode(',', $all_id) . ") AND acat_alias != ''";
        }

        if(count($all_aid)) {
            $sql_aid  = "SELECT 'aid' AS alias_type, article_cid AS id, article_id AS aid, article_alias AS alias FROM ".DB_PREPEND.'cmsgo_article ';
...
if(!empty($cmsgo['gt_mod']) && strpos($content['all'], '{GT') !== false) {
