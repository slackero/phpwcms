...
//CMSGO_TEMPLATE.'calendar/calendar.ini'
...
function getCurrentCalendarDate()
{
	...
	if (!empty($_getVar['calendardate'])) {
		...
		if (($int_time = cmsgo_strtotime($_date['year'] . '/' . $_date['month'] . '/' . $_date['day'])) === false) {
			$_date['year'] = date('Y');
			$_date['month'] = date('n');
			$_date['day'] = date('j');
		} else {
	...
	$_date['prev_month']['month'] = ($_date['month'] == 1) ? 12 : (int)$_date['month'] - 1;
