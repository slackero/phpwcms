...
function breadcrumb_wrapper($match) {
    ...
    if(empty($match[2])) {
        ...
        $cat_only = (bool)intval(trim($match[2], ','));
    }
...
function breadcrumb($start_id, $struct_array, $end_id=0, $spacer=' &gt; ', $cat_only=false) {
        ...
        foreach($data as $key => $value) {
            ...
            if(!$hidden) { // check if the structure should be unvisible when active
                ...
                if ($act_id != $key) {
                    ...
                    $breadcrumb[$key] .= $nolink_prefix . html($value) . $nolink_suffix;
    ...
    if($with_article && isset($GLOBALS['content']["article_menutitle"])) {
        ...
        $breadcrumb['article']  = '<a href="' . rel_url(null, true, defined('CMSGO_ALIAS') ? html(CMSGO_ALIAS) : 'aid='.$GLOBALS['content']["article_id"]);
...
function get_struct_data($root_name='', $root_info='') {
    ...
    $sql  = "SELECT * FROM ".DB_PREPEND."cmsgo_articlecat WHERE ";
...
function get_actcat_articles_data($act_cat_id) {
    ...
    if(!PREVIEW_MODE) {
        ...
        if($content['struct'][ $act_cat_id ]['acat_archive'] == 0) {
            ...
            $sql .= " AND IF(article_archive_status=1, 1, (article_end IS NULL OR article_end > NOW()))";
        }
    }
    $sql .= ' ORDER BY '.$ao[2];
    ...
    if(isset($result[0]['article_id'])) {
        foreach($result as $row) {
            ...
            if($row["article_aliasid"]) {
                ...
                $alias_sql .= "FROM ".DB_PREPEND."cmsgo_article ";
...
function add_linkid($img='', $linkid='') {
    ...
    return preg_replace('/( \/>|>)$/', $linkid."$1", $img);
}
...
function build_levels($struct, $level, $temp_tree, $act_cat_id, $nav_table_struct, $count, $div, $link_to) {
    ...
    $space_cell     = '';
    $space_row      = '';
    $cell_height    = $nav_table_struct["cell_height"] ?: 1;
    ...
    foreach($struct as $key => $value) {
        ...
        if( _getStructureLevelDisplayStatus($key, $level) ) {
            ...
            if(!$value["acat_redirect"]) {
                $link = rel_url(array(), array('newsdetail'), $value["acat_alias"] ?: 'id='.$key);
                $redirect['target'] = '';
            } else {
            ...
            if($nav_table_struct["js_over_effects"]) {
                ...
                if($redirect['target'] != ' target="_blank"') {
                    ...
                    $js .= " onclick=\"window.open('".js_singlequote($link)."', 'cmsgonewwin');return false;\"";
                }
            ...
            if(!empty($value["acat_class"])) {
                $nav_table_struct_temp = $nav_table_struct;
                $nav_table_struct["row_norm_class"]     = trim($nav_table_struct["row_norm_class"].' '. $value["acat_class"]);
                $nav_table_struct["row_active_class"]   = trim($nav_table_struct["row_active_class"].' '. $value["acat_class"]);
                $nav_table_struct["row_space_class"]    = 'row_space '. $value["acat_class"];
            } else {
            ...
            if(!empty($temp_tree[$key])) {
                ...
                if($act_cat_id == $key || (!empty($nav_table_struct["all_nodes_active"]) && isset($GLOBALS['LEVEL_KEY'][$key]))) {
                    ...
                    $temp_menu .= html_specialchars($value["acat_name"]);
        ...
        if($temp_counter < $temp_topcount) {
            ...
            if(!empty($article["article_image"]["list_hash"])) {
                ...
                $thumb_image = get_cached_image(array(
                    ...
                    "thumb_name"    =>  md5($article["article_image"]['list_hash'].$article["article_image"]['list_width'].$article["article_image"]['list_height'].$GLOBALS['cmsgo']["sharpen_level"].$GLOBALS['cmsgo']['colorspace'])
                ));
                ...
                if($thumb_image != false) {
                    ...
                    $img_thumb_abs      = CMSGO_URL.$thumb_image['src'];
                    ...
                    if($article["article_image"]["list_zoom"]) {
                        ...
                        $zoominfo = get_cached_image(array(
                            "target_ext"    =>  $article["article_image"]['list_ext'],
                            "image_name"    =>  $article["article_image"]['list_hash'] . '.' . $article["article_image"]['list_ext'],
                            "max_width"     =>  $GLOBALS['cmsgo']["img_prev_width"],
                            "max_height"    =>  $GLOBALS['cmsgo']["img_prev_height"],
                            "thumb_name"    =>  md5($article["article_image"]['list_hash'].$GLOBALS['cmsgo']["img_prev_width"].$GLOBALS['cmsgo']["img_prev_height"].$GLOBALS['cmsgo']["sharpen_level"].$GLOBALS['cmsgo']['colorspace'])
                        ));
                        ...
                        if($zoominfo != false) {
                            ...
                            $img_zoom_abs       = CMSGO_URL.$zoominfo['src'];
                            $img_zoom_width     = $zoominfo[1];
                            $img_zoom_height    = $zoominfo[2];

                            $article["article_image"]["poplink"] = CMSGO_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo['src'], $zoominfo[3], $article["article_image"]['list_name']);
            ...
            if(empty($article["article_image"]['tmpllist']) || $article["article_image"]['tmpllist'] == 'default') {
                ...
                if(empty($tmpllist['default'])) {
                    ...
                    $tmpllist['default'] = file_get_contents(CMSGO_TEMPLATE.'inc_default/article_summary_list.tmpl');
            ...
            if(!isset($tmpllist[ $article["article_image"]['tmpllist'] ])) {
                $tmpllist[ $article["article_image"]['tmpllist'] ] = @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/articlesummary/list/'.$article["article_image"]['tmpllist']);
            }
            if($tmpllist[ $article["article_image"]['tmpllist'] ]) {
                ...
                if(str_contains($tmpl, '[SYSTEM]')) {
                    ...
                    $sql_cnt .= "AND acontent_livedate < NOW() AND (acontent_killdate IS NULL OR acontent_killdate > NOW()) ";
...
function include_ext_php($inc_file, $t=0) {
    ...
    if(!$t && preg_match('~^(?:ht)tps?://~i', $inc_file, $match) && is_file($inc_file)) {
        ...
        $root_path = rtrim(str_replace("\\", '/', realpath(CMSGO_ROOT)), '/');

        if(str_starts_with($this_path, $root_path)) {
            $t = 1;
        }

    } elseif(!$t && !empty($GLOBALS['cmsgo']['allow_remote_URL'])) {
...
function international_date_format_callback($matches) {
    ...
    if($matches[1] && str_contains($matches[1], ' set=')) {
...
function date_callback($matches) {
    ...
    return date($matches[1], $GLOBALS['cmsgo']['callback']);
}
...
function international_date_format($language='', $format="Y/m/d", $date_now=0) {
    ...
    if($language === 'en') {
        ...
        if(empty($GLOBALS['cmsgo']['DOCTYPE_LANG'])) {

            return date($format, $date_now);

        }
        $language = $GLOBALS['cmsgo']['DOCTYPE_LANG'];

    }

    $lang_include = CMSGO_ROOT.'/include/inc_lang/date/'.substr($language, 0, 2).'.date.lang.php';
...
function url_search($query) {
    ...
    return $query[1].'="'.$file.CMSGO_REWRITE_EXT.$query_string.$anchor.'"';
}

function js_url_search($query) {
    $query_string   = '';
    $anchor         = '';
    if (str_starts_with($query[1], 'id=')) {
    ...
    return "onclick=\"location.href='".$file.CMSGO_REWRITE_EXT.$query_string.$anchor."'";
}

function get_related_articles_callback($matches) {
    return get_related_articles(($matches[2] ?? $GLOBALS['related_keywords']), $GLOBALS['aktion'][1], $GLOBALS['template_default']["related"], $matches[1]);
}
...
function get_related_articles($keywords, $current_article_id, $template_default, $max_cnt_links=0, $dbcon=null) {
    ...
    if(is_array($keywordarray) && count($keywordarray)) {
        ...
        $sql .= "FROM ".DB_PREPEND."cmsgo_article WHERE article_deleted=0 AND ";
        ...
        if(!PREVIEW_MODE) {
            $sql .= "article_begin < NOW() AND (article_end IS NULL OR article_end > NOW()) AND ";
        }
        $sql .= '('.$where.') ';
...
function get_new_articles($template_default, $max_cnt_links=0, $cat='', $dbcon=null) {
    ...
    $sql .= "FROM ".DB_PREPEND."cmsgo_article WHERE ".$cat;
    // VISIBLE_MODE: 0 = frontend (all) mode, 1 = article user mode, 2 = admin user mode
    switch(VISIBLE_MODE) {
        case 0: $sql .= "article_aktiv=1 AND ";
                break;
    ...
    if(!PREVIEW_MODE) {
        $sql .= "AND article_begin < NOW() AND (article_end IS NULL OR article_end > NOW()) ";
    }
...
function get_article_idlink($article_id=0, $link_text="", $dbcon=null) {
    ...
    if($article_id) {
        $sql =  "SELECT article_id, article_title, article_cid, article_alias ".
                "FROM ".DB_PREPEND."cmsgo_article WHERE article_id=".$article_id." AND ".
...
function get_keyword_link($keywords) {
    ...
    if($where) {
        ...
        $sql  = "SELECT article_id, article_cid, article_title, article_alias FROM ".DB_PREPEND."cmsgo_article WHERE ";
        // VISIBLE_MODE: 0 = frontend (all) mode, 1 = article user mode, 2 = admin user mode
        switch(VISIBLE_MODE) {
            case 0: $sql .= "article_aktiv=1 AND ";
                    break;
        ...
        if(!PREVIEW_MODE) {
            $sql .= "article_begin < NOW() AND (article_end IS NULL OR article_end > NOW()) ";
        }
        $sql .= "AND (".$where.")";
...
function get_index_link_next($linktext, $cat_down=0) {
    ...
    $a_id = $content['article_id'] ?? $GLOBALS['aktion'][1];
...
function include_int_php($string) {
    ...
    $s = str_replace('$cmsgo', '$notavailable', $s);
    $s = str_replace(array('["cmsgo"]', "['cmsgo']"), '["notavailable"]', $s);
...
function build_sitemap_articlelist($cat, $counter=0, $sitemap=array()) {
    ...
    $sql  = "SELECT article_id, article_title, article_alias FROM ".DB_PREPEND."cmsgo_article ";
...
function render_cnt_template($text='', $tag='', $value='', $value_else='') {
    ...
    // Keep it compatible with older templates
...
function include_url($url, $ignore_ssl=false) {
    ...
    if($url && $cache) {
        ...
        $cache_file     = CMSGO_CONTENT.'tmp/'.$cache_filename;   // set caching file
    ...
    if($cache_status !== 'VALID' && $url) {  // cache file is missing or outdated
        ...
        if($k) {
            ...
            if($charset != false) {
                $k = makeCharsetConversion($k, $charset, CMSGO_CHARSET, 1);
            }
...
function render_PHPcode($string='') {
    if($string && str_contains($string, 'PHP')) {

        if(empty($GLOBALS['cmsgo']['enable_inline_php'])) {
    ...
    foreach($struct as $key => $value) {
        ...
        if( _getStructureLevelDisplayStatus($key, $level) ) {
            ...
            if(!empty($temp_tree[$key])) {
                ...
                $temp_menu .= '<a href="'.$link.'">'.html_specialchars($value["acat_name"]).'</a>';
                ...
                $temp_menu .= html_specialchars($value["acat_name"])."</a></li>\n";
            }
        }
    }
...
function get_fe_userinfo($forum_userID) {
    ...
    if($forum_userID != 0 && (!isset($GLOBALS['FE_USER']) || !isset($GLOBALS['FE_USER'][$forum_userID]))) {
        ...
        $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_user WHERE usr_id=".$forum_userID." LIMIT 1";
        $result = _dbQuery($sql);
        ...
        if(isset($result[0]['usr_id'])) {
            ...
            $GLOBALS['FE_USER'][$forum_userID] = array(
                ...
                'lang'      => empty($result[0]['usr_lang']) ? $GLOBALS['cmsgo']['default_lang'] : $result[0]['usr_lang'],
    ...
    if(($forum_userID === 0 && !isset($GLOBALS['FE_USER'][$forum_userID])) || !$got_the_info) {
        ...
        $GLOBALS['FE_USER'][$forum_userID] = array(
            ...
            'aktiv'     => 1,
            'name'      => 'Guest',
            'lang'      => $GLOBALS['cmsgo']['default_lang'],
            'wysiwyg'   => empty($GLOBALS['cmsgo']['wysiwyg_editor']) ? 0 : 1
        );
    }
...
function highlightSearchResult($string='', $search=null) {
    ...
    if(isset($GLOBALS['cmsgo']['current_highlight_match'])) {

        $highlight_match = $GLOBALS['cmsgo']['current_highlight_match'];
    ...
    if(!preg_match('/<.+?>/', $string)) {
        $string = preg_replace('/('.$highlight_match.')/i', $GLOBALS['cmsgo']['search_highlight']['prefix'].'$1'.$GLOBALS['cmsgo']['search_highlight']['suffix'], $string);
    } else {
        $string = preg_replace('/(?<=>)([^<]+)?('.$highlight_match.')/i', '$1'.$GLOBALS['cmsgo']['search_highlight']['prefix'].'$2'.$GLOBALS['cmsgo']['search_highlight']['suffix'], $string);
    }
...
function get_level_ahref($key=0, $custom_link_add='') {
    ...
    if($GLOBALS['content']['struct'][$key]["acat_redirect"]) {
        ...
        $link .= '#';
        $link .= $GLOBALS['content']['struct'][$key]['acat_alias'] ?: 'opid'.$key;
        $link .= '"';
    } else {
        $link .= rel_url(array(), array('newsdetail', 'profile_manage', 'profile_reminder', 'profile_register'), $GLOBALS['content']['struct'][$key]['acat_alias'] ?: 'id='.$key);
        $link .= '"';
    }
...
function getImageCaption($caption, $array_index='NUM', $short=false) {
    ...
    $caption[1] = isset($caption[1]) ? trim($caption[1]) : ($filedata['f_alt'] ?? '');
    $caption[3] = isset($caption[3]) ? trim($caption[3]) : ($filedata['f_title'] ?? '');
    ...
    $caption[4] = isset($caption[4]) ? trim($caption[4]) : ($filedata['f_copyright'] ?? '');
...
function getFileDetails($file) {
    ...
    $result = _dbGet('cmsgo_file', '*', $where, '', '', 1);
    if(isset($result[0]['f_id'])) {
        ...
        if(!empty($result['f_vars'])) {
            ...
            if(count($GLOBALS['cmsgo']['allowed_lang']) > 1 && isset($result['f_vars'][ $lang ])) {
...
function _checkFrontendUserAutoLogin() {
    if(!_getFeUserLoginStatus() && session_id() && !empty($_COOKIE['cmsgoFeLoginRemember']) && !isset($_POST['cmsgoFeLoginRemember']) && !isset($_GET['cmsgoFeLoginRemember'])) {
        $_loginData = explode('##-|-##', $_COOKIE['cmsgoFeLoginRemember']);
        if(!empty($_loginData[0]) && !empty($_loginData[1])) {
            ...
            if($_loginData['query_result'] !== false && is_array($_loginData['query_result']) && count($_loginData['query_result'])) {
                ...
                unset($_COOKIE['cmsgoFeLoginRemember']);
            }
        }
    }
    // logout
...
function get_article_morelink(& $article) {
    if($article['article_redirect']) {
        ...
        if($link[0]) {
            $link[0] = str_replace('{SITE}', CMSGO_URL, $link[0]);
        }
        if(empty($link[1])) {
            $link[1] = '';
        }
...
function getFrontendEditLink($type='', $id_1=0, $id_2=0, $uid=0) {
    ...
    switch($type) {
        ...
        case 'CP':          if(!$_SESSION["wcs_user_admin"] && $id_1) {
                                $sql  = 'SELECT COUNT(*) FROM '.DB_PREPEND.'cmsgo_article WHERE article_id='._dbEscape($id_1);
        ...
        case 'module':      if(!$_SESSION["wcs_user_admin"] && $id_2) {
                                $sql  = 'SELECT COUNT(*) FROM '.DB_PREPEND.'cmsgo_article WHERE article_id='._dbEscape($id_2);
                                $sql .= ' AND article_uid='._dbEscape($_SESSION["wcs_user_id"]);
                                if(!_dbCount($sql)) {
                                    return '';
                                }
    ...
    if($href) {
        ...
        $link  = '<a href="'.CMSGO_URL.'cmsgo.php?'.$href.'&'.FE_EDIT_LINK.'" target="backend" ';
...
function render_CKEDitor_resized_images($match) {
    ...
    $src = explode(CMSGO_FILES, $match[1]);
    if(empty($src[1])) {
        return $match[0];
    }
    ...
    foreach($sizes as $size) {
        ...
        if(str_starts_with($size, 'width:')) {
            $width  = intval(trim(substr($size, 6)));
        } elseif(str_starts_with($size, 'height:')) {
...
function get_structurelevel_single_article_alias($article_cid=0) {
    ...
    if(empty($content['struct'][ $article_cid ]['acat_articlecount'])) {
        $sql  = 'SELECT COUNT(article_id) FROM '.DB_PREPEND.'cmsgo_article ';
...
function render_device($string) {
    ...
    if(empty($GLOBALS['cmsgo']['render_device']) || (!str_contains($string, '<!--if:') && !str_contains($string, '<!--!if:'))) {
        return $string;
    }
...
function get_css_keywords($text, $return_as_string=true) {
    ...
    if(empty($text) || !is_string($text) || !str_contains($text, '*CSS-')) {
