...
function html_parser($string) {
	...
	// path to internal article, alias unbound and safe (in case of unknown article, the link whole anchor will be removed, leaves content) 
	$string = preg_replace_callback("/\[LINKHREF(.*?)\]/m", 'html_parse_aliashref', $string);
	$string = preg_replace_callback("/\[LINKHREF(.*?)\](.*?)\[\/LINKHREF\]/m", 'html_parse_aliashref', $string);
	
	// Remove unknown LINKHREF, fallback to linktext/anchor-innerhtml
	$string = preg_replace("/<a\s+(?:[^>]*?\s+)?href=\"\[LINKHREF(.*?)\]\">(.*?)<\/a>/m", "$2", $string);

	// internal Link (with textfallback) and index.php?alias link: [LINK $id/$alias]linktext[/LINK]
	$string = preg_replace_callback("/\[LINK(.*?)\](.*?)\[\/LINK\]/m", 'html_parse_aliaslink', $string);
	$string = preg_replace_callback("/\[LINK(.*?)\]/m", 'html_parse_aliaslink', $string);
...
/*
 * Parse a aliaslink, output: a link to intern article (in case of unknown article: fallback to linktext)
 * @param: array(match,id/alias,linktext)
 * @author: q23/geckse
 */
function html_parse_aliaslink($para) {
	...
	$para[1] = preg_replace('/\s+/', '', $para[1]);
	
	// got an anchor?
	if(strpos($para[1], '#') !== false) {
		list($para[1], $anchor) = explode('#', $para[1], 2);
		if($anchor) {
			$anchor = '#'.$anchor;
		}
	} else {
	...
	$sql  = "SELECT article_alias, article_title ";
    $sql .= "FROM ".DB_PREPEND."phpwcms_article ";
    if($id == 0){
    ...
    $sql .= "AND article_aktiv=1 AND article_deleted=0 ";
	if(!PREVIEW_MODE) {
	    $sql .= "AND article_public=1 AND (article_archive_status=1 AND article_begin<NOW() ";
    ...
    if(isset($result['article_alias'])){
		if($text == ""){
			$text = $result['article_title'];
		}	 
		$link = '<a href="index.php?'.$result['article_alias'].$anchor.'" title="'.$text.'"';
		if(!empty($GLOBALS['template_default']['classes']['link-internal'])) {
			$link .= ' class="'.$GLOBALS['template_default']['classes']['link-internal'].'"';
		}
		$link .= '>'.$text.'</a>';    
    }
	return $link;
...
/*
 * Parse a alias-href, output: a path to intern article (in case of unknown article: fallback to the inner of the removed <a>-element)
 * @param: array(match,id/alias,id/alias)
 * @author: q23/geckse
 */
function html_parse_aliashref($para) {
	
	if(!is_array($para)) return "[LINKHREF 0]"; // null link, so the next regex will clear the complete anchor element
	...
	if(isset($para[2])){
		$para[2] = trim(str_replace('&nbsp;', '', $para[2]));
		$para[2] = str_replace(':', '', $para[2]);	
		$para[2] = preg_replace('/\s+/', '', $para[2]);
