...
if(!$content["newsletter"]["change_text"] || !$content["newsletter"]["reg_text"]) {
    ...
    $temp_mailtext .= $cmsgo['SMTP_FROM_NAME']."\n";
    $temp_mailtext .= $cmsgo["admin_email"]."\n\n";
...
if(empty($content["newsletter"]["recaptcha"])) {
    ...
    $recaptcha = new cmsgoRecaptcha($content["newsletter"]["recaptcha_config"]['site_key'], $content["newsletter"]["recaptcha_config"]['secret_key']);

    if(empty($content["newsletter"]["recaptcha_config"]['lang'])) {
        $content["newsletter"]["recaptcha_config"]['lang'] = $cmsgo['default_lang'];
    }
...
if(isset($_POST["newsletter_send"]) && intval($_POST["newsletter_send"])) {
    ...
    if(empty($content["newsletter"]["newsletter_error"]) && is_valid_email($content["newsletter"]["email_address"]) && $content["newsletter"]["email_subscription"]) {
        ...
        $check_sql = "SELECT address_id,address_key FROM ".DB_PREPEND."cmsgo_address WHERE address_email="._dbEscape($content["newsletter"]["email_address"])." LIMIT 1";
        ...
        if($content["newsletter"]["reffering_key"]) {
            ...
            $e_sql = "UPDATE ".DB_PREPEND."cmsgo_address SET ".
            ...
            $e_sql = "INSERT INTO ".DB_PREPEND."cmsgo_address (".
        ...
        $content["newsletter"]["verify_link"] = CMSGO_URL."verify.php?s=".rawurlencode($content["newsletter"]["reffering_key"]);
        $content["newsletter"]["delete_link"] = CMSGO_URL."verify.php?u=".rawurlencode($content["newsletter"]["reffering_key"]);
        ...
        if(empty($content['newsletter']['subject']['tag'])) {
            ...
            $content['newsletter']['subject'] = 'Newsletter verification for '.$cmsgo["site"];
        } else {
            $content["newsletter"]["mailtext"] = $content['newsletter']['subject']['new'];
...
if($content["newsletter"]["success"]) {
    ...
    if(is_array($content["newsletter"]["subscription"]) && count($content["newsletter"]["subscription"])) {
        ...
        $content["newsletter"]['temp'] = _dbQuery("SELECT * FROM ".DB_PREPEND."cmsgo_subscription WHERE subscription_active=1 ORDER BY subscription_name");
