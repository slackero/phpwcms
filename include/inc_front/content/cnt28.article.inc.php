...
if(!empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_cntpart/felogin/'.$crow["acontent_template"])) {

    $_loginData                     = @unserialize($crow["acontent_form"]);

    $_loginData['template']         = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/felogin/'.$crow["acontent_template"]) );
        ...
        // otherwise check login and send password
            ...
            if($_loginData['validate_db']['userdetail']) {
                ...
                if(isset($result[0])) {
                    ...
                    _dbUpdate('cmsgo_userdetail', array('detail_password'=>md5($result[0]['PASSWORD'])), 'WHERE detail_id='.$result[0]['detail_id']);
            ...
            if($_loginData['validate_db']['backenduser'] && !isset($result[0])) {
                ...
                if(isset($result[0])) {
                    $result[0]['PASSWORD'] = generic_string(8);
                    _dbUpdate('cmsgo_user', array('usr_pass'=>md5($result[0]['PASSWORD'])), 'WHERE usr_id='.$result[0]['usr_id']);
                    $_loginData['remind_password'] = $result[0];
                }
            }
        }
