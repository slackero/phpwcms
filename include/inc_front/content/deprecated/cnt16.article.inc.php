...
if(isset($_POST['ecard_chooser'])) {
	...
	if(!is_valid_email($ecard["sender_email"]) || !is_valid_email($ecard["recipient_email"])) {
		...
		if(!$ecard['mailer']->setLanguage($cmsgo['default_lang'], CMSGO_ROOT.'/include/inc_ext/phpmailer/language/')) {
			$ecard['mailer']->setLanguage('en', CMSGO_ROOT.'/include/inc_ext/phpmailer/language/');
		}
		...
		if($ecard["mail"]) {
			...
			$ecard["mailer"]->Body = '<div align="center"><h3>E-Card &quot;'.html_specialchars(chop($ecard["capt"][$ecard["chooser"]])).'&quot;</h3>'.
									 ...
									 '<p>'.$list_img_temp.'</p>'.
									 '<p>'.nl2br(html_specialchars($ecard["sender_msg"])).'</p><hr /><a href="'.$cmsgo["site"].'" target="_blank">'.$cmsgo["site"].'</a></div>';
		}
