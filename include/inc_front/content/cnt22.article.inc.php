...
if( isset($rssfeed['rssurl']) && !empty($rssfeed['rssurl']) ) {
	...
	if( empty($rssfeed['template']) || !is_file(CMSGO_TEMPLATE.'inc_cntpart/rssfeed/'.$rssfeed['template']) ) {
		$rssfeed['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/rssfeed.tmpl') );
	} else {
		$rssfeed['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/rssfeed/'.$rssfeed['template']) );
	}
	if(!$rssfeed['template']) {
...
<!--RSSFEED_START//-->[TITLE]<h3>{TITLE}</h3>[/TITLE][SUBTITLE]<h4>{SUBTITLE}</h4>[/SUBTITLE]<div class="rss-feed">{FEEDINFO}<ul>{ITEMS}</ul></div><!--RSSFEED_END//-->';
	...
	// Load SimplePie
	require_once(CMSGO_ROOT.'/include/inc_ext/simplepie/SimplePie.compiled.php');
	...
	if($rssfeed["timeout"]) {
		...
		$rss_obj->set_cache_location ( CMSGO_RSS );
	...
	if( $rss_obj->data ) {
		...
		if( $rss_obj->get_image_url() ) {
			...
			$rss['temp_feedinfo'] .= '<img src="' . $rss_obj->get_image_url() . '" alt="' . $rss_obj->get_image_title() . '"' . CMSGO_LAZY_LOADING . HTML_TAG_CLOSE;
			$rss['temp_feedinfo'] .= '</a>';
