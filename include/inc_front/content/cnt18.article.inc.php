...
include_once CMSGO_ROOT.'/include/inc_front/content/cnt_functions/cnt18.func.inc.php';
...
$guestbook['image_dir']  = CMSGO_ROOT.'/'.CMSGO_FILES.'guestbook_'.$guestbook['cid'];

// getting guestbook template
if(is_file(CMSGO_TEMPLATE.'inc_cntpart/guestbook/'.$guestbook['template'])) {
    $guestbook['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/guestbook/'.$guestbook['template']) );
} else {
    $guestbook['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/guestbook.tmpl') );
}
...
if($guestbook['visible']) {
                    ...
                    if($guestbook['image']['info'][2] == 1 || $guestbook['image']['info'][2] == 2 || $guestbook['image']['info'] == 3) {
                        ...
                        if($_FILES['guestbook_image']['size'] > $guestbook["max_image_filesize"]) {
                            ...
                            if(!is_dir($guestbook['image_dir'])) {
                                $old_umask = umask(0);
                                $guestbook['owner'] = fileowner(CMSGO_ROOT.'/'.CMSGO_FILES);
    ...
    if(!empty($guestbook['cookie']) && !empty($guestbook['time'])) {
        ...
        if(isset($_COOKIE['cmsgo_guestbook'.$guestbook['cid']])) {

            if($_COOKIE['cmsgo_guestbook'.$guestbook['cid']]+$guestbook['time'] >= time()) {
        ...
        if(!$guestbook['flooding']) {
            ...
            $guestbook['sql']  = "SELECT MAX(guestbook_created) AS dbcreate FROM ".DB_PREPEND."cmsgo_guestbook WHERE ";
            $guestbook['sql'] .= "guestbook_cid="._dbEscape($guestbook['cid'])." AND ";
            $guestbook['sql'] .= "guestbook_trashed != '9' AND ";
            $guestbook['sql'] .= "guestbook_ip="._dbEscape(CMSGO_GDPR_MODE ? getAnonymizedIp() : getRemoteIP())." AND ";
    ...
    if(isset($_POST['guestbook_email']) && !$guestbook['flooding']) {
        ...
        if(!checkFormTrackingValue()) {
            $guestbook['flooding']  = 1;
            $guestbook['readform']  = 1;
            $guestbook['spamalert'] = '<div class="spamFormAlert">Your IP '.(CMSGO_GDPR_MODE ? getAnonymizedIp() : getRemoteIP()).' is not allowed to send form!</div>';
        }
    }
    ...
    if(isset($_POST['guestbook_email']) && !$guestbook['flooding']) {
        ...
        if(!count($guestbook['error'])) {
            ...
            $guestbook['sql']  = "INSERT INTO ".DB_PREPEND."cmsgo_guestbook SET ";
            ...
            $guestbook['sql'] .= "guestbook_ip="._dbEscape(CMSGO_GDPR_MODE ? getAnonymizedIp() : getRemoteIP()).", ";
            $guestbook['sql'] .= "guestbook_useragent=MD5("._dbEscape($_SERVER['HTTP_USER_AGENT']).")";
            ...
            if(isset($guestbook['result']['INSERT_ID'])) {
                ...
                if($guestbook['cookie'] && $guestbook['time']) {
                    setcookie('cmsgo_guestbook'.$guestbook['cid'], time(), time()+intval($guestbook['time']), '/', getCookieDomain(), CMSGO_SSL, true);
                }

                // check if notify email should be sent
                if(!empty($guestbook['notify'])) {

                    $guestbook['notify'] = @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/guestbook/notify_email.txt');
                    if(!$guestbook['notify']) {
                        $guestbook['notify'] = 'New entry - please proof:'.LF.CMSGO_URL.'index.php?id='.implode(',', $aktion);
                    }
                    ...
                    $guestbook['notify']    = str_replace(
                        ...
                        array(
                            CMSGO_URL.'index.php?id='.implode(',', $aktion),
                            CMSGO_URL.'cmsgo.php?do=articles&p=2&s=1&aktion=2&id='.$aktion[1].'&acid='.$guestbook['cid'],
                            CMSGO_GDPR_MODE ? getAnonymizedIp() : getRemoteIP(),  $_SERVER['HTTP_USER_AGENT'], date('Y/m/d H:i:s'),
                            ...
                            empty($guestbook['image']['file']) ? '' : CMSGO_URL.CMSGO_FILES.'guestbook_'.$guestbook['cid'].'/'.$guestbook['image']['file']
                        ),
                        $guestbook['notify']
                    );
    ...
    if(isset($GLOBALS['_getVar']['guestbookentry'])) {
        ...
        $guestbook['sql']  = 'SELECT * FROM '.DB_PREPEND.'cmsgo_guestbook ';
        $guestbook['sql'] .= 'WHERE guestbook_id='.intval($GLOBALS['_getVar']['guestbookentry']);
        $guestbook['sql'] .= " AND guestbook_ip="._dbEscape(CMSGO_GDPR_MODE ? getAnonymizedIp() : getRemoteIP());
    ...
    $guestbook['sql']  = "SELECT * FROM ".DB_PREPEND."cmsgo_guestbook WHERE guestbook_cid=";
    ...
    if($guestbook['listing'] && $guestbook['listcount']) {
        ...
        if($guestbook['pagecount'] > 1 || $guestbook['archivedate']) {
            ...
            if( ! ( strpos($guestbook['nav'],'{ARCHIVE')===false ) ) {
                ...
                $guestbook['asql'] .= ") AS guestbook_date FROM ".DB_PREPEND."cmsgo_guestbook WHERE guestbook_cid=";
            ...
            if(isset($guestbook['imgdata']) && !empty($guestbook['row']['guestbook_image'])) {
                ...
                if(file_exists($guestbook['image_dir'].'/'.$guestbook['row']['guestbook_image'])) {
                    ...
                    $thumb_image = get_cached_image(array(
                        ...
                        "thumb_name"    =>  md5($guestbook['row']['guestbook_image'].$guestbook['imgdata'][0].$guestbook['imgdata'][1].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
                    ));
                    ...
                    if($thumb_image != false) {
                        ...
                        if($guestbook['imgdata'][2]) {
                            $zoominfo = get_cached_image(array(
                                ...
                                "thumb_name"    =>  md5($guestbook['row']['guestbook_image'].$cmsgo["img_prev_width"].$cmsgo["img_prev_height"].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
                            ));

                            if($zoominfo != false) {

                                $popup_img = CMSGO_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo['src'], $zoominfo[3], $guestbook['row']['guestbook_image']);
