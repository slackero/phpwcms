...
// get template
if($crow['acontent_template_listmode'] && empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/list.images.tmpl')) {

    $image['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/list.images.tmpl') );

} elseif(empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/images.tmpl')) {

    $image['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/images.tmpl') );

} elseif($crow['acontent_template_listmode'] && is_file(CMSGO_TEMPLATE.'inc_cntpart/images/list.'.$crow["acontent_template"])) {

    $image['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/images/list.'.$crow["acontent_template"]) );

} elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/images/'.$crow["acontent_template"])) {

    $image['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/images/'.$crow["acontent_template"]) );
    ...
    if(is_array($image['images']) && ($image['count'] = count($image['images']))) {
        ...
        foreach($image['images'] as $key => $value) {
            ...
            $thumb_image = get_cached_image(array(
                ...
                "thumb_name"    =>  md5($image['images'][$key][2].$image['width'].$image['height'].$cmsgo["sharpen_level"].$image['crop'].$cmsgo['colorspace']),
            ...
            if($image['zoom']) {
                ...
                $zoominfo = get_cached_image(array(
                    ...
                    "thumb_name"    =>  md5($image['images'][$key][2].$cmsgo["img_prev_width"].$cmsgo["img_prev_height"].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
                ));
            }
            ...
            if(strpos($image['tmpl_entry'], '[LANDSCAPE') !== false) {
                ...
                } elseif(is_file(CMSGO_ROOT.'/'.CMSGO_FILES.$image['images'][$key][2] . '.' . $image['images'][$key][3])) {

                    $img_landscape = @getimagesize(CMSGO_ROOT.'/'.CMSGO_FILES.$image['images'][$key][2] . '.' . $image['images'][$key][3]);
            ...
            if($image['zoom'] && isset($zoominfo) && $zoominfo != false) {
                ...
                $open_popup_link = CMSGO_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo['src'], $zoominfo[3], $image['images'][$key][1]);
                if($caption[2][0]) {
