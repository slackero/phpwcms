...
if($image) {
    ...
    if($crow['is_imagetext']) {
        ...
        switch($image[7]) {
            case 0: // top left
                $image_text .= imagediv($cmsgo, $image, $crow["settings"]['class_top_left']);
                $image_text .= LF . $crow["acontent_text"];
                break;
            case 1: // top center
                $image_text .= imagediv($cmsgo, $image, $crow["settings"]['class_top_center']);
                $image_text .= LF . $crow["acontent_text"];
                break;
            case 2: // top right
                $image_text .= imagediv($cmsgo, $image, $crow["settings"]['class_top_right']);
                $image_text .= LF . $crow["acontent_text"];
                break;
            case 3: // bottom left
                $image_text .= $crow["acontent_text"] . LF;
                $image_text .= imagediv($cmsgo, $image, $crow["settings"]['class_bottom_left']);
                break;
            case 4: // bottom center
                $image_text .= $crow["acontent_text"] . LF;
                $image_text .= imagediv($cmsgo, $image, $crow["settings"]['class_bottom_center']);
                break;
            case 5: // bottom right
                $image_text .= $crow["acontent_text"] . LF;
                $image_text .= imagediv($cmsgo, $image, $crow["settings"]['class_bottom_right']);
                break;
            case 6: // float left
                $image_text .= imagediv($cmsgo, $image, $crow["settings"]['class_float_left']);
                $image_text .= LF . $crow["acontent_text"];
                break;
            case 7: // float right
                $image_text .= imagediv($cmsgo, $image, $crow["settings"]['class_float_right']);
                $image_text .= LF . $crow["acontent_text"];
                break;
            case 8: // column left
                $iconimg = imagediv($cmsgo, $image, $crow["settings"]['class_column_left_image']);
                ...
                break;
            case 9: // column right
                $iconimg = imagediv($cmsgo, $image, $crow["settings"]['class_column_right_image']);
                if(trim($iconimg.$crow["acontent_text"])) {
...
if(!$crow['has_image']) {
    ...
    $crow['image_tag'] .= '"' . CMSGO_LAZY_LOADING . HTML_TAG_CLOSE;
