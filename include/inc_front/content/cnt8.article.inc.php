...
if((is_array($content['alink']['alink_id']) && count($content['alink']['alink_id'])) || (!empty($content['alink']['alink_type']) && ((is_array($content['alink']['alink_level']) && count($content['alink']['alink_level'])) || (isset($content['alink']['alink_category']) && count($content['alink']['alink_category']))))) {
    ...
    if($crow['acontent_template_listmode'] && !empty($content['alink']['alink_template']) && is_file(CMSGO_TEMPLATE.'inc_cntpart/teaser/list.'.$content['alink']['alink_template'])) {

        $content['alink']['alink_template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/teaser/list.'.$content['alink']['alink_template']) );

    } elseif(!empty($content['alink']['alink_template']) && is_file(CMSGO_TEMPLATE.'inc_cntpart/teaser/'.$content['alink']['alink_template'])) {

        $content['alink']['alink_template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/teaser/'.$content['alink']['alink_template']) );

    } elseif($crow['acontent_template_listmode'] && is_file(CMSGO_TEMPLATE.'inc_default/list.teaser.tmpl')) {

        $content['alink']['alink_template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/list.teaser.tmpl') );

    } elseif(is_file(CMSGO_TEMPLATE.'inc_default/teaser.tmpl')) {

        $content['alink']['alink_template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/teaser.tmpl') );
    ...
    if($content['alink']['alink_category_count']) {
        ...
        $alink_sql .= ', COUNT(*) AS matched_articles, pcat.cat_name FROM '.DB_PREPEND.'cmsgo_article ar ';
        ...
        $alink_sql .= "LEFT JOIN ".DB_PREPEND."cmsgo_categories pcat ON (pcat.cat_type='article' AND pcat.cat_pid=ar.article_id ";
        ...
        $alink_sql .= ', 1 AS matched_articles FROM '.DB_PREPEND.'cmsgo_article ar ';
        ...
        foreach($content['alink']['alink_id'] as $key => $value) {
            ...
            foreach($content['alink']['result'] as $row) {
                ...
                if($value == $row['article_id'] && isset($content['struct'][ $row['article_cid'] ])) {
                    ...
                    $content['alink']['tr'][$key]   = render_cnt_date($content['alink']['tr'][$key], $row[ $content['alink']['date_basis'] ], cmsgo_strtotime($row['article_begin']), cmsgo_strtotime($row['article_end']));
                        ...
                        if($img_thumb_id) {
                            ...
                            if(strpos($content['alink']['tr'][$key], 'cmsimage.php') !== false && $img_thumb_hash) {
                                ...
                                $thumb_image = get_cached_image(array(
                                    ...
                                    "thumb_name"    =>  md5($row['article_image']['list_hash'].$row['article_image']['list_width'].$row['article_image']['list_height'].$cmsgo['sharpen_level'].$img_thumb_crop.$cmsgo['colorspace']),
                    ...
                    if(strpos($content['alink']['tr'][$key], 'IMAGE_DETAIL') !== false && isset($row['article_image']['hash'])) {
                        ...
                        $row['article_image']['detail'] = get_cached_image(array(
                            ...
                            "thumb_name"    =>  md5($row['article_image']['hash'].$row['article_image']['width'].$row['article_image']['height'].$cmsgo['sharpen_level'].$row['article_image']['crop'].$cmsgo['colorspace']),
