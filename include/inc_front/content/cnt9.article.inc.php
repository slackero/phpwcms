...
// read template
if(empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/multimedia.tmpl')) {

    $crow["acontent_template"] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/multimedia.tmpl') );

} elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/multimedia/'.$crow["acontent_template"])) {

    $crow["acontent_template"] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/multimedia/'.$crow["acontent_template"]) );
...
if($media["media_src"]) {
    ...
    $media["sql"]  = "SELECT * FROM ".DB_PREPEND."cmsgo_file WHERE f_public=1 AND f_aktiv=1 AND f_id=".intval($media["media_id"])." AND ";
    ...
    if(isset($media["result"][0])) {
        ...
        $media["source"] = CMSGO_FILES.$media["result"][0]["f_hash"];
...
if(!empty($media["image_id"])) {
    ...
    $media["alt"] .= '<img src="'.CMSGO_RESIZE_IMAGE.'/' . $media["media_width"] . 'x' . $media["media_height"] . 'x1/' . $media["image_id"] . '" ';
    ...
    switch($media["media_player"]) {
        ...
        case 3: //Flash Player/Plugin
                if(!$media["media_src"] && (!$media["media_width"] || !$media["media_height"])) {
                    $media['local'] = @getimagesize(CMSGO_ROOT.'/'.$media["source"]);
                ...
                $block['custom_htmlhead'][$randomID] .= '   swfobject.registerObject("'.$randomID.'", "9.0.0", "'.CMSGO_URL.TEMPLATE_PATH.'inc_js/swfobject/2.1/expressInstall.swf");';
