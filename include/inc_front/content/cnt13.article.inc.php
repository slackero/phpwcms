...
// read template
if(empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/search.tmpl')) {
    $crow["acontent_template"]  = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/search.tmpl') );
} elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/search/'.$crow["acontent_template"])) {
    $crow["acontent_template"]  = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/search/'.$crow["acontent_template"]) );
} else {
...
if(!empty($_POST["search_input_field"]) || !empty($_GET['searchwords'])) {
    ...
    include_once CMSGO_ROOT.'/include/inc_front/content/cnt_functions/cnt13.func.inc.php';
    ...
    if(count($content['highlight'])) {
        ...
        $sql .= "LEFT JOIN ".DB_PREPEND."cmsgo_articlecat ac ON ";
        ...
        if(isset($content['search']['module']) && is_array($content['search']['module']) && count($content['search']['module'])) {
            foreach($content['search']['module'] as $key => $value) {
                if(isset($cmsgo['modules'][$key]) && is_file($cmsgo['modules'][$key]['path'].'frontend.search.php')) {

                    // include module search
                    include $cmsgo['modules'][$key]['path'].'frontend.search.php';
        ...
        if($s_run) {
            ...
            foreach($s_rank as $s_key => $svalue) {
                ...
                if($crow['template']['image_render'] && isset($s_list[$s_key]['image']['hash']) && is_file(CMSGO_ROOT.'/'.CMSGO_FILES.$s_list[$s_key]['image']['hash'].'.'.$s_list[$s_key]['image']['ext'])) {
