...
if(empty($content['reference']["tmpl"]) && is_file(CMSGO_TEMPLATE.'inc_default/reference.tmpl')) {
	...
	$content['reference']["tmpl"] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/reference.tmpl') );

} elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/reference/'.$content['reference']["tmpl"])) {

	$content['reference']["tmpl"] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/reference/'.$content['reference']["tmpl"]) );
...
if($content['reference']['ref_count']) {
	...
	$thumb_image = get_cached_image(array(
		...
		"thumb_name"	=>	md5($content['reference']["list"][0][2].$content['reference']["list"][0][4].$content['reference']["list"][0][5].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
	));
	...
	if($thumb_image != false) {
		...
		$content['reference']['ref_image'] .= CMSGO_LAZY_LOADING . HTML_TAG_CLOSE;
	}
	...
	if($content['reference']['ref_count'] > 1) {
		...
		foreach($content['reference']["list"] as $captkey => $captvalue) {
			...
			$over_image = get_cached_image(array(
				...
				"thumb_name"	=>	md5($content['reference']["list"][$captkey][2].$content['reference']["list"][$captkey][4].$content['reference']["list"][$captkey][5].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
			));
			...
			$thumb_image = get_cached_image(array(
				...
				"thumb_name"	=>	md5($content['reference']["list"][$captkey][2].$content['reference']["temp_list_width"].$content['reference']["temp_list_height"].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
			));
			...
			if($thumb_image != false) {
				...
				$content['reference']['ref_list'] .= 'document.MM_returnValue;"' . CMSGO_LAZY_LOADING . HTML_TAG_CLOSE;
