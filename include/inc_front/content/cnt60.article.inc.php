...
// get template
if($crow['acontent_template_listmode'] && empty($crow["acontent_template"]) && is_file(PHPWCMS_TEMPLATE.'inc_default/list.custom.tmpl')) {

    $custom['template'] = render_device( @file_get_contents(PHPWCMS_TEMPLATE.'inc_default/list.custom.tmpl') );

} elseif(empty($crow["acontent_template"]) && is_file(PHPWCMS_TEMPLATE.'inc_default/custom.tmpl')) {

    $custom['template'] = render_device( @file_get_contents(PHPWCMS_TEMPLATE.'inc_default/custom.tmpl') );

} elseif($crow['acontent_template_listmode'] && is_file(PHPWCMS_TEMPLATE.'inc_cntpart/custom/list.'.$crow["acontent_template"])) {

    $custom['template'] = render_device( @file_get_contents(PHPWCMS_TEMPLATE.'inc_cntpart/custom/list.'.$crow["acontent_template"]) );

} elseif(is_file(PHPWCMS_TEMPLATE.'inc_cntpart/custom/'.$crow["acontent_template"])) {

    $custom['template'] = render_device( @file_get_contents(PHPWCMS_TEMPLATE.'inc_cntpart/custom/'.$crow["acontent_template"]) );
                ...
                foreach($custom['custom_tab_fields'] as $custom_field_key) {
                    ...
                    if($custom['fieldgroup'][$custom_field_key]['type'] === 'bool') {
                        ...
                        $thumb_image = get_cached_image(array(
                          ...
                          "thumb_name"    =>  md5($custom_field_value['f_hash'].$custom['width'].$custom['height'].$phpwcms["sharpen_level"].$custom['crop_zoom'].$phpwcms['colorspace']),
                        ...
                        if($custom['zoom']) {
                          ...
                          $zoom_image = get_cached_image(array(
                            ...
                            "thumb_name"    =>  md5($custom_field_value['f_hash'].$custom['width_zoom'].$custom['height_zoom'].$phpwcms["sharpen_level"].$custom['crop_zoom'].$phpwcms['colorspace']),
                            'crop_image'    =>  $custom['crop_zoom']
                          ));
                        }
                        ...
                        if($custom['zoom'] && isset($zoom_image) && $zoom_image != false) {
                            ...
                            $open_popup_link = PHPWCMS_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoom_image['src'].'?'.$zoom_image[3]);
                            ...
                            $img_zoom_abs       = PHPWCMS_URL.$zoom_image['src'];
