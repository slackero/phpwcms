...
include_once CMSGO_ROOT.'/include/inc_front/content/cnt_functions/cnt23.func.inc.php';
...
if(!empty($cnt_form['ssl']) && !CMSGO_SSL) {
    headerRedirect($cmsgo['site_ssl_url'] . rel_url(), 301);
}

// save default form tracking status
$default_formtracking_value = $cmsgo['form_tracking'];
...
if(empty($cnt_form['sendertype']) || $cnt_form['sendertype'] == 'system') {
    $cnt_form['sender'] = $cmsgo['SMTP_FROM_EMAIL'];
...
if(empty($cnt_form['sendernametype'])) {
    ...
    $cnt_form['sendername'] = $cmsgo['SMTP_FROM_NAME'];
...
if(isset($cnt_form["fields"]) && is_array($cnt_form["fields"]) && count($cnt_form["fields"])) {
    ...
    foreach($cnt_form["fields"] as $key => $value) {
        ...
        switch($cnt_form["fields"][$key]['type']) {
            ...
            case 'captcha':
                ...
                if($POST_DO && isset($_POST[$POST_name])) {
                    $POST_val[$POST_name] = remove_unsecure_rptags(clean_slweg($_POST[$POST_name]));
                    include_once CMSGO_ROOT.'/include/inc_ext/SPAF_FormValidator.class.php';
            ...
            case 'recaptcha':
                 ...
                 * reCAPTCHA
                 */
                require_once CMSGO_ROOT.'/include/inc_lib/classes/class.recaptcha.php';
                ...
                $cnt_form['recaptcha'] = array(
                        ...
                        'lang' => empty($cnt_form["fields"][$key]['value']['lang']) ? $cmsgo['default_lang'] : $cnt_form["fields"][$key]['value']['lang'],
            ...
            case 'recaptchainv':
                ...
                $block['custom_htmlhead']['recaptcha_api.js'] = '  ' . $recaptcha->get_api_src($cnt_form['recaptcha']['lang'], true);
                $block['custom_htmlhead']['recaptchainv_submit'.$crow["acontent_id"]]  = '  '.$recaptcha->get_onsubmit_function($crow['acontent_id'], 'cmsgoForm'.$crow["acontent_id"], true);
            ...
            case 'select':
                ...
                if($POST_DO && isset($_POST[$POST_name])) {
                    ...
                    if($POST_val[$POST_name] != '' && $cnt_form["fields"][$key]['type'] == 'selectemail') { // decrypt
                        $POST_val[$POST_name] = cmsgo_decrypt(base64_decode($POST_val[$POST_name]));
                    }
                    if($cnt_form["fields"][$key]['required'] && $POST_val[$POST_name] == '') {
                        $POST_ERR[$key] = $cnt_form["fields"][$key]['error'];
                        $cnt_form["fields"][$key]['class'] = getFieldErrorClass($value['class'], $cnt_form["error_class"]);
                ...
                if($cnt_form["fields"][$key]['type'] == 'country') {
                    ...
                    $form_value['lang'] = isset($form_value['lang']) ? preg_replace('/[^a-zA-Z]/', '', $form_value['lang']) : $cmsgo['default_lang'];
                    ...
                    if(count($form_value)) {
                        ...
                        foreach($form_value as $option_value) {
                            ...
                            if(substr($option_value, -2) === ' -') {
                                ...
                                $form_field .= '<option value="'.($cnt_form["fields"][$key]['type'] == 'selectemail' && $option_value ? base64_encode(cmsgo_encrypt($option_value)) : $option_value).'"';
                            } else {
                                $option_value = str_replace(' selected', '', $option_value);
                                $form_field .= '<option value="'.($cnt_form["fields"][$key]['type'] == 'selectemail' && $option_value ? base64_encode(cmsgo_encrypt($option_value)) : $option_value).'" selected="selected"';
                            }
                    ...
                    if($cnt_form["fields"][$key]['required'] && empty($_FILES[$POST_name]['name'])) {
                        ...
                        if(
                                ...
                                !@move_uploaded_file(
                                        ...
                                        CMSGO_ROOT.'/'.$cnt_form['upload_value']['folder'].'/'.$cnt_form['upload_value']['filename']
                                        )
                                ) {
                                    ...
                                    @chmod(CMSGO_ROOT.'/'.$cnt_form['upload_value']['folder'].'/'.$cnt_form['upload_value']['filename'], 0644);

                                }
                    }
                    if(isset($POST_ERR[$key])) {
                        @unlink($_FILES[$POST_name]['tmp_name']);
                        @unlink(CMSGO_ROOT.'/'.$cnt_form['upload_value']['folder'].'/'.$cnt_form['upload_value']['filename']);
                        $cnt_form["fields"][$key]['class'] = getFieldErrorClass($value['class'], $cnt_form["error_class"]);
                    }
                }
            ...
            case 'submit':
                 ...
                 * Submit
                 */
                $cnt_form["fields"][$key]['class'] = trim('cmsgo-recaptcha-class '.$cnt_form["fields"][$key]['class']);
            ...
            case 'reset':
                ...
                if(strpos(strtolower($cnt_form["fields"][$key]['value']), 'src=') === false) {
                    ...
                    $form_field .= ' onclick="document.cmsgoForm'.$crow["acontent_id"].'.reset();" />';
                }
                break;
            ...
            case 'mathspam':
                ...
                if($POST_DO) {
                    ...
                    $mathspam_result  = $POST_val[$POST_name] * 123345 * strlen($cmsgo['db_user']);
                    $mathspam_result  = md5( CMSGO_URL . md5($cmsgo['db_pass']) . $mathspam_result );
                ...
                $mathspam_result = intval($mathspam_result) * 123345 * strlen($cmsgo['db_user']);
                $mathspam_result = md5( CMSGO_URL . md5($cmsgo['db_pass']) . $mathspam_result );
                ...
                if($cnt_form['labelpos'] == 0) {
                    ...
                    if($cnt_form["fields"][$key]['type'] != 'break') {
                        ...
                        if($cnt_form["fields"][$key]['label'] != '') {
                            $form_cnt .= '<label class="mb-0" for="'.$form_name.'">';
                    ...
                    if($cnt_form["fields"][$key]['label'] !== '') {
                        $form_cnt .= '">' . LF . '  <label class="form-label'.$cnt_form['requiredClass'].'" for="'.$form_name.'">';
                        $form_cnt .= $cnt_form['label_wrap'][0];
                        $form_cnt .= html_specialchars($cnt_form["fields"][$key]['label']);
                        $form_cnt .= $cnt_form['labelReqMark'];
                        $form_cnt .= $cnt_form['label_wrap'][1];
    ...
    if(empty($crow['recaptcha_submit_data'])) {
        ...
        $form_cnt = str_replace('cmsgo-recaptcha-class ', '', $form_cnt);
    } else {
        $form_cnt = str_replace(' data-recaptchainv-submit', $crow['recaptcha_submit_data'], $form_cnt);
        $form_cnt = str_replace('cmsgo-recaptcha-class', ' g-recaptcha', $form_cnt);
    }
...
if((!empty($POST_DO) && empty($POST_ERR)) || !empty($doubleoptin_values)) {
            ...
            if(is_array($POST_keyval)) {
                ...
                // check if this is an array - and is an upload value
                    if (empty($POST_keyval['name'])) {
                        ...
                        $POST_valurl = CMSGO_URL . $POST_keyval['folder'] . '/' . rawurlencode( $POST_keyval['name'] );
                        if ( isset( $POST_keyval['attachment'] ) && $POST_keyval['attachment'] ) {
                            $POST_attach[] = CMSGO_ROOT . '/' . $POST_keyval['folder'] . '/' . $POST_keyval['name'];
                        }
    ...
    if(empty($cnt_form["subject"])) {
        $cnt_form["alt_subj"] = str_replace('http://', '', $cmsgo['site']);
        $cnt_form["alt_subj"] = substr($cnt_form["alt_subj"], 0, trim($cmsgo['site'], '/'));
...
if((!empty($POST_DO) && empty($POST_ERR)) || (!empty($doubleoptin_values) && !$doubleoptin_error)) {
        ...
        if(isset($POST_val) && count($POST_val)) {
            foreach($POST_val as $file_key => $file_val) {
                if(isset($POST_val[$file_key]['name'])) {
                    @unlink(CMSGO_ROOT.'/'.$POST_val[$file_key]['folder'].$POST_val[$file_key]['name']);
                }
            }
        }
    }
    ...
    if($cnt_form["onerror_redirect"] === 1) {
        ...
        headerRedirect(str_replace('{SITE}', CMSGO_URL, $cnt_form["onerror"]));
...
if($form_cnt) {
    ...
    $CNT_TMP .= '<form id="cmsgoForm'.$crow["acontent_id"].'"'.$cnt_form['class'].' action="'.rel_url();
    if(empty($cnt_form['anchor_off'])) {
        $CNT_TMP .= '#';
...
$cmsgo['form_tracking'] = $default_formtracking_value;
