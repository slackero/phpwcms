...
// read template
if(empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/wysiwyg.tmpl')) {

	$crow["acontent_template"]	= render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/wysiwyg.tmpl') );

} elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/wysiwyg/'.$crow["acontent_template"])) {

	$crow["acontent_template"]	= render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/wysiwyg/'.$crow["acontent_template"]) );
        ...
        foreach($crow['custom_cnt_fields'] as $custom_field_key) {
			...
			if($crow['fieldgroup'][$custom_field_key]['type'] === 'bool') {
				...
				if($crow['fieldgroup'][$custom_field_key]['render'] === 'markdown') {
					init_markdown();
					$crow["acontent_template"] = render_cnt_template($crow["acontent_template"], $custom_field_replacer, $cmsgo['parsedown_class']->text($custom_field_value));
