...
// get template
if($crow['acontent_template_listmode'] && empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/list.imagespecial.tmpl')) {

    $image['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/list.imagespecial.tmpl') );

} elseif(empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/imagespecial.tmpl')) {

    $image['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/imagespecial.tmpl') );

} elseif($crow['acontent_template_listmode'] && is_file(CMSGO_TEMPLATE.'inc_cntpart/imagespecial/list.'.$crow["acontent_template"])) {

    $image['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/imagespecial/list.'.$crow["acontent_template"]) );

} elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/imagespecial/'.$crow["acontent_template"])) {

    $image['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/imagespecial/'.$crow["acontent_template"]) );
    ...
    if(is_array($image['images']) && ($image['count'] = count($image['images']))) {
        ...
        foreach($image['images'] as $key => $value) {
            ...
            if($image['zoom']) {
                ...
                if($value['zoom_hash']) {
                    $zoominfo = get_cached_image(array(
                        ...
                        "thumb_name"    =>  md5($value['zoom_hash'].$image['width_zoom'].$image['height_zoom'].$cmsgo["sharpen_level"].$image['crop_zoom'].$cmsgo['colorspace']),
                    ...
                    $zoominfo = get_cached_image(array(
                        ...
                        "max_width"     =>  $image['width_zoom'],
                        "max_height"    =>  $image['height_zoom'],
                        "thumb_name"    =>  md5($value['thumb_hash'].$image['width_zoom'].$image['height_zoom'].$cmsgo["sharpen_level"].$image['crop_zoom'].$cmsgo['colorspace']),
                        'crop_image'    =>  $image['crop_zoom']
                    ));
                }
            }
            ...
            if($image['zoom'] && isset($zoominfo) && $zoominfo != false) {
                ...
                $open_popup_link = CMSGO_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo['src'], $zoominfo[3], $value['zoom_name']);
                if($caption[2][0]) {
