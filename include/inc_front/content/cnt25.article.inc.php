...
if(!function_exists('get_mediaplayer_stream')) {
    ...
    function get_mediaplayer_stream($fileid=0, $flash=false) {
        ...
        // internal
        $sql = 'SELECT * FROM '.DB_PREPEND.'cmsgo_file WHERE f_aktiv=1 AND f_public=1 AND f_id='.$fileid;
        ...
        if(isset($file[0])) {
            ...
            $file = $file[0];
            $file['fmp_file'] = CMSGO_URL. 'download.php?file='.$file['f_hash'];
...
if(isset($fmp_data['fmp_template'])) {
    ...
    // read template
    if(empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/flashplayer.tmpl')) {

        $fmp_data['fmp_template']   = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/flashplayer.tmpl') );

    } elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/flashplayer/'.$crow["acontent_template"])) {

        $fmp_data['fmp_template']   = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/flashplayer/'.$crow["acontent_template"]) );
    ...
    if($fmp_data['fmp_int_ext']) {
        ...
        // internal
        $sql = 'SELECT * FROM '.DB_PREPEND.'cmsgo_file WHERE f_aktiv=1 AND f_public=1 AND f_id='.$fmp_data['fmp_internal_id'];
        if(!FEUSER_LOGIN_STATUS) {
            $sql .= ' AND f_granted=0';
        }
        ...
        if(isset($fmp_data['file'][0])) {
            ...
            $fmp_data['fmp_file']       = CMSGO_URL . 'download.php?file='.$fmp_data['file']['f_hash'];
    ...
    if($fmp_data['fmp_img_id']) {
        ...
        $sql = 'SELECT * FROM '.DB_PREPEND.'cmsgo_file WHERE f_aktiv=1 AND f_public=1 AND f_id='.$fmp_data['fmp_img_id'];
        if( !FEUSER_LOGIN_STATUS ) {
            $sql .= ' AND f_granted=0';
        }
    ...
    if($fmp_data['file']) {
        ...
        // NonverBlaster:hover
        $fmp_data['player_swf'] = CMSGO_URL.TEMPLATE_PATH.'lib/nonverblaster/NonverBlaster.swf';
    ...
    if(count($fmp_data['video'])) {
        ...
        if(!$fmp_data['fmp_set_html5only']) {
            ...
            if($fmp_data['fmp_set_skin_html5'] && $fmp_data['fmp_set_skin_html5'] != 'default' && is_file(CMSGO_TEMPLATE.'lib/video-js/skins/'.$fmp_data['fmp_set_skin_html5'].'.css')) {
