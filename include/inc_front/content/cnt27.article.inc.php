...
if(!empty($crow["acontent_form"]['faq_template']) && file_exists(CMSGO_TEMPLATE.'inc_cntpart/faq/'.$crow["acontent_form"]['faq_template'])) {
	...
	$crow["acontent_form"]['faq_template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/faq/'.$crow["acontent_form"]['faq_template']) );
...
if(!empty($crow["acontent_image"][2])) {
	...
	$thumb_image = get_cached_image(array(
		...
		"thumb_name"	=>	md5($crow["acontent_image"][2].$crow["acontent_image"][4].$crow["acontent_image"][5].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
	));
	...
	if($thumb_image != false) {
		...
		if($crow["acontent_image"][8]) {
			...
			$zoominfo = get_cached_image(array(
				...
				"thumb_name"	=>	md5($crow["acontent_image"][2].$cmsgo["img_prev_width"].$cmsgo["img_prev_height"].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
			));

			if($zoominfo != false) {

				$popup_img = CMSGO_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo['src'], $zoominfo[3], $crow["acontent_image"][1]);
