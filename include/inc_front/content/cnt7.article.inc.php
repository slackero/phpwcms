...
foreach($crow["acontent_files"] as $fkey => $fkey_value) {
    ...
    if($fkey_value) {
        ...
        $content['files'][$fkey]['file_info'] = '';
        if(isset($crow['acontent_text'][$fkey])) {
            $content['files'][$fkey]['file_info'] = is_array($crow['acontent_text'][$fkey]) ? $crow['acontent_text'][$fkey] : trim($crow['acontent_text'][$fkey]);
        }
...
if($content['files_sql']) {
    ...
    if(empty($content['file_static_result'][0])) {
        ...
        $content['files_sql']  = "SELECT * FROM ".DB_PREPEND."cmsgo_file WHERE f_public=1 AND f_aktiv=1 AND f_kid=1 AND f_trash=0 AND " . $content['files_sql'];
    ...
    if(is_array($content['files_result']) && count($content['files_result'])) {
        ...
        if(empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/filelist.tmpl')) {

            $crow["acontent_template"] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/filelist.tmpl') );

        } elseif($crow["acontent_template"] == 'download-inline') {

            $crow["acontent_template"] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/filelist_inline.tmpl') );

        } elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/filelist/'.$crow["acontent_template"])) {

            $crow["acontent_template"] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/filelist/'.$crow["acontent_template"]) );
                ...
                if($content['files_result'][ $_files_x ]['f_ext']) {
                    ...
                    if($content['files_result'][ $_files_x ]['f_vars'] && count($cmsgo['allowed_lang']) > 1) {
                        ...
                        if(!empty($content['files_result'][ $_files_x ]['f_vars'][ $cmsgo['default_lang'] ]['longinfo'])) {
                            $content['files_result'][ $_files_x ]['f_longinfo'] = $content['files_result'][ $_files_x ]['f_vars'][ $cmsgo['default_lang'] ]['longinfo'];
                        }
                        if(!empty($content['files_result'][ $_files_x ]['f_vars'][ $cmsgo['default_lang'] ]['copyright'])) {
                            $content['files_result'][ $_files_x ]['f_copyright'] = $content['files_result'][ $_files_x ]['f_vars'][ $cmsgo['default_lang'] ]['copyright'];
                        }
                        ...
                        switch($content['files_result'][ $_files_x ]['f_ext']) {
                            ...
                            case 'svg':
                                $_files_image = get_cached_image(array(
                                    ...
                                    "thumb_name"    =>  md5($content['files_result'][ $_files_x ]['f_hash'].$_file_info[4][0].$_file_info[4][1].$cmsgo["sharpen_level"].$_file_info[4][2].$cmsgo['colorspace']),
                    ...
                    if($_files_get_imagesize) {
                        ...
                        if(!empty($content['files_result'][ $_files_x ]['f_svg']) || !empty($content['files_result'][ $_files_x ]['f_image_width'])) {
                            ...
                            $_files_get_imagesize = @getimagesize(CMSGO_ROOT.'/'.CMSGO_FILES.$content['files_result'][$_files_x ]['f_hash'].'.'.$content['files_result'][ $_files_x ]['f_ext']);
                        }
