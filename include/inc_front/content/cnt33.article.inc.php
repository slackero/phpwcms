...
if(empty($crow["acontent_template"]) && is_file(CMSGO_TEMPLATE.'inc_default/news.tmpl')) {
    $news['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_default/news.tmpl') );
} elseif(is_file(CMSGO_TEMPLATE.'inc_cntpart/news/'.$crow["acontent_template"])) {
    $news['template'] = render_device( @file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/news/'.$crow["acontent_template"]) );
} else {
    $news['template'] = '';
}
...
$sql = 'FROM '.DB_PREPEND.'cmsgo_content pc ';
        ...
        foreach($news['result'] as $key => $value) {
            ...
            if($value['cnt_teasertext']) {
                ...
                if(empty($value['cnt_object']['cnt_textformat']) || $value['cnt_object']['cnt_textformat'] === 'plain') {
                    ...
                    init_textile();
                    $value['cnt_teasertext'] = $cmsgo['textile_class']->parse($value['cnt_teasertext']);
                    $value['cnt_description'] = $cmsgo['textile_class']->parse($value['cnt_description']);
                } else {
                ...
                if(preg_match_all('/{IMAGE_(HASH|WIDTH|HEIGHT)}/', $news['entries'][$key], $matches)) {
                    ...
                    if($content['images']['news'][ $_this_id ]['details'] !== null) {
                        ...
                        if(count($matches[0]) > 1 || $matches[1][0] !== 'HASH') {
                            $newsimage_file = CMSGO_STORAGE . $content['images']['news'][ $_this_id ]['details']['f_hash'];
                                ...
                                if(isset($value['gallery_id'][ $ivalue ])) {
                                    ...
                                    if($news['config']['gallery_filecenter_info'] && !isset($value['gallery_captions'][$ikey])) {
                                        ...
                                        if($news['config']['check_lang'] && !empty($ivalue['f_vars'])) {
                                            ...
                                            if(!empty($ivalue['f_vars'][$cmsgo['DOCTYPE_LANG']]['copyright'])) {
                                                $ivalue['f_copyright'] = $ivalue['f_vars'][$cmsgo['DOCTYPE_LANG']]['copyright'];
                                            }
                                        }
                                    ...
                                    if(preg_match('/{IMAGE_(WIDTH|HEIGHT)}/', $ivalue['tmpl'])) {
                                        ...
                                        $ivalue['file'] = CMSGO_STORAGE . $ivalue['f_hash'];
                                        if($ivalue['f_ext']) {
