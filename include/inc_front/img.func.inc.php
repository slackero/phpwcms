...
function imagetable($phpwcms, & $image, $rand="0:0:0:0", $align=0) {
    ...
    if($thumb_image != false) {
        ...
        if($image[8]) {
            ...
            $open_popup_link = PHPWCMS_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo['src'], $zoominfo[3], $image[1]);
...
function imagediv($phpwcms, $image, $classname='') {
    ...
    if($thumb_image != false) {
        ...
        if($image[8]) {
            ...
            $open_popup_link = PHPWCMS_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo[0], $zoominfo[3], $image[1]);
    ...
    if(($count_images = count($imagelist['images']))) {
        ...
        foreach($imagelist['images'] as $key => $value) {
            ...
            if($imagelist['zoom'] && isset($zoominfo) && $zoominfo != false) {
                ...
                $open_popup_link = PHPWCMS_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo['src'], $zoominfo[3], $imagelist['images'][$key][1]);
                if($caption[2][0]) {
                    $open_link = $caption[2][0];
                    $return_false = '';
                } else {
