...
function imagediv($cmsgo, $image, $classname='') {
    ...
    if($thumb_image != false) {
        ...
        if($image[8]) {
            ...
            $open_popup_link = CMSGO_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo[0], $zoominfo[3], $image[1]);
            ...
            if(!$cnt_image_lightbox || $caption[2][0]) {
                ...
                $image_block .= '<a href="'.CMSGO_IMAGES.$zoominfo[0].'" rel="lightbox"'.get_attr_data_gallery('', ' ', '');
                if($caption[0]) {
    ...
    if(($count_images = count($imagelist['images']))) {
        ...
        foreach($imagelist['images'] as $key => $value) {
            ...
            $thumb_image = get_cached_image(array(
                ...
                "thumb_name"    =>  md5($imagelist['images'][$key][2].$imagelist['images'][$key][4].$imagelist['images'][$key][5].$GLOBALS['cmsgo']["sharpen_level"].$crop.$GLOBALS['cmsgo']['colorspace']),
                'crop_image'    =>  $crop
            ));
            ...
            if($thumb_image && $imagelist['zoom']) {
                ...
                $zoominfo = get_cached_image(array(
                    ...
                    "thumb_name"    =>  md5($imagelist['images'][$key][2].$GLOBALS['cmsgo']["img_prev_width"].$GLOBALS['cmsgo']["img_prev_height"].$GLOBALS['cmsgo']["sharpen_level"].$GLOBALS['cmsgo']['colorspace'])
                ));
            }
            ...
            if($imagelist['zoom'] && isset($zoominfo) && $zoominfo != false) {
                ...
                $open_popup_link = CMSGO_URL . 'image_zoom.php?'.getClickZoomImageParameter($zoominfo['src'], $zoominfo[3], $imagelist['images'][$key][1]);
                if($caption[2][0]) {
                    $open_link = $caption[2][0];
                    $return_false = '';
                } else {
