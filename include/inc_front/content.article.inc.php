...
$sql .= "FROM ".DB_PREPEND."cmsgo_article ar LEFT JOIN ".DB_PREPEND."cmsgo_articlecat ac ON ";
...
if(isset($result[0]['article_id'])) {
    ...
    if($row["article_redirect"]) {
        ...
        $row["article_redirect"]        = str_replace('{SITE}', CMSGO_URL, $row["article_redirect"]);
    ...
    if($row['article_cache'] != '') {
        $cmsgo['cache_timeout'] = $row['article_cache'];
    }
    ...
    $content['opengraph']['support'] = cmsgo_boolval($row["article_opengraph"]);
    ...
    if(!empty($row["article_image"]["hash"])) {
        ...
        $thumb_image = get_cached_image(array(
            ...
            "thumb_name" => md5($row["article_image"]['hash'].$row["article_image"]['width'].$row["article_image"]['height'].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
        ));
        ...
        if($thumb_image != false) {
            ...
            if($row["article_image"]["zoom"]) {
                ...
                $zoominfo = get_cached_image(array(
                    ...
                    "thumb_name" => md5($row["article_image"]['hash'].$cmsgo["img_prev_width"].$cmsgo["img_prev_height"].$cmsgo["sharpen_level"].$cmsgo['colorspace'])
                ));
    ...
    if($row['article_paginate'] && $aktion[2] != 1) { // no pagination in print mode
        ...
        $sql_cnt .= "acontent_livedate < NOW() AND (acontent_killdate IS NULL OR acontent_killdate > NOW()) ";
    ...
    if(!empty($row["article_image"]['tmplfull']) && $row["article_image"]['tmplfull']!='default' && is_file(CMSGO_TEMPLATE.'inc_cntpart/articlesummary/article/'.$row["article_image"]['tmplfull'])) {

        // try to read the template files

        if($_CpPaginate && $content['aId_CpPage'] > 1 && is_file(CMSGO_TEMPLATE.'inc_cntpart/articlesummary/article/paginate/'.$row["article_image"]['tmplfull'])) { // check for default cp paginate template
            $row["article_image"]['tmplfull'] = file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/articlesummary/article/paginate/'.$row["article_image"]['tmplfull']);
        } else {
            $row["article_image"]['tmplfull'] = file_get_contents(CMSGO_TEMPLATE.'inc_cntpart/articlesummary/article/'.$row["article_image"]['tmplfull']);
        }

    } elseif(is_file(CMSGO_TEMPLATE.'inc_default/article_summary.tmpl')) {

        // load default template

        if($_CpPaginate && $content['aId_CpPage'] > 1 && is_file(CMSGO_TEMPLATE.'inc_default/article_summary_paginate.tmpl')) { // check for default cp paginate template
            $row["article_image"]['tmplfull'] = file_get_contents(CMSGO_TEMPLATE.'inc_default/article_summary_paginate.tmpl');
        } else {
            $row["article_image"]['tmplfull'] = file_get_contents(CMSGO_TEMPLATE.'inc_default/article_summary.tmpl');
        }
    ...
    foreach($cresult as $crow) {
        ...
        if($crow["acontent_type"] != 30) {
            ...
            include CMSGO_ROOT."/include/inc_front/content/cnt".$crow["acontent_type"].".article.inc.php";

        } elseif($crow["acontent_type"] == 30 && is_file($cmsgo['modules'][$crow["acontent_module"]]['path'].'inc/cnt.article.php')) {

            if($content['article_frontend_edit']) {
                $CNT_TMP .= getFrontendEditLink('module', $cmsgo['modules'][$crow["acontent_module"]]['name'], $crow['acontent_aid']);
            }
            // now try to include module content part code
            include $cmsgo['modules'][$crow["acontent_module"]]['path'].'inc/cnt.article.php';
        ...
        if(empty($cmsgo["allow_cntPHP_rt"]) || empty($cmsgo['enable_inline_php'])) {
...
if(!defined('CMSGO_ALIAS') && $content['article_alias']) {
    define('CMSGO_ALIAS', $content['article_alias']);
}
...
} elseif(empty($cmsgo['canonical_off'])) {

    $_tempAlias = defined('CMSGO_ALIAS') && CMSGO_ALIAS !== '' ? CMSGO_ALIAS : 'aid='.$content['article_id'];

    if($content['set_canonical'] && empty($cmsgo['force301_2struct'])) {
