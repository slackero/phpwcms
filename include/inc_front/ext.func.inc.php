...
function showPollImage($image, $zoom = 0) {
    ...
    $thumb_image = get_cached_image(array(
            ...
            "max_width"     =>  $image[4],
            "max_height"    =>  $image[5],
            "thumb_name"    =>  md5($image[2].$image[4].$image[5].$GLOBALS['cmsgo']["sharpen_level"].$GLOBALS['cmsgo']['colorspace'])
    ));
    ...
    if($zoom) {
        $zoominfo = get_cached_image(array(
            ...
            "thumb_name"    =>  md5($image[2].$GLOBALS['cmsgo']["img_prev_width"].$GLOBALS['cmsgo']["img_prev_height"].$GLOBALS['cmsgo']["sharpen_level"].$GLOBALS['cmsgo']['colorspace'])
        ));
    }
        ...
        if($cp = explode(',', $param)) {
            ...
            if($type === 'AS') {
                ...
                if(isset($mode[1])) {
                    ...
                    } elseif(empty($mode[2]) && strlen($mode[1]) > 4 && ($mode[1] == 'default' || is_file(CMSGO_TEMPLATE.'inc_cntpart/articlesummary/list/'.$mode[1]))) {
                        $template = $mode[1];
                    }
                }
                if(isset($mode[2])) {
                    ...
                    } elseif(strlen($mode[2]) > 4 && ($mode[2] == 'default' || is_file(CMSGO_TEMPLATE.'inc_cntpart/articlesummary/list/'.$mode[2]))) {
                        $template = $mode[2];
                    }
                }
    ...
    // Content Part mode CP, CPA, CPAD, CPS, CPAS, CPASD
        ...
        foreach($cp as $value) {
            ...
            if($mode == 'CP') {
                ...
                $sql .= "AND (ac.acontent_killdate IS NULL OR ac.acontent_killdate > NOW()) ";
            ...
            if(!empty($sql)) {
                ...
                if(isset($cresult[0]['acontent_type'])) {
                    foreach($cresult as $crow)  {
                        ...
                        if($crow["acontent_type"] == 30 && !isset($cmsgo['modules'][$crow["acontent_module"]])) {
                            continue;
                        }
                        ...
                        } elseif($crow["acontent_type"] == 30 && file_exists($cmsgo['modules'][$crow["acontent_module"]]['path'].'inc/cnt.article.php')) {
...
function get_article_data($article_id, $limit=0, $sort='', $where='', $not=array()) {
    ...
    if(!PREVIEW_MODE) {
        $sql_where[] = "(article_begin IS NULL OR article_begin < NOW())";
        $sql_where[] = "(article_end IS NULL OR article_end > NOW())";
    }
...
function parse_images($matches) {
    ...
    if(isset($matches[1])) {
        ...
        $image      = '<img src="'.CMSGO_URL.CMSGO_RESIZE_IMAGE.'/'.$width.'x'.$height.'x'.$crop;
...
function parse_downloads($match) {
    ...
    if(isset($match[1])) {
        ...
        if(isset($value['cnt_object']['cnt_files']['id']) && is_array($value['cnt_object']['cnt_files']['id']) && count($value['cnt_object']['cnt_files']['id'])) {
            ...
            $value['cnt_object']['cnt_files']['caption'] = isset($match[3]) ? @html_entity_decode(trim($match[3]), ENT_QUOTES, CMSGO_CHARSET) : '';
...
function seReferrer($ref = false) {
    ...
    if(CMSGO_CHARSET != 'utf-8' && cmsgo_seems_utf8($SeQuery)) {
        $SeQuery = makeCharsetConversion($SeQuery, 'utf-8', CMSGO_CHARSET, false);
    }
