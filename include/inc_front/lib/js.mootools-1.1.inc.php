...
function initJSLib() {
    if(empty($GLOBALS['block']['custom_htmlhead']['mootools.js'])) {
        if(CMSGO_USE_CDN) {
            $GLOBALS['block']['custom_htmlhead']['mootools.js'] = getJavaScriptSourceLink(CMSGO_HTTP_SCHEMA.'://ajax.googleapis.com/ajax/libs/mootools/1.1/mootools-yui-compressed.js');
        } else {
