...
function initJSLib() {
	if(empty($GLOBALS['block']['custom_htmlhead']['mootools.js'])) {
		if(CMSGO_USE_CDN) {
            $GLOBALS['block']['custom_htmlhead']['mootools.js'] = getJavaScriptSourceLink(CMSGO_HTTP_SCHEMA.'://ajax.googleapis.com/ajax/libs/mootools/1.2.6/mootools-yui-compressed.js');
        } else {
	...
	if($more === false) {
		...
		if(count($plugin)) {
			...
			foreach($plugin as $more) {
				if(empty($GLOBALS['block']['custom_htmlhead'][$more]) && is_file(CMSGO_TEMPLATE.'lib/mootools/more/'.$more.'.js')) {
