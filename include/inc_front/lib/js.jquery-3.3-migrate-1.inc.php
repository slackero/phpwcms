...
define('CMSGO_JSLIB', 'jquery-3.3-migrate-1');

/**
 * Init jQuery 3.3.x + jQuery Migrate 1.4.x & 3.1.x Library
 */
function initJSLib() {
    if(empty($GLOBALS['block']['custom_htmlhead']['jquery.js'])) {
        if(CMSGO_USE_CDN) {
            ...
            if(IE8_CC) {
                ...
                $GLOBALS['block']['custom_htmlhead']['jquery.js'] = '  <!--[if gte IE 9]><!-->' . getJavaScriptSourceLink(CMSGO_HTTP_SCHEMA.'://code.jquery.com/jquery-3.3.1.min.js', '') . '<!--<![endif]-->';
            } else {
