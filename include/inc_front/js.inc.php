...
if(empty($block['jslib'])) {
    $block['jslib'] = key($cmsgo['js_lib']);
}

// set if CDN can be used
define('CMSGO_USE_CDN', empty($block['googleapi']) ? FALSE : TRUE);

// include the related JavaScript Library wrapper
@include CMSGO_ROOT.'/include/inc_front/lib/js.'.$block['jslib'].'.inc.php';
...
function initVideoJs() {
    if(empty($GLOBALS['cmsgo']['video-js'])) {
        ...
        $GLOBALS['cmsgo']['video-js'] = CMSGO_HTTP_SCHEMA.'://vjs.zencdn.net/7.20.3/';
    } else {
        $GLOBALS['cmsgo']['video-js'] = rtrim($GLOBALS['cmsgo']['video-js'], '/') . '/';
    }
    $GLOBALS['block']['custom_htmlhead']['video-js.css'] = '  <link rel="stylesheet" type="text/css" href="' . $GLOBALS['cmsgo']['video-js'] . 'video-js.min.css" />';
}
