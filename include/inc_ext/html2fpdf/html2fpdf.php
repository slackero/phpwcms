...
class HTML2FPDF extends FPDF {
	...
	function HTML2FPDF( $orientation = 'P', $unit = 'mm', $format = 'A4' )
	{
		...
		$this->SetFont( 'Helvetica', '', 11 ); // Changeable?(not yet...)
	...
	function Header( $content = '' )
	{
		...
		if ( $this->usetableheader && $content != '' ) {
			$y = $this->y;
			foreach( $content as $tableheader ) {
				...
				// Border
				if ( isset( $border ) && $border != 'all' ) $this->_tableRect( $x, $y, $w, $h, $border );
		...
		foreach( $a as $i => $e ) {
			if ( $i % 2 == 0 ) {
				...
				if ( strpos( $e, "&" ) !== false ) { // HTML-ENTITIES decoding
					...
					*/
// EDIT START
// for use in phpwcms and handle given Charset there
					 $e = decode_entities( $e );
// EDIT END
				}
				...
				if ( $this->titulo ) {
					...
					if ( $this->specialcontent == "type=select" && $this->selectoption['ACTIVE'] == true ) { // SELECT tag (form element)
				...
				} elseif ( $this->pbegin || $this->HREF || $this->divbegin || $this->SUP || $this->SUB || $this->strike || $this->buffer_on ) {
	...
	function OpenTag( $tag, $attr )
	{
		...
		if ( isset( $attr['CLASS'] ) || isset( $attr['ID'] ) || isset( $attr['STYLE'] ) ) {
			if ( isset( $attr['CLASS'] ) ) {
		...
		switch ( $tag ) {
			...
			case 'OUTLINE': // custom-tag (CSS2 property - browsers don't support it yet - Jan2005)
				...
				break;
			case 'BDO':
				if ( isset( $attr['DIR'] ) && ( strtoupper( $attr['DIR'] ) == 'RTL' ) ) $this->divrevert = true;
				break;
			case 'S':
			case 'STRIKE':
			case 'DEL':
			...
			case 'ADDRESS':
				...
				if ( !$this->tdbegin && $this->x != $this->lMargin ) $this->Ln( $this->lineheight );
				break;
			case 'TABLE': // TABLE-BEGIN
				if ( $this->x != $this->lMargin ) $this->Ln( $this->lineheight );
				$this->tablestart = true;
				$this->table['nc'] = $this->table['nr'] = 0;
				if ( isset( $attr['REPEAT_HEADER'] ) && $attr['REPEAT_HEADER'] == true ) $this->UseTableHeader( true );
			...
			case 'UL':
				...
				if ( $this->listlvl == 0 ) {
					...
					if ( !$this->pjustfinished ) {
						...
						// OG 2007-04-28 check if $this->listoccur[$this->listlvl] empty
						// validation against 0 is not necessary because empty checks against !isset and value=0
						if ( empty($this->listoccur[$this->listlvl]) ) { //|| $this->listoccur[$this->listlvl] == 0
			...
			case 'LI':
				...
				if ( $this->listlvl == 0 ) { // in case of malformed HTML code. Example:(...)</p><li>Content</li><p>Paragraph1</p>(...)
					// First of all, skip a line
					if ( !$this->pjustfinished && $this->x != $this->lMargin ) $this->Ln( 2 * $this->lineheight );
					$this->oldx = $this->x;
			...
			case 'A':
				if ( isset( $attr['NAME'] ) && $attr['NAME'] != '' ) $this->textbuffer[] = array( '', '', '', array(), '', false, false, $attr['NAME'] ); //an internal link (adds a space for recognition)
			...
			case 'DIV':
				...
				$this->divbegin = true;
				if ( $this->x != $this->lMargin ) $this->Ln( $this->lineheight );
				if ( isset( $attr['ALIGN'] ) && $attr['ALIGN'] != '' ) $this->divalign = $align[strtolower( $attr['ALIGN'] )];
				break;
			case 'IMG':
				if ( !$this->isdisplay ) break;
				if ( !empty( $this->textbuffer ) && !$this->tablestart ) {
				...
				if ( isset( $attr['SRC'] ) ) {
					...
					// Edited by Oliver Georgi, 2007-04-30 to better handle src having NO width
					...
					if($attr['WIDTH'] == 0 || $attr['HEIGHT'] == 0) {
						...
						if($f_exists) {
							...
							$imgdata = getimagesize($srcpath);

							if(isset($imgdata[0]) && isset($imgdata[1])) {
								
								$attr['WIDTH']	= ConvertSize($imgdata[0], $this->pgwidth);
								$attr['HEIGHT']	= ConvertSize($imgdata[1], $this->pgwidth );
					...
					if ( $this->tdbegin ) {
						...
						// $f_exists = @fopen( $srcpath, "rb" );	//Edited by Oliver Georgi, 2007-04-30
						...
						$this->y = $bak_y;
						$this->x = $bak_x;
					} elseif ( $this->pbegin || $this->divbegin ) {
			...
			case 'P':
				...
				if ( $this->tablestart ) {
					$this->cell[$this->row][$this->col]['textbuffer'][] = array( ""/* previous: $e, UNDEF:BUG? */ ,
						$this->HREF,
						$this->currentstyle, $this->colorarray,
						$this->currentfont, $this->SUP, $this->SUB,
						''/*internal link*/,
				...
				if ( !$this->tablestart ) {
					switch ( strtoupper( $attr['TYPE'] ) ) {
						...
						default: // default == TEXT
							...
							if ( isset( $attr['VALUE'] ) ) $texto = $attr['VALUE'];
							$tamanho = 20;
							if ( isset( $attr['SIZE'] ) && ctype_digit( $attr['SIZE'] ) ) $tamanho = $attr['SIZE'];
							$this->SetFillColor( 235, 235, 235 );
							$this->x += 2;
	...
	function CloseTag( $tag )
	{
		...
		if ( $tag == 'EM' || $tag == 'CITE' ) $eq_tag = 'I';

		if ( $tag == 'OUTLINE' ) {
			if ( !$this->pbegin && !$this->divbegin && !$this->tablestart ) {
		...
		if ( $tag == 'A' ) {
			if ( !$this->pbegin && !$this->divbegin && !$this->tablestart && !$this->buffer_on ) {
		...
		if ( $tag == 'TH' || $tag == 'TD' ) $this->tdbegin = false;
		if ( $tag == 'SPAN' ) {
			if ( !$this->pbegin && !$this->divbegin && !$this->tablestart ) {
				if ( !empty( $this->CSSproperties ) ) {
					// Check if we have borders to print
					if ( ( $this->divborder || $this->dash_on || $this->dotted_on || $this->divbgcolor ) ) {
						$texto = '';
		...
		if ( $tag == 'P' || $tag == 'DIV' ) { // CSS in BLOCK mode
			$this->blockjustfinished = true; //Eliminate exceeding left-side spaces
			if ( !$this->tablestart ) {
		...
		if ( $tag == 'CODE' || $tag == 'PRE' || $tag == 'TT' || $tag == 'KBD' || $tag == 'SAMP' ) {
			$this->currentfont = '';
			$this->SetFont( 'helvetica' );
		}
		if ( $eq_tag == 'B' || $eq_tag == 'I' || $eq_tag == 'U' ) {
		...
		if ( $tag == 'SUB' || $tag == 'SUP' ) { // subscript or superscript
			if ( !$this->pbegin && !$this->divbegin && !$this->tablestart && !$this->buffer_on && !$this->strike ) {
		...
		if ( $tag == 'ADDRESS' || $tag == 'CENTER' ) { // <ADDRESS> or <CENTER> tag
			$this->blockjustfinished = true; //Eliminate exceeding left-side spaces
			if ( !$this->pbegin && !$this->divbegin && !$this->tablestart ) {
				// Save x,y coords ???
				$x = $this->x;
	...
	function printlistbuffer()
	{
		...
		foreach( $this->listitem as $item ) {
			...
			$this->SetFont( 'Helvetica', '', $oldsize );
	...
	function printbuffer( $arrayaux, $outofblock = false, $is_table = false )
	{
		...
		for( $i = 0;$i < $array_size; $i++ ) {
			...
			if ( empty( $vetor[0] ) && empty( $vetor[7] ) ) continue; //Ignore empty text and not carrying an internal link
			// Activating buffer properties
			if ( isset( $vetor[10] ) && !empty( $vetor[10] ) ) { // Background color
				$cor = $vetor[10];
				$this->SetFillColor( $cor['R'], $cor['G'], $cor['B'] );
				$this->divbgcolor = true;
			}
			if ( isset( $vetor[9] ) && !empty( $vetor[9] ) ) { // Outline parameters
				$cor = $vetor[9]['COLOR'];
			...
			if ( isset( $vetor[7] ) && $vetor[7] != '' ) { // internal link: <a name="anyvalue">
			...
			if ( isset( $vetor[6] ) && $vetor[6] === true ) { // Subscript
				$this->SUB = true;
				$this->SetFontSize( 6 );
			}
			if ( isset( $vetor[5] ) && $vetor[5] === true ) { // Superscript
				$this->SUP = true;
				$this->SetFontSize( 6 );
			}
			if ( isset( $vetor[4] ) && $vetor[4] != '' ) $this->SetFont( $vetor[4] ); // Font Family
			if ( !empty( $vetor[3] ) ) { // Font Color
				$cor = $vetor[3];
				$this->SetTextColor( $cor['R'], $cor['G'], $cor['B'] );
			}
			if ( isset( $vetor[2] ) && $vetor[2] != '' ) { // Bold,Italic,Underline styles
			...
			//if ( isset($vetor[0]) && $vetor[0]{0} == '»' && $vetor[0]{1} == '¤' && $vetor[0]{2} == '¬' ) { // identifier has been identified!
			// OG 2007-04-28 make strpos check -> should be 
			if ( isset($vetor[0]) && strpos($vetor[0], '»¤¬') === 0 ) {
				...
				switch ( strtoupper( $specialcontent['type'] ) ) {
					case 'IMAGE':
						...
						$xpos = 0;
						$ypos = 0;
						if ( isset( $specialcontent['ypos'] ) && $specialcontent['ypos'] != '' ) $ypos = ( float )$specialcontent['ypos'];
						if ( isset( $specialcontent['xpos'] ) && $specialcontent['xpos'] != '' ) $xpos = ( float )$specialcontent['xpos'];
			...
			if ( ( isset( $vetor[1] ) && $vetor[1] != '' ) || $this->HREF != '' ) {
				$this->SetTextColor( 0 );
				$this->SetStyle( 'U', false );
				$this->HREF = '';
			}
			if ( isset( $vetor[2] ) && $vetor[2] != '' ) {
				$this->SetStyle( 'B', false );
				$this->SetStyle( 'I', false );
				$this->SetStyle( 'U', false );
			}
			if ( isset( $vetor[3] ) && $vetor[3] != '' ) {
				unset( $cor );
				$this->SetTextColor( 0 );
			}
			if ( isset( $vetor[4] ) && $vetor[4] != '' ) $this->SetFont( 'Helvetica' );
			if ( isset( $vetor[5] ) && $vetor[5] === true ) {
				$this->SUP = false;
				$this->SetFontSize( 11 );
			}
			if ( isset( $vetor[6] ) && $vetor[6] === true ) {
				$this->SUB = false;
				$this->SetFontSize( 11 );
			}
		...
		if ( !empty( $this->CSSproperties ) && ( $this->divborder || $this->dash_on || $this->dotted_on || $this->divbgcolor ) ) {
	...
	function ReadMetaTags( $html )
	{
		...
		foreach ( $aux[1] as $meta ) {
			...
			if ( isset( $attr['NAME'] ) && isset( $attr['CONTENT'] ) ) {
				switch ( $attr['NAME'] ) {
					case "KEYWORDS": $this->SetKeywords( $attr['CONTENT'] );
						break;
					case "AUTHOR": $this->SetAuthor( $attr['CONTENT'] );
						break;
					case "DESCRIPTION": $this->SetSubject( $attr['CONTENT'] );
						break;
				}
			}
		}
  ...
  */
		...
		for( $j = 0 ; $j < $nc ; $j++ ) { // columns
			...
			for( $i = 0 ; $i < $nr ; $i++ ) { // rows
				if ( isset( $cs[$i][$j] ) && $cs[$i][$j] ) {
					...
					if ( $minwidth < 0 && ( - $minwidth ) > $wc['miw'] ) $wc['miw'] = ( - $minwidth ) + 2; //increase minimum width
		...
		for( $i = 0 ; $i < $numrows ; $i++ ) { // rows
			...
			for( $j = 0 ; $j < $numcols ; $j++ ) { // columns
				if ( isset( $cells[$i][$j] ) && $cells[$i][$j] ) {
					...
					if ( $c['s'] > $nostyles_size && !isset( $c['form'] ) ) { // Text with styles
	...
	function _tableWrite( &$table )
	{
		...
		if ( isset( $table['a'] ) && ( $table['w'] != $this->pgwidth ) ) {
