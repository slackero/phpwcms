 ...
 *
 * v1.5.1  01-27-04
 * fixed some RSS 1.0 glitches (thanks to Stèphane Vanpoperynghe)
...
class UniversalFeedCreator extends FeedCreator {
    ...
    function _setMIME($format) {
        ...
        } elseif (substr($format, 0, 4) === 'ATOM') { // ATOM, ATOM0.3, ATOM1.0
            $contentType = 'application/xml';
        } elseif ($format === 'JS' || $format === 'JAVASCRIPT') {
            $contentType = 'text/javascript';
        } else { // 1.0, 2.0, 0.91, RSS0.91, RSS1.0, RSS2.0, PIE0.1, OPML
    ...
    function __construct($dateString = "") {
        ...
        if (preg_match("~(?:(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun),\\s+)?(\\d{1,2})\\s+([a-zA-Z]{3})\\s+(\\d{4})\\s+(\\d{2}):(\\d{2}):(\\d{2})\\s+(.*)~", $dateString, $matches)) {
            ...
            if (substr($matches[7], 0, 1) == '+' or substr($matches[7], 0, 1) == '-') {
                $tzOffset = (intval(substr($matches[7], 0, 3)) * 60 + (int) substr($matches[7], -2)) * 60;
            } else {
...
class MBOXCreator extends FeedCreator {
    ...
    function createFeed() {
        ...
        for ($i = 0; $i < count($this->items); $i++) {
            ...
            $feed     .= "Content-Type: text/plain; charset=\"" . $this->encoding . "\"\n";
...
class HTMLCreator extends FeedCreator {
    ...
    function createFeed() {
        ...
        $targetInsert = $this->openInNewWindow ? " target='_blank'" : '';

        // use this array to put the lines in and implode later with "document.write" javascript
        $feedArray = array();
        if ($this->image != null) {
            $imageStr = "<a href='" . $this->image->link . "'" . $targetInsert . ">" . "<img src='" . $this->image->url . "' border='0' alt='" . FeedCreator::iTrunc(html($this->image->title), 100) . "' align='" . $this->imageAlign . "' ";
            if ($this->image->width) {
        ...
        if ($this->title) {
            $feedArray[] = "<div class='" . $this->stylePrefix . "title'><a href='" . $this->link . "' " . $targetInsert . " class='" . $this->stylePrefix . "title'>" . FeedCreator::iTrunc(html($this->title), 100) . "</a></div>";
        }
        ...
        for ($i = 0; $i < count($this->items); $i++) {
            ...
            if ($this->items[$i]->title) {
                if ($this->items[$i]->link) {
                    $feedArray[]
                        = "<div class='" . $this->stylePrefix . "item_title'><a href='" . $this->items[$i]->link . "' class='" . $this->stylePrefix . "item_title'" . $targetInsert . ">" . FeedCreator::iTrunc(html(strip_tags($this->items[$i]->title)), 100) . "</a></div>";
                }
                else {
                    $feedArray[]
                        = "<div class='" . $this->stylePrefix . "item_title'>" . FeedCreator::iTrunc(html(strip_tags($this->items[$i]->title)), 100) . "</div>";
                }
            }
            if ($this->items[$i]->getDescription()) {
                $feedArray[]
                    = "<div class='" . $this->stylePrefix . "item_description'>" . str_replace("]]>", "", str_replace("<![CDATA[", "", $this->items[$i]->getDescription())) . "</div>";
            }
        }
        if ($this->footer) {
