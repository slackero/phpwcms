...
class MagpieRSS {
    ...
    function normalize () {
        ...
        if ( $this->is_atom() ) {
			...
			if($this->is_atom() == '1.0') {
            	$this->channel['description'] = $this->channel['subtitle'];
			} else {
				$this->channel['description'] = $this->channel['tagline'];
			}
			if(empty($this->channel['link']) && isset($this->channel['link_'])) {
				$this->channel['link'] = $this->channel['link_'];
			}
            for ( $i = 0; $i < count($this->items); $i++) {
				...
				if(empty($item['link']) && isset($item['link_'])) {
    ...
    function is_atom() {
        ...
        if ( $this->feed_type == ATOM ) {
            return empty($this->feed_version) ? '1.0' : $this->feed_version;
        }
        else {
            return false;
        }
