...
if ($action == 'form') {
    ...
    if(isset($data[0]['article_id'])) {
        foreach($data as $crow) {
            echo '<a href="phpwcms.php?'.get_token_get_string().'&do=articles&p=2&s=1&aktion=2&id='.$crow[1].'&acid='.$crow[2].'" target=_blank>'.$crow[0].' <i class="fas fa-pencil-alt text-primary ml-1"></i></a><br>';
        }
    }
