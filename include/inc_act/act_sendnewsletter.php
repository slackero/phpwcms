...
if($newsletter_id) {
    ...
    $sql  = "SELECT *FROM ".DB_PREPEND."cmsgo_newsletter WHERE newsletter_id=".$newsletter_id." LIMIT 1";
...
if(!$newsletter) {
    ...
    if(!empty($_GET['send_testemail'])) {
        ...
        $sql .= 'FROM '.DB_PREPEND.'cmsgo_address ';
        $sql .= 'LEFT JOIN '.DB_PREPEND.'cmsgo_newsletterqueue ';
        $sql .= 'ON '.DB_PREPEND.'cmsgo_address.address_id = '.DB_PREPEND.'cmsgo_newsletterqueue.queue_rid ';
        $sql .= 'WHERE '.DB_PREPEND.'cmsgo_newsletterqueue.queue_status=0 AND ';
        $sql .= DB_PREPEND.'cmsgo_newsletterqueue.queue_pid='.$newsletter["newsletter_id"];
        if($loop) {
            $sql .= ' LIMIT '.$loop;
        }
    ...
    if(count($recipient)) {
        ...
        if(!empty($newsletter['newsletter_vars']['template']) && ($template = @file_get_contents(CMSGO_TEMPLATE.'inc_newsletter/'.$newsletter['newsletter_vars']['template'].'/newsletter.tmpl'))) {
...
function build_email_text($text, $value) {
    ...
    $text = str_replace('###SITE_URL###', CMSGO_URL, $text);
    $text = str_replace('###VERIFY_LINK###', CMSGO_URL.'verify.php?s='.$refkey, $text);
    $text = str_replace('###DELETE_LINK###', CMSGO_URL.'verify.php?u='.$refkey, $text);
	//tag for opener, add absolute url
	$text = str_replace('###OPENER###', '<img src="'.CMSGO_URL.'verify.php?o='.intval($value['queue_id']).'" alt="" />', $text);
	$text = str_replace('href="download.php', 'href="'.CMSGO_URL.'download.php', $text);
...
function updateSentDate($id=0) {
    ...
    $sql  = "UPDATE ".DB_PREPEND."cmsgo_newsletter SET ";
