...
function cmsgo_form_encode($in_str, $charset) {
   $out_str = $in_str;
   if ($out_str && $charset) {
...
if(	isset($cmsgo["formmailer_set"])
	&& !empty($cmsgo["formmailer_set"]['global_recipient_email'])
	&& $cmsgo["formmailer_set"]['global_recipient_email'] != 'form@localhost'
	&& is_valid_email($cmsgo["formmailer_set"]['global_recipient_email'])) {
	$recipient = $cmsgo["formmailer_set"]['global_recipient_email'];
}
...
if(isset($_POST["subject"])) {
	...
	$subject_encoded = cmsgo_form_encode($subject, $charset);
	unset($_POST["subject"]);
}
...
if(isset($_POST["send_copy"])) {
	if(!empty($cmsgo["formmailer_set"]['allow_send_copy']) && intval($_POST["send_copy"])) {
...
if(isset($form_error)) {
	if(isset($redirect_error)) {
		...
		$error_template = read_textfile(CMSGO_ROOT.'/include/inc_lang/formmailer/'.$lang.'_formmailer.error.html');
	...
	if(isset($redirect) && !$false) {
		...
		$success_template = read_textfile(CMSGO_ROOT.'/include/inc_lang/formmailer/'.$lang.'_formmailer.success.html');
