...
if($_SESSION["wcs_user_admin"] === 1) { // Only for admin users
    ...
    if(isset($_POST["acat_id"]) && $_POST["acat_id"] === 'index') {
        ...
        write_textfile(CMSGO_ROOT.'/include/config/conf.indexpage.inc.php', $sql);
    }
    ...
    if(isset($_POST["acat_new"]) && intval($_POST["acat_new"]) == 1 && !intval($_POST["acat_id"]) && $_POST["acat_id"] !== 'index') {
        if(trim($_POST["acat_name"])) {
            ...
            $sql = "INSERT INTO ".DB_PREPEND."cmsgo_articlecat (acat_name, acat_title, acat_info, acat_aktiv, acat_ssl, acat_regonly, ".
    ...
    if(isset($_POST["acat_new"]) && isset($_POST["acat_id"]) && intval($_POST["acat_new"]) == 0 && intval($_POST["acat_id"])) {
        if(trim($_POST["acat_name"])) {
            ...
            $sql = "UPDATE ".DB_PREPEND."cmsgo_articlecat SET ".
...
if($action) {
    ...
    if($_SESSION["wcs_user_admin"] === 1) {
        ...
        if($action === 1) {
            ...
            if($do[1]) { // && $do[2] = 0 for Root
                $sql =  "UPDATE ".DB_PREPEND."cmsgo_articlecat SET acat_struct=".$do[2].", acat_sort=".$do[3]." WHERE acat_id=".$do[1];
                _dbQuery($sql, 'UPDATE');
            }
        ...
        // Change sorting
            ...
            if($do[1] && $do[2]>=10 && $do[3] && $do[4]>=10) {
                $sql =  "UPDATE ".DB_PREPEND."cmsgo_articlecat SET acat_sort=".$do[2]." WHERE acat_id=".$do[1];
                _dbQuery($sql, 'UPDATE');

                $sql =  "UPDATE ".DB_PREPEND."cmsgo_articlecat SET acat_sort=".$do[4]." WHERE acat_id=".$do[3];
                _dbQuery($sql, 'UPDATE');
            }
            ...
            if($do[1]) {
                ...
                if(count($article_del)) {
                    ...
                    foreach($article_del as $value) {
                        ...
                        $sql = "DELETE FROM ".DB_PREPEND."cmsgo_cache WHERE cache_aid=".intval($value);
                        _dbQuery($sql, 'DELETE');
                    ...
                    if(count($a_del)) {
                        $sql = "UPDATE ".DB_PREPEND."cmsgo_article SET article_deleted=9, article_alias=CONCAT(article_alias,'_del-','".date('YmdHis')."') WHERE article_id IN (".implode(',', $a_del).")";
                        _dbQuery($sql, 'UPDATE');
                    }
                }
                ...
                if(count($struct_del)) {
                    ...
                    foreach($struct_del as $value) {
                        ...
                        $sql = "DELETE FROM ".DB_PREPEND."cmsgo_cache WHERE cache_cid=".intval($value);
                        _dbQuery($sql, 'DELETE');
                    ...
                    if(count($s_del)) {
                        $sql = "UPDATE ".DB_PREPEND."cmsgo_articlecat SET acat_trash=9, acat_alias=CONCAT(acat_alias,'_del-','".date('YmdHis')."') WHERE acat_id IN (".implode(',', $s_del).")";
                        _dbQuery($sql, 'UPDATE');
                    }
                }
    ...
    if($action === 3) {
        ...
        if($do[1]) { // && $do[2] = 0 for Root
            ...
            $sql =  "UPDATE ".DB_PREPEND."cmsgo_article SET article_cid=".$do[2].", article_sort=".$new_sort." WHERE article_id=".$do[1];
            _dbQuery($sql, 'UPDATE');
        }
    ...
    // Change sorting of articles
        ...
        if($do[1] && $do[2]>=10 && $do[3] && $do[4]>=10) {
            $sql =  "UPDATE ".DB_PREPEND."cmsgo_article SET article_sort=".$do[2].", article_tstamp=article_tstamp WHERE article_id=".$do[1];
            _dbQuery($sql, 'UPDATE');

            $sql =  "UPDATE ".DB_PREPEND."cmsgo_article SET article_sort=".$do[4].", article_tstamp=article_tstamp WHERE article_id=".$do[3];
            _dbQuery($sql, 'UPDATE');
        }
    ...
    // Cut & Paste Content Part
        ...
        $do[3] = intval($do[3]); //sort Number
        if($do[1]) {
            $sql = "SELECT acontent_aid, acontent_sorting FROM ".DB_PREPEND."cmsgo_articlecontent WHERE acontent_id=".$do[1];
            $result = _dbQuery($sql);
            if(isset($result[0]['acontent_aid'])) {

                $sql = "UPDATE ".DB_PREPEND."cmsgo_articlecontent SET acontent_sorting=acontent_sorting-10 WHERE acontent_aid=".$result[0]['acontent_aid']." AND acontent_sorting >= ".$result[0]['acontent_sorting']."+10";
                _dbQuery($sql, 'UPDATE');

                $sql = "UPDATE ".DB_PREPEND."cmsgo_articlecontent SET acontent_sorting=acontent_sorting+10 WHERE acontent_aid=".$do[2]." AND acontent_sorting >= ".$do[3]."+10";
                _dbQuery($sql, 'UPDATE');

                $sql = "UPDATE ".DB_PREPEND."cmsgo_articlecontent SET acontent_aid=".$do[2].", acontent_sorting=".$do[3]."+10 WHERE acontent_id=".$do[1];
                _dbQuery($sql, 'UPDATE');
    ...
    // Copy Content Part
        ...
        if($do[1]) {
            ...
            $sql  = "SELECT * FROM ".DB_PREPEND."cmsgo_articlecontent WHERE acontent_id=".$do[1];
            $result = _dbQuery($sql);
            if(isset($result[0]['acontent_id'])) {
                ...
                $sql = "INSERT INTO ".DB_PREPEND."cmsgo_articlecontent (".$key1s.") VALUES (".$value1s.")";
                _dbQuery($sql, 'INSERT');
            }
        }
    }
...
function get_struct_del_id($s_id=0) {
    ...
    $sql = "SELECT article_id FROM ".DB_PREPEND."cmsgo_article WHERE article_deleted=0 AND article_cid=".$s_id;
    $result = _dbQuery($sql);
    ...
    $sql = "SELECT acat_id FROM ".DB_PREPEND."cmsgo_articlecat WHERE acat_trash=0 AND acat_struct=".$s_id;
    $result = _dbQuery($sql);
...
function copy_article_to_level($do) {
    ...
    $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_article WHERE article_deleted=0 AND article_id=".$do[1];
    $result = _dbQuery($sql);
    if(isset($result[0]['article_id'])) {
        ...
        $sql =  "INSERT INTO ".DB_PREPEND."cmsgo_article (".$keys.") VALUES (".$values.")";
        $result = _dbQuery($sql, 'INSERT');
        ...
        if(isset($result['INSERT_ID'])) {
            ...
            $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_articlecontent WHERE acontent_aid=".$do[1];
            $result = _dbQuery($sql);
...
function copy_level_to_level($do) {
    ...
    $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_articlecat WHERE acat_trash=0 AND acat_id=".$do[1];
    $result = _dbQuery($sql);
    $acat_insert_id = 0;
    ...
    if(isset($result[0]['acat_id'])) {
        ...
        $sql = "INSERT INTO ".DB_PREPEND."cmsgo_articlecat (".$keys.") VALUES (".$values.")";
        $result = _dbQuery($sql, 'INSERT');
        if(isset($result['INSERT_ID'])) {
    ...
    if($acat_insert_id) {
        $sql = "SELECT article_id FROM ".DB_PREPEND."cmsgo_article WHERE article_deleted=0 AND article_cid=".$do[1];
        $result = _dbQuery($sql);
        if(isset($result[0]['article_id'])) {
            foreach($result as $row) {
        ...
        $sql = "SELECT acat_id,acat_sort FROM ".DB_PREPEND."cmsgo_articlecat WHERE acat_trash=0 AND acat_struct=".$do[1];
        $result = _dbQuery($sql);
        if(isset($result[0]['acat_id'])) {
            foreach($result as $row) {
