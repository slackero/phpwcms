...
if(isset($_GET['del']) && intval($_GET['del'])) {
    ...
    $sql  = "UPDATE ".DB_PREPEND."cmsgo_guestbook SET guestbook_trashed=9 WHERE guestbook_cid=";
...
if(isset($_GET['edit']) && intval($_GET['edit'])) {
    ...
    if(isset($_POST['gbsubmit'])) {
        ...
        if(!$gberror) {
            $sql  = "UPDATE ".DB_PREPEND."cmsgo_guestbook SET ";
...
if($gbid) {
    $sql  = "SELECT * FROM ".DB_PREPEND."cmsgo_guestbook WHERE guestbook_cid=".$gbid;
