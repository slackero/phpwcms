...
if($_SESSION["wcs_user_admin"] == 1) { //Wenn Benutzer Admin-Rechte hat
    ...
    switch(intval($do[0])) {
        ...
        case 1: //Aktiv/Inaktiv File Category
                ...
                if($do[1]) {
                    $sql =  "UPDATE ".DB_PREPEND."cmsgo_filecat SET fcat_aktiv=".$do[2]." WHERE fcat_id=".$do[1];
                    _dbQuery($sql, 'UPDATE');
                }
                break;
        ...
        case 2: //Aktiv/Inaktiv File Key
                ...
                $do[2] = intval($do[2]); //active value
                if($do[1]) {
                    $sql =  "UPDATE ".DB_PREPEND."cmsgo_filekey SET fkey_aktiv=".$do[2]." WHERE fkey_id=".$do[1];
                    _dbQuery($sql, 'UPDATE');
                }
                break;
        ...
        case 8: //Löschen der File Category
                ...
                if($do[1]) {
                    $sql =  "UPDATE ".DB_PREPEND."cmsgo_filecat SET fcat_deleted=9 WHERE fcat_id=".$do[1];
                    _dbQuery($sql, 'UPDATE');
                }
                break;
        ...
        case 9: //L�schen des File Keys
                ...
                if($do[1] && $do[2]) {
                    $sql =  "UPDATE ".DB_PREPEND."cmsgo_filekey SET fkey_deleted=9 WHERE fkey_id=".$do[1]." AND fkey_cid=".$do[2];
                    _dbQuery($sql, 'UPDATE');
                }
                break;
