...
if($action == 'exportformresult' && isset($_GET['fid']) && ($fid = intval($_GET['fid']))) {
	...
	$data = _dbQuery("SELECT *, DATE_FORMAT(formresult_createdate, '%Y-%m-%d %H:%i:%s') AS formresult_date  FROM ".DB_PREPEND.'cmsgo_formresult WHERE formresult_pid='.$fid);
	...
	foreach($export[0] as $key => $value) {
		$elements[0] .= '		<th style="font-family: arial, verdana, sans-serif; padding: 5px; font-size: 14px;">';
		$elements[0] .= $key;
		$elements[0] .= '</th>'.LF;
	}
	$elements[0] .= '	</tr>';
	...
	for($x = 1; $x < $row; $x++) {
		...
		foreach($export[0] as $key => $value) {
			...
			$elements[$x] .= '		<td style="font-family: arial, verdana, sans-serif; padding: 5px; font-size: 14px;">';
	...
	echo '<table border="1" cellspacing="1" cellpadding="2" style="border: 1px solid #000;">'.LF;
	...
	$data		= _getDatabaseQueryResult("SELECT *, DATE_FORMAT(formresult_createdate, '%Y-%m-%d %H:%i:%S') AS formresult_date FROM ".DB_PREPEND.'cmsgo_formresult WHERE formresult_pid='.$fid);
	...
	for($x = 1; $x < $row; $x++) {
		...
		foreach($export[0] as $key => $value) {
			...
			if(isset($export[$x][$key])) {
				...
				if(strpos($export[$x][$key], '/'.$cmsgo["content_path"].'form/')) {
	...
	$data = _dbQuery("SELECT *, DATE_FORMAT(address_tstamp, '%Y-%m-%d %H:%i:%s') AS addate FROM ".DB_PREPEND."cmsgo_address".$_userInfo['where_query'].' ORDER BY address_tstamp');
	if($data) {
