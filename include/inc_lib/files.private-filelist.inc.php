...
if(isset($file_result[0]['f_id'])) {
    ...
    foreach($file_result as $file_row) {
        ...
        echo '<a class="btn btn-sm btn-blue" role="button" title="'.$BL['be_fprivfunc_editfile'].": ".$filename.'" data-bs-toggle="tooltip" href="'.$zieldatei.'&amp;editfile='.$file_row["f_id"].'"><i class="fa-solid fa-pencil-alt fa-fw"></i></a>';
        ...
        if($cutID == $file_row["f_id"]) {
            echo '<i class="fa-solid fa-cut disabled" aria-hidden="true" data-bs-toggle="tooltip" title="'.$BL['be_fprivfunc_clipfile'].': '.$filename.'"></i>';
        } else {
