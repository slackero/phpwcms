 ...
 *
 * Taken from MediaWiki 1.28.0 and adapted for cmsgo
...
class ConvertibleTimestamp {
	...
	public function setTimestamp( $ts = false ) {
		...
		# Day of week
			...
			throw new Exception( __METHOD__ . ": Invalid timestamp - $ts" );
		}
		...
		try {
			...
			throw new Exception( __METHOD__ . ': Invalid timestamp format.', $e->getCode(), $e );
		}

		if ( $final === false ) {
			throw new Exception( __METHOD__ . ': Invalid timestamp format.' );
		}
	...
	public function getTimestamp( $style = TS_UNIX ) {
		if ( !isset( self::$formats[$style] ) ) {
			throw new Exception( __METHOD__ . ': Illegal timestamp output type.' );
		}
		...
		if ( $style == TS_MW && strlen( $output ) !== 14 ) {
			throw new Exception( __METHOD__ . ': The timestamp cannot be represented in the specified format' );
		}
	...
	public function setTimezone( $timezone ) {
		try {
			...
			throw new Exception( __METHOD__ . ': Invalid timezone.', $e->getCode(), $e );
		}
	}
