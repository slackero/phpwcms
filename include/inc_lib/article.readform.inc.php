...
if($set_livedate && $content['livedate']) {
    $content['livedate'] = cmsgo_strtotime($content['livedate']);
...
if($set_killdate && $content['killdate']) {
    $content['killdate'] = cmsgo_strtotime($content['killdate']);
...
if(!empty($_POST['ctype_change_aid'])) {
    ...
    if($ctype_change_aid && $ctype_change_aid != $content["aid"]) {
        ...
        $ctype_change_aid   = _dbQuery('SELECT article_id FROM '.DB_PREPEND.'cmsgo_article WHERE article_id='.$ctype_change_aid.' AND article_deleted=0');
...
if($content["type"] != 30 && file_exists(CMSGO_ROOT."/include/inc_lib/content/cnt".$content["type"].".readform.inc.php")) {

    $content["module"]  = '';
    include_once CMSGO_ROOT."/include/inc_lib/content/cnt".$content["type"].".readform.inc.php";

} elseif($content["type"] == 30 && file_exists($cmsgo['modules'][$content['module']]['path'].'inc/cnt.post.php')) {

    include_once $cmsgo['modules'][$content['module']]['path'].'inc/cnt.post.php';

} else {

    $content["module"]  = '';
    include_once CMSGO_ROOT."/include/inc_lib/content/cnt0.readform.inc.php";
