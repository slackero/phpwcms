...
function handle_csrf_error($reason)
{
    ...
    if (empty($BL)) {
        ...
        } elseif (!empty($_COOKIE['phpwcmsBELang'])) {
            $user_lang = strtolower(substr(trim($_COOKIE['phpwcmsBELang']), 0, 2));
        }

        $en_lang_file = PHPWCMS_ROOT . '/include/inc_lang/backend/en/lang.inc.php';
        if (is_file($en_lang_file)) {
            include_once $en_lang_file;
        }

        if ($user_lang !== 'en') {
            $cust_lang_file = PHPWCMS_ROOT . '/include/inc_lang/backend/' . $user_lang . '/lang.inc.php';
                        ...
                        <div class="card mt-5 shadow-sm">
                            ...
                            <div class="card-body text-center pb-4">
                                ...
                                <div class="d-flex justify-content-center gap-2" style="gap: 12px;">
                                    ...
                                    <a href="<?php echo PHPWCMS_URL . get_login_file(); ?>" class="btn btn-light"><?php echo html_specialchars($BL['CSRF_BTN_LOGIN'] ?? 'Login'); ?></a>
                                </div>
 							</div>
                        </div>
                    </div>
