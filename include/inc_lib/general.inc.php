...
function list_country($default = '', $lang = '', $prepend = []) {
    ...
    if (empty($default)) {
        $default = strtoupper($GLOBALS['cmsgo']['default_lang']);
    }
    $country_list = '';
...
function is_ext_true($extension) {
    ...
    if ($ext && !empty($GLOBALS['cmsgo']["imgext_disabled"])) {
        if (is_string($GLOBALS['cmsgo']["imgext_disabled"])) {
            $GLOBALS['cmsgo']["imgext_disabled"] = convertStringToArray(strtolower($GLOBALS['cmsgo']["imgext_disabled"]));
        }
        if (in_array($ext, $GLOBALS['cmsgo']["imgext_disabled"])) {
            $ext = false;
        }
    }
