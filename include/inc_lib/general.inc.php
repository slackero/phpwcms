...
function list_country($c, $lang = '') {
    ...
    //$c = selected value
    if (empty($c)) {
        $c = strtoupper($GLOBALS['cmsgo']['default_lang']);
    }
    $country_list = '';
    ...
    if ($get == 'COUNTRY_NAME') {
        ...
        if (isset($result[0])) {
            foreach ($result as $row) {
                $cmsgo['country'][$country_lang_var][$row['country_iso']] = $row['country'];
            }
        }
    }
...
function extimg($ext) {
    //get extension image
    $img = array("exe" => "file", "com" => "file", "bat" => "filef", "pdf" => "file-pdf", "txt" => "file-altf", "xls" => "file-excel", "cvs" => "file-excel", "rtf" => "file-alt", "htm" => "file-code", "html" => "file-code", "pix" => "file", "tif" => "file-image", "jpg" => "file-image", "jpeg" => "file-image", "gif" => "file-image", "png" => "file-image", "psd" => "file", "rar" => "file-archive", "zip" => "file-archive", "tar" => "file-archive", "gzip" => "file-archive", "sit" => "file", "sea" => "file", "doc" => "file-word", "dot" => "file", "ai" => "file", "ps" => "file", "eps" => "file", "gz" => "file-archive", "tgz" => "file-archive", "aif" => "file", "aiff" => "file", "mp3" => "file-audio", "snd" => "file", "wav" => "file-audio", "mid" => "file-audio", "mov" => "file-video", "avi" => "file-video", "qt" => "file-video", "mpeg" => "file-video",);
...
function is_ext_true($extension) {
    ...
    if ($ext && !empty($GLOBALS['cmsgo']["imgext_disabled"])) {
        $GLOBALS['cmsgo']["imgext_disabled"] = str_replace(' ', '', $GLOBALS['cmsgo']["imgext_disabled"]);
        $GLOBALS['cmsgo']["imgext_disabled"] = strtolower($GLOBALS['cmsgo']["imgext_disabled"]);
        $disabled_ext = explode(',', $GLOBALS['cmsgo']["imgext_disabled"]);
...
function make_date($datestring, $dateformat = "d.m.y") {
    return cmsgo_strtotime($datestring, $dateformat, '');
}
...
function sendEmail($data = array('recipient' => '', 'toName' => '', 'subject' => '', 'isHTML' => 0, 'html' => '', 'text' => '', 'attach' => array(), 'from' => '', 'fromName' => '', 'sender' => '', 'stringAttach' => array())) {
    ...
    $sendTo = array();
    $from = empty($data['from']) || !is_valid_email($data['from']) ? $cmsgo['SMTP_FROM_EMAIL'] : $data['from'];
...
function replaceGlobalRT($string = '') {
    $string = str_replace(array('{SITE}', '{CMSGO_URL}'), CMSGO_URL, $string);
    $string = str_replace('{CMSGO_TEMPLATE}', TEMPLATE_PATH, $string);
    $string = str_replace('{IP}', CMSGO_GDPR_MODE ? getAnonymizedIp() : getRemoteIP(), $string);
...
function formatRTDate($matches) {
    ...
    return $type($matches[0], cmsgo_strtotime($matches[1], null, now()));
}
...
function saveUploadedFile($file, $target, $exttype = '', $imgtype = '', $rename = 0, $maxsize = 0) {
    ...
    $file_status['maxsize'] = empty($file_status['maxsize']) ? $GLOBALS['cmsgo']['file_maxsize'] : $file_status['maxsize'];
    ...
    if (count($rename)) {
        ...
        foreach ($rename as $value) {
            switch ($value) {
                case 1:
                    $_temp_name = str_replace(array(':', '/', "\\", ' '), array('-', '-', '-', '_'), cmsgo_remove_accents($_temp_name));
...
function attribute_name_clean($name = '') {
    $name = trim(cmsgo_remove_accents($name));
...
function uri_sanitize($text) {
    ...
    $text = get_alnum_dashes($text, !CMSGO_ALIAS_UTF8, '-', CMSGO_ALIAS_WSLASH);
    $text = trim($text);
    if ($text !== '') {
