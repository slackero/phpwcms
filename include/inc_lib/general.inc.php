...
{
    if (strtolower(substr($GLOBALS['cmsgo']['site'], 0, 5)) != 'https') {
        $url = 'http://';
    } else {
        $url = 'https://';
    }
...
function getFormTrackingValue()
{
    ...
    $entry_id   = time();
    if (!empty($GLOBALS['cmsgo']["form_tracking"])) {
...
function saveUploadedFile($file, $target, $exttype='', $imgtype='', $rename=0, $maxsize=0)
{
    ...
    if (count($rename)) {
        ...
        foreach ($rename as $value) {
            switch ($value) {
                case 1:     $_temp_name = str_replace(array(':','/',"\\",' '), array('-','-','-','_'), cmsgo_remove_accents($_temp_name));
