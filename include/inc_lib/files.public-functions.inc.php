...
function list_public($pid, $counter, $zieldatei, $userID, $wcs_user_thumb, $cmsgo) {
    ...
    foreach($result as $row) {
        ...
        if(!$klapp_status && $count_wert) {
            list_public($row["f_id"], $counter+1, $zieldatei, $userID, $wcs_user_thumb, $cmsgo);
...
function has_public_files($folder_id, $userID) {
    ...
    $file_sql = 'SELECT COUNT(f_id) FROM '.DB_PREPEND.'cmsgo_file WHERE f_pid='.$folder_id.' AND f_uid='.$userID.' AND f_public=1 AND f_aktiv=1 AND f_kid=1 AND f_trash=0';
    if(_dbQuery($file_sql, 'COUNT') > 0) {
        return true;
    }

    // Check if there are subfolders, and if any of them contain public files
    $sub_sql = 'SELECT f_id FROM '.DB_PREPEND.'cmsgo_file WHERE f_pid='.$folder_id.' AND f_uid='.$userID.' AND f_public=1 AND f_aktiv=1 AND f_kid=0 AND f_trash=0';
