...
function list_public($pid, $counter, $zieldatei, $userID, $show_thumb=1, $cmsgo) {
    ...
    foreach($result as $row) {
        ...
        if(!$klapp_status && $count_wert) {
            list_public($row["f_id"], $counter+1, $zieldatei, $userID, $show_thumb, $cmsgo);

            //Listing eventuell im Verzeichnis enthaltener Dateien
            $file_sql  = "SELECT * FROM ".DB_PREPEND."cmsgo_file WHERE f_pid=".$row["f_id"]." AND f_uid=".intval($userID);
