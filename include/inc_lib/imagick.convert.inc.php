 ...
 */
require_once CMSGO_ROOT . '/include/inc_lib/helper.image.php';
...
function image_manipulate($config=array()) {
    ...
    $image_config = array(
        'image_library' => $cmsgo['image_library'],
        'library_path' => $cmsgo['library_path'],
    ...
    if(empty($GLOBALS['cmsgo']['gd_memcheck_off']) && ($cmsgo['image_library'] === 'gd2' || $cmsgo['image_library'] === 'gd')) {
    ...
    if(!$config['crop_image'] && substr($cmsgo['image_library'], 0, 2) === 'gd' && !empty($IMG->orig_width) && !empty($IMG->orig_height) && $image_config['width'] > $IMG->orig_width && $image_config['height'] > $IMG->orig_height) {
...
function get_cached_image($val=array(), $db_track=true, $return_all_imageinfo=true) {
    ...
    $val = array_merge(
        array(
            'max_width' => $GLOBALS['cmsgo']['img_list_width'],
 ...
 * @param bool $cache
 */
function cmsgo_empty_gif($cache=false) {
 ...
 * @return array
 */
function cmsgo_convertimage_gs($source_img='', $target_img='', $config=array()) {
    ...
    $IMG = new Cmsgo_Image_lib($config);
    $IMG->resize();
 ...
 * @return array|null
 */
function cmsgo_svg_getimagesize($svg_file) {
