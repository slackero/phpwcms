...
if(!isset($cmsgo['set_article_active'])) {
    $cmsgo['set_article_active'] = 1;
}
if(!isset($cmsgo['set_category_active'])) {
    $cmsgo['set_category_active'] = 1;
}
if(!isset($cmsgo['set_file_active'])) {
    $cmsgo['set_file_active'] = 1;
}
if(!isset($cmsgo['set_news_active'])) {
    $cmsgo['set_news_active'] = 1;
}
if(isset($cmsgo['set_sociallink']) && is_array($cmsgo['set_sociallink'])) {
    $cmsgo['set_sociallink'] = array_merge(array('article' => false, 'articlecat' => false, 'news' => false, 'shop' => false, 'render' => true), $cmsgo['set_sociallink']);
} else {
    $cmsgo['set_sociallink'] = array('article' => false, 'articlecat' => false, 'news' => false, 'shop' => false, 'render' => true);
}
...
function update_cache() {
    ...
    //$sql = "UPDATE ".DB_PREPEND."cmsgo_cache SET cache_timeout='0'";
    //_dbQuery($sql, 'UPDATE');
}
...
function getParentStructArray($structID) {
    ...
    $result = _dbQuery("SELECT * FROM ".DB_PREPEND."cmsgo_articlecat WHERE acat_id=".intval($structID)." LIMIT 1");
...
function getArticleSortValue($cat_id=0) {
    ...
    $sql  = "SELECT article_id, article_sort FROM ".DB_PREPEND."cmsgo_article ";
...
function getArticleReSorted($cat_id, $ordered_by) {
    ...
    if(isset($result[0]['article_id'])) {
        ...
        foreach($result as $row) {
            ...
            $update_sql  = "UPDATE ".DB_PREPEND."cmsgo_article SET ";
...
function cmsgoversionCheck() {
    ...
    if(empty($cmsgo['version_check'])) {
        $version = '<div class="versioncheck">';
        $version .= '<h1>' . $BL['Version_information'] . '</h1>';
        $version .= '<p class="alert alert-info">';
        $version .= sprintf($BL['Current_version_info'], CMSGO_VERSION . ' ('. CMSGO_RELEASE_DATE. ', r' . CMSGO_REVISION . ')');
        $version .= '</p>';
        $version .= '</div>';
    ...
    if(!empty($_SESSION['cmsgo_version_check'])) {
        return $_SESSION['cmsgo_version_check'];
    }
    ...
    $identify  = '?version='.rawurlencode(CMSGO_VERSION.' '.str_replace('/', '', CMSGO_RELEASE_DATE));
    ...
    if(function_exists('fsockopen')) {
        $has_sockets = true;
        if($fsock = @fsockopen('ssl://www.pixels-points.ch', 443, $errno, $errstr, 10)) {
    ...
    if(!$get_info && function_exists('file_get_contents') && ini_get('allow_url_fopen')) {
        ...
        $context = stream_context_create([
            'http' => [
                'ignore_errors' => true,
                'timeout' => 10,
                'header' => 'User-Agent: cmsGO/' . CMSGO_VERSION . "\r\n"
            ]
        ]);
        $version_info = @file_get_contents('https://www.pixels-points.ch/versioncheck/'.$identify, false, $context);
    ...
    if($get_info && preg_match('/.*BEGIN -->(.+)<!-- END.*/s', $version_info, $match)) {
        ...
        if($latest_revision > $cmsgo['revision']) {
            $mark_rev_prefix = '<span class="error">';
            $mark_rev_suffix = '</span>';
        } elseif($latest_revision < $cmsgo['revision']) {
        ...
        $version_info .= sprintf($BL['Current_version_info'], $mark_rev_prefix.CMSGO_VERSION.$mark_rev_suffix.' ('.$mark_date_prefix.CMSGO_RELEASE_DATE.$mark_date_suffix.', r'.CMSGO_REVISION.')') . '</p>';
    ...
    $_SESSION['cmsgo_version_check'] = '<div class="versioncheck"><h1>'.$BL['Version_information'].'</h1> '.$version_info.'</div>';
...
function countNewsletterRecipients($target) {
    ...
    $recipients = _dbQuery('SELECT * FROM '.DB_PREPEND.'cmsgo_address WHERE address_verified=1');
    $counter    = 0;
...
function getContentPartOptionTag($value='', $text='', $selected='', $module='') {
    ...
    if($value == 30) {
        ...
        foreach($GLOBALS['cmsgo']['modules'] as $module_value) {
...
function isContentPartSet($value='') {
    ...
    } elseif(       isset($GLOBALS['cmsgo']['modules'][$value[1]])
                &&  $GLOBALS['cmsgo']['modules'][$value[1]]['cntp']
                &&  file_exists($GLOBALS['cmsgo']['modules'][$value[1]]['path'].'inc/cnt.list.php')) {
...
function proof_alias($current_id, $alias='', $mode='CATEGORY') {
    ...
    if($alias === '') {
        ...
        if(!empty($GLOBALS['cmsgo']['allow_empty_alias'])) {
            return '';
    ...
    if(CMSGO_ALIAS_WSLASH && strpos($alias, '/') !== false) {

        $root_folders = returnSubdirListAsArray(CMSGO_ROOT);
    ...
    while(is_file(CMSGO_ROOT.'/'.$alias.$alias_proof_suffix.CMSGO_REWRITE_EXT)) {
        $alias_proof_suffix_count++;
        $alias_proof_suffix = '-'.$alias_proof_suffix_count;
    }
    ...
    // new reserved alias can be defined in $cmsgo['reserved_alias']
    if(isset($cmsgo['reserved_alias']) && is_array($cmsgo['reserved_alias']) && count($cmsgo['reserved_alias'])) {
    ...
    if( $acat_count > 0 || $article_count > 0 || $content_count > 0 ) {
        ...
        $sql  = "SELECT acat_alias FROM ".DB_PREPEND."cmsgo_articlecat WHERE ";
        $sql .= $where_acat;
        $sql .= "acat_alias LIKE "._dbEscape($alias, true, '', '%');
        $all_acat_alias = _dbQuery($sql);

        $sql  = "SELECT article_alias FROM ".DB_PREPEND."cmsgo_article WHERE ";
        $sql .= $where_article;
        $sql .= "article_alias LIKE "._dbEscape($alias, true, '', '%');
        $all_article_alias = _dbQuery($sql);

        $sql  = "SELECT cnt_alias FROM ".DB_PREPEND."cmsgo_content WHERE ";
        $sql .= $where_content;
...
function _dbSaveCategories($categories=array(), $type='', $pid=0, $seperator=',') {
    ...
    if($type && $pid) {
        ...
        $sql = 'DELETE FROM '.DB_PREPEND.'cmsgo_categories WHERE cat_pid='.$pid." AND cat_type="._dbEscape( $type );
        _dbQuery($sql, 'DELETE');
    ...
    if(is_array($categories) && count($categories) && $type && $pid) {
        ...
        foreach($categories as $value) {
            ...
            if($value != '') {
                ...
                $data['cat_name'] = $value;
                _dbInsert('cmsgo_categories', $data);
...
function setItemsPerPage($default=25) {
    if( isset($_GET['showipp']) ) {
        ...
        $ipp = intval( $_COOKIE['cmsgoBEItemsPerPage'] );
    } else {
        $ipp = $default;
    }
        ...
        default:
            ...
            if(is_array($more) && count($more)) {
                ...
                foreach($more as $item) {
                    ...
                    if(empty($GLOBALS['BE']['HEADER'][$name]) && is_file(CMSGO_TEMPLATE.'lib/mootools/more/'.$item.'.js')) {
                        $GLOBALS['BE']['HEADER'][$name] = getJavaScriptSourceLink(TEMPLATE_PATH.'lib/mootools/more/'.$item.'.js');
                    }
                }
            }
    }
    $GLOBALS['cmsgo']['mootools_mode'] = $mode;
}
...
function cmsgo_revision_check($revision) {
    ...
    $revision_temp = cmsgo_revision_check_temp($revision);
    ...
    $revision_function = 'cmsgo_revision_r'.$revision;
    if(function_exists($revision_function) && empty($GLOBALS['cmsgo']['check_r'.$revision])) {
        $GLOBALS['cmsgo']['revision_return'] = '';
        if( call_user_func($revision_function) !== false ) {
            $GLOBALS['cmsgo']['check_r'.$revision] = true;
            $cmsgo_revision_return = empty($GLOBALS['cmsgo']['revision_return']) ? '' : "\n\nReturn:\n-------\n".strval($GLOBALS['cmsgo']['revision_return']);
            @write_textfile(CMSGO_TEMP.'r'.$revision.'.checked.tmp', date('Y-d-m H:i:s').$cmsgo_revision_return);
            return true;
        } else {
            return false;
        }
...
function cmsgo_revision_check_temp($revision) {
    if(empty($revision) || !preg_match('/^\d+$/', $revision)) {
        return NULL;
    }
    return is_file(CMSGO_TEMP.'r'.$revision.'.checked.tmp');
}
...
function dir_menu($pid, $zid, $vor, $userID, $vorzeichen = ":") {
    $pid  = intval($pid);
    $sql  = "SELECT f_id, f_name, f_uid, usr_login FROM ".DB_PREPEND."cmsgo_file f ";
...
function get_struct_alias($start_id=0, $parent_alias=false) {
    ...
    $sql .= 'FROM '.DB_PREPEND.'cmsgo_articlecat WHERE acat_trash=0 ';
    if($parent_alias) {
    ...
    return implode($GLOBALS['cmsgo']['alias_allow_slash'] ? '/' : '-', array_reverse($data));
}
...
function render_iptc_fileinfo($iptc_data) {
    ...
    $iptc_rules = $GLOBALS['cmsgo']['iptc_rules'];
    $iptc_keys = $GLOBALS['cmsgo']['iptc_keys'];
    $fileinfo = array(
        'title' => '',
        'longinfo' => '',
        'copyright' => '',
    ...
    if(is_array($iptc_data) && count($iptc_data)) {
        ...
        foreach($iptc_data as $iptc_key => $iptc_value) {
            ...
            if(empty($iptc_value)) {
                ...
                if(empty($GLOBALS['cmsgo']['iptc_rules_multiple'])) {
                    $iptc_value = $iptc_value[0];
                } else {
                    $iptc_value = implode($GLOBALS['cmsgo']['iptc_separator'], $iptc_value);
    }
