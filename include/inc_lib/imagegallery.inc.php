 ...
 * cmsgoImageGallery
 *
 * A file center driven gallery for cmsgo
 *
 */

class cmsgoImageGallery {
   ...
   */
  function cmsgoImageGallery() {

    $this->_getGallerySortString();

    if($this->alias === '') {
      global $aktion;
      $this->alias = defined('CMSGO_ALIAS') ? CMSGO_ALIAS : ( $aktion[1] == 0 ? 'id='.$aktion[0] : 'aid='.$aktion[1] );
    }

    $this->setAlias( $this->alias );

    $this->image_src = CMSGO_URL.CMSGO_RESIZE_IMAGE.$this->query_separator;

  }

  function setQuerySeparator($query_separator) {

    $this->image_src = CMSGO_URL.CMSGO_RESIZE_IMAGE.$query_separator;
   ...
   * get images
   *
   * Dive into cmsgo's db file storage and get all
  ...
  function getImages($folder=0) {
    ...
    $sql  = 'SELECT COUNT(*) FROM '.DB_PREPEND.'cmsgo_file WHERE ';
    ...
    if(_dbCount($sql) > 0) {
      ...
      $sql  = 'SELECT * FROM '.DB_PREPEND.'cmsgo_file WHERE ';
  ...
  function getGalleryTree() {
    ...
    $sql  = 'SELECT * FROM '.DB_PREPEND.'cmsgo_file ';
