...
function backend_list_keywords() {
  ...
  $list   .= LF . '<table cellspacing="0" cellpadding="0" border="0" class="table listingTable">' . LF;
  $list   .= '  <tr>' . LF;
  ...
  $sql     = "SELECT * FROM ".DB_PREPEND."cmsgo_keyword WHERE keyword_trash=0 ORDER BY keyword_name";
...
function backend_edit_keywords() {
  ...
  if(isset($_POST['send_update'])) {
    ...
    if(empty($update['keyword_name'])) {
      ...
      $sql   = "UPDATE ".DB_PREPEND."cmsgo_keyword SET ";
  ...
  // INSERT keyword
    ...
    if(empty($insert['keyword_name'])) {
      ...
      $sql     = "SELECT * FROM ".DB_PREPEND."cmsgo_keyword ";
      $sql  .= "WHERE keyword_trash=0 AND keyword_name=" . _dbEscape($insert['keyword_name']);
      $check   = _dbQuery($sql);

      if(empty($check[0])) {

        $sql  = "INSERT INTO ".DB_PREPEND."cmsgo_keyword SET ";
  ...
  $sql     = "SELECT * FROM ".DB_PREPEND."cmsgo_keyword WHERE keyword_trash=0 AND keyword_id=" . $keyword_id." LIMIT 1";
