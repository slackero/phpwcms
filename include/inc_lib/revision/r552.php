...
function cmsgo_revision_r552() {

	$status = true;

	// do former revision check – fallback to r551
	if(cmsgo_revision_check_temp('551') !== true) {
		$status = cmsgo_revision_check('551');
	}

    // Try to fix SVG again
    // Search existing SVG files and try to set width and height
    $result = _dbGet('cmsgo_file', 'f_id,f_name,f_hash,f_ext', "f_trash=0 AND f_kid=1 AND f_ext='svg' AND f_image_width='' AND f_image_height=''");
