...
function cmsgo_revision_r497() {
	...
	if(isset($result[0]['Type']) && substr(strtolower($result[0]['Type']), 0, 3) == 'int') {
		...
		// Drop index first
		_dbQuery("ALTER TABLE ".DB_PREPEND. 'cmsgo_crossreference DROP INDEX cref_type', 'ALTER');
		...
		// Add new index
		_dbQuery("ALTER TABLE ".DB_PREPEND. 'cmsgo_crossreference ADD INDEX (cref_type)', 'ALTER');
		_dbQuery("ALTER TABLE ".DB_PREPEND. 'cmsgo_crossreference ADD INDEX (cref_int)', 'ALTER');
		_dbQuery("ALTER TABLE ".DB_PREPEND. 'cmsgo_crossreference ADD INDEX (cref_rid)', 'ALTER');
		_dbQuery("ALTER TABLE ".DB_PREPEND. 'cmsgo_crossreference ADD INDEX (cref_str)', 'ALTER');
	...
	if(empty($result)) {
		_dbQuery("ALTER TABLE ".DB_PREPEND."cmsgo_articlecat ADD acat_lang VARCHAR(255) NOT NULL DEFAULT ''", 'ALTER');
	...
	if(empty($result)) {
		_dbQuery("ALTER TABLE ".DB_PREPEND."cmsgo_article ADD article_lang VARCHAR(255) NOT NULL DEFAULT ''", 'ALTER');
	...
	if(empty($result)) {
		_dbQuery("ALTER TABLE ".DB_PREPEND."cmsgo_articlecontent ADD acontent_lang VARCHAR(255) NOT NULL DEFAULT ''", 'ALTER');
