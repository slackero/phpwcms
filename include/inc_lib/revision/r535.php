...
function cmsgo_revision_r535() {

	$status = true;

	// do former revision check – fallback to r534
	if(cmsgo_revision_check_temp('534') !== true) {
		$status = cmsgo_revision_check('534');
	}

	// change type of some content related fields from TEXT to MEDIUMTEXT

	// Retrieve Type of profession name
	$result = _dbQuery("SHOW COLUMNS FROM `".DB_PREPEND."cmsgo_profession` WHERE Field='prof_name'");
	if(isset($result[0]['Type']) && strpos($result[0]['Type'], '100')) {
		$update = _dbQuery("ALTER TABLE `".DB_PREPEND."cmsgo_profession` CHANGE `prof_name` `prof_name` VARCHAR(255) NOT NULL DEFAULT ''", 'ALTER');
		if(!$update) {
			$status = false;
		}
	}
	...
	_dbUpdate('cmsgo_profession', array('prof_name'=>'n/a'), "prof_name=' n/a'");

	// Import new professions
	$result = _dbCount("SELECT COUNT(*) FROM `".DB_PREPEND."cmsgo_profession`");
	if($result < 25) {
		$jobs = array(
			'academic',
			'accountant',
		...
		foreach($jobs as $job) {
			$sql = 'INSERT IGNORE INTO `'.DB_PREPEND.'cmsgo_profession` (prof_name) VALUES('._dbEscape($job).')';
			_dbQuery($sql, 'INSERT');
		}
	}
