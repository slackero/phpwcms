...
if(isset($_POST["newsletter_id"])) {
    ...
    if($newsletter["newsletter_id"]) {
        $query_mode = 'UPDATE';
        $sql  = "UPDATE ".DB_PREPEND."cmsgo_newsletter SET ".$sql;
        $sql .= "WHERE newsletter_id=".$newsletter["newsletter_id"]." LIMIT 1";
    } else {
        $query_mode = 'INSERT';
        $sql  = "INSERT INTO ".DB_PREPEND."cmsgo_newsletter SET newsletter_created=NOW(), ".$sql;
    }
    ...
    if(!isset($newsletter['error'])) {
        ...
        if($newsletter['newsletter_active']) {
            ...
            if($recipients = _dbQuery('SELECT * FROM '.DB_PREPEND.'cmsgo_address WHERE address_verified=1')) {
            ...
            $sql  = 'DELETE FROM '.DB_PREPEND.'cmsgo_newsletterqueue ';
            $sql .= 'WHERE queue_pid='.$newsletter["newsletter_id"].' AND queue_status=0';
            _dbQuery($sql, 'DELETE');
...
// read the given subscription datas from db
    ...
    $sql .= DB_PREPEND."cmsgo_newsletter WHERE newsletter_id=".$newsletter["newsletter_id"]." LIMIT 1";
    $result = _dbQuery($sql);
