...
class Phpwcms_Image_lib {
    ...
    function image_process_gd( $action = 'resize' ) {
        ...
        if ( $this->image_type === 1 && ( $transparent_index = imagecolorallocatealpha( $dst_img, 255, 255, 255,
                127 ) ) ) // gif preserve transparency
        {
    ...
    function image_process_imagemagick( $action = 'resize' ) {
        ...
        if ( $this->target_ext == 'jpg' ) {
            ...
            if ( ( $gif_content = @file_get_contents( $this->full_src_path ) ) && preg_match( '/\x00\x21\xF9\x04.{4}\x00(\x2C|\x21)/s',
                    $gif_content ) ) {
    ...
    function display_errors( $open = '<p>', $close = '</p>', $wrap_open = '', $wrap_close = '' ) {
        return ( count( $this->error_msg ) > 0 ) ? $wrap_open . $open . implode( $close . $open,
                $this->error_msg ) . $close . $wrap_close : '';
    }
