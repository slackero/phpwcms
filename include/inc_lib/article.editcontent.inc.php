...
if((isset($_GET["s"]) && intval($_GET["s"]) == 1) || isset($_GET['struct'])) { //Show single article information
    ...
    if((!isset($_POST["article_update"]) || !intval($_POST["article_update"])) && !isset($_GET['struct'])) {
        ...
        $sql .= "FROM ".DB_PREPEND."cmsgo_article LEFT JOIN ".DB_PREPEND."cmsgo_articlecat ON ";
        $sql .= DB_PREPEND."cmsgo_article.article_cid=".DB_PREPEND."cmsgo_articlecat.acat_id WHERE ";
        $sql .= DB_PREPEND."cmsgo_article.article_id='".$article["article_id"]."' ";
        $sql .= $_SESSION["wcs_user_admin"] ? '' : 'AND '.DB_PREPEND.'cmsgo_article.article_uid='._dbEscape($_SESSION["wcs_user_id"]).' ';
        $sql .= "LIMIT 1";
    ...
    // add new article inside structure
        ...
        $article["article_aktiv"]               = $cmsgo['set_article_active'];
        $article["article_begin"]               = '';
        $article["article_end"]                 = '';
        ...
        $article['article_opengraph']           = empty($cmsgo['set_sociallink']['article']) ? 0 : 1;
        ...
        if($set_begin && $article["article_begin"]) {
            $article["article_begin"] = cmsgo_strtotime($article["article_begin"]);
        ...
        if((!RESPONSIVE_MODE && $article['image']['width'] > $cmsgo["content_width"]) || $article['image']['width'] === '') {
            $article['image']['width'] = $cmsgo["content_width"];
        }

        if((!RESPONSIVE_MODE && $article['image']['list_width'] > $cmsgo["content_width"]) || $article['image']['list_width'] == '') {
            $article['image']['list_width'] = $cmsgo["content_width"];
        }

        if($article['image']['id']) {
            // check for image information and get alle infos from file
            $img_sql  = "SELECT * FROM " . DB_PREPEND . "cmsgo_file WHERE f_id=";
        ...
        if(!count($article_err)) {
            ...
            if(empty($article["article_id"])) {
                ...
                $sql =  "UPDATE ".DB_PREPEND."cmsgo_article SET ".
                        "article_cid=".$article["article_catid"].",".
    ...
    if(!isset($article["acat_overwrite"])) {
        ...
        if($article['article_catid']) {
            $article["acat_overwrite"] = _dbGet('cmsgo_articlecat', 'acat_overwrite', 'acat_trash != 9 AND acat_id = '.$article['article_catid'], '', '', 1);
    ...
    if($article["acat_overwrite"] && is_file(CMSGO_TEMPLATE.'inc_settings/template_default/'.$article["acat_overwrite"])) {
        @include CMSGO_TEMPLATE.'inc_settings/template_default/'.$article["acat_overwrite"];
    }

    // list mode
    if( (!isset($_GET["aktion"]) || !intval($_GET["aktion"])) && !isset($_GET['struct'])) {

        include_once CMSGO_ROOT."/include/inc_tmpl/articlecontent.list.tmpl.php";
    ...
    // edit article summary
        ...
        if(isset($_GET["acid"]) && intval($_GET["acid"])) {
            ...
            $sql =  "SELECT * FROM ".DB_PREPEND."cmsgo_articlecontent WHERE acontent_id=".$content["id"]." AND acontent_aid=".$content["aid"]." LIMIT 1";
            ...
            if(isset($result[0]['acontent_id'])) {
                ...
                if($content["type"] != 30 && is_file(CMSGO_ROOT.'/include/inc_lib/content/cnt'.$content["type"].'.takeval.inc.php')) {
                    ...
                    include CMSGO_ROOT.'/include/inc_lib/content/cnt'.$content["type"].'.takeval.inc.php';

                } elseif($content["type"] == 30 && is_file($cmsgo['modules'][$content['module']]['path'].'inc/cnt.read.php')) {

                    $content['comment'] = $row["acontent_comment"];

                    // load module data
                    include $cmsgo['modules'][$content['module']]['path'].'inc/cnt.read.php';

                } else {

                    include CMSGO_ROOT.'/include/inc_lib/content/cnt0.takeval.inc.php';
        ...
        if(isset($_POST["caktion"]) && intval($_POST["caktion"])) {
            ...
            include_once CMSGO_ROOT."/include/inc_lib/article.readform.inc.php"; //get posted values from form
            ...
            if(!isset($content["error"])) { //if no error
                ...
                if($content['type'] != 30 && file_exists(CMSGO_ROOT.'/include/inc_lib/content/cnt'.$content['type'].'.sql.inc.php')) {

                    include CMSGO_ROOT.'/include/inc_lib/content/cnt'.$content['type'].'.sql.inc.php';

                } elseif($content['type'] == 30 && file_exists($cmsgo['modules'][$content['module']]['path'].'inc/cnt.sql.php')) {

                    include $cmsgo['modules'][$content['module']]['path'].'inc/cnt.sql.php';

                } else {

                    include CMSGO_ROOT.'/include/inc_lib/content/cnt0.sql.inc.php';
                ...
                if(!$content["id"]) { //if new content part should be created
                    ...
                    $SQL  = "INSERT INTO ".DB_PREPEND."cmsgo_articlecontent SET acontent_created=NOW(), " . $SQL;
                    ...
                    $SQL  = "UPDATE ".DB_PREPEND."cmsgo_articlecontent SET " . $SQL;
                    ...
                    if(isset($result['AFFECTED_ROWS'])) {
                        ...
                        if($content["update_type"]) { //If content part type was changed
                            $sql  = "UPDATE ".DB_PREPEND."cmsgo_articlecontent SET";
        ...
        include CMSGO_ROOT."/include/inc_tmpl/articlecontent.edit.tmpl.php";
