...
function struct_articlelist($struct_id, $counter, $copy_article_content, $cut_article_content, $copy_article, $cut_article, $article_order=0) {
    ...
    foreach($article as $akey => $avalue) {
        ...
        //copy article
        $a .= '<a class="dropdown-item" href="cmsgo.php?do=articles&amp;acopy='.$article[$akey]["article_id"].'"><i class="fa fa-copy fa-fw" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_copy'].' ['.$at.'] </a>';
        //cut article
        $a .= '<a class="dropdown-item" href="cmsgo.php?do=articles&amp;acut='.$article[$akey]["article_id"].'"><i class="fa fa-cut fa-fw disabled" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_cut'].' ['.$at.'] </a>';
        //sort article up
        if($sort_up) {
        ...
        //sort article up
        if($sort_down) {
          $a .= '<a class="dropdown-item" href="include/inc_act/act_structure.php?do=4%7C'.$article[$akey]["article_id"] . '%7C' . $article[$akey]['sort_down'] . '%7C'.$article[$akey+1]["article_id"] . '%7C' . $article[$akey]['article_sort'].'"><i class="fa fa-caret-down fa-fw" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_sort_down'].'</a>';
        }
        //delete article
        if($article[$akey]["article_uid"] == $_SESSION["wcs_user_id"] || $_SESSION["wcs_user_admin"]) {
...
function struct_articlecontentlist($article, $akey, $copy_article_content, $cut_article_content, $counter, $sbutton_string){
    ...
    if(isset($result[0]['acontent_id'])) {
        ...
        foreach($result as $article_content) {
            ...
            if(!isset($GLOBALS['wcs_content_type'][ $article_content["acontent_type"] ]) || ($article_content['acontent_type'] == 30 && !isset($GLOBALS['cmsgo']['modules'][$article_content["acontent_module"]]))) {
                continue;
            }
            ...
            if($copy_article_content) {
                if($copy_article_content != $article_content["acontent_id"]) {
                    $a .= '<a class="btn btn-xs btn-warning mr-1" href="include/inc_act/act_structure.php?do='.rawurlencode('8|'.$copy_article_content.'|'.$article_content["acontent_aid"].'|'.$article_content["acontent_sorting"]);
                    $a .= '" data-toggle="tooltip" title="'.$GLOBALS['BL']['be_func_content_paste'];
                    $a .= " [".$at."]\"><i class=\"fa fa-arrow-down\" aria-hidden=\"true\"></i></a>";
                } else {
                    $a .= "<a class=\"btn btn-xs btn-danger mr-1\" href=\"cmsgo.php?do=articles\" title=\"";
...
function listmode_edits($listmode, $struct, $key, $an, $copy_article_content, $cut_article_content, $copy_article, $copy_id, $cut_article, $cut_id, $forbid_cut, $forbid_copy, $count_row, $child_sort) {
    ...
    case 0:     //cut article
                if($cut_article) {
                    $a .= '<a class="btn btn-xs btn-warning mr-1" href="include/inc_act/act_structure.php?do=3'.'%7C'.$cut_article.'%7C';
                    $a .= $struct[$key]["acat_id"].'" data-toggle="tooltip" title="'.$GLOBALS['BL']['be_func_struct_paste_article']." \n[".$an;
                    $a .= "]\"><i class=\"fa fa-arrow-down\" aria-hidden=\"true\"></i></a>";
                }
                //copy article
                if($copy_article) {
                    $a .= '<a class="btn btn-xs btn-warning mr-1" href="include/inc_act/act_structure.php?do=5'.'%7C'.$copy_article.'%7C';
                    $a .= $struct[$key]["acat_id"].'" data-toggle="tooltip" title="'.$GLOBALS['BL']['be_func_struct_paste_article']." \n[".$an;
                    $a .= "]\"><i class=\"fa fa-arrow-down\" aria-hidden=\"true\"></i></a>";
                }
                //cut structure
                if($cut_id) {
                    if($cut_id != $struct[$key]["acat_id"] && !$forbid_cut) {
                        $a .= '<a class="btn btn-xs btn-warning mr-1" href="include/inc_act/act_structure.php?do=1'.'%7C'.$cut_id.'%7C'.$struct[$key]["acat_id"].'%7C';
                        $a .= $child_sort.'" data-toggle="tooltip" title="'.$GLOBALS['BL']['be_func_struct_paste_level'];
                ...
                if($copy_id) {
                    if($copy_id != $struct[$key]["acat_id"] && !$forbid_copy) {
                        $a .= '<a class="btn btn-xs btn-warning mr-1" href="include/inc_act/act_structure.php?do=6'.'%7C'.$copy_id.'%7C'.$struct[$key]["acat_id"].'%7C';
                        $a .= $child_sort.'" data-toggle="tooltip" title="'.$GLOBALS['BL']['be_func_struct_paste_level'];
                        $a .= " [".$an."]\"><i class=\"fa fa-arrow-down\" aria-hidden=\"true\"></i></a>";
                ...
                $a .= '<a class="btn btn-xs btn-blue" role="button" type="button" aria-disabled="true" title="'.$GLOBALS['BL']['be_func_struct_sedit'].' ['.$an.']" data-toggle="tooltip" href="cmsgo.php?do=articles&amp;p=6&amp;struct=';
                ...
                $a .= '<a class="dropdown-item" href="cmsgo.php?do=articles&amp;p=6&amp;struct='.$struct[$key]["acat_id"].'&amp;sort='.$child_sort.'" data-toggle="tooltip" title="'.$GLOBALS['BL']['be_func_struct_insert_level'].' ['.$an.']"><i class="fa fa-plus fa-fw" aria-hidden="true"></i> <i class="fa fa-folder fa-fw" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_insert_level'].' ['.$an.']</a>';
                 $a .= '<a class="dropdown-item" href="cmsgo.php?do=articles&amp;p=1&amp;struct='.$struct[$key]["acat_id"].'" data-toggle="tooltip" title="'.$GLOBALS['BL']['be_func_struct_new_article'].'"><i class="fa fa-plus fa-fw" aria-hidden="true"></i> <i class="fa fa-file fa-fw" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_new_article'].'</a>';
                //copy structur
                if($struct[$key]["acat_id"]) {
                  $a .= '<a class="dropdown-item" href="cmsgo.php?do=articles&amp;cop='.$struct[$key]["acat_id"].'"><i class="fa fa-copy fa-fw" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_copy_level'].' ['.$an.'] </a>';
                }
                //cut structur
                if($struct[$key]["acat_id"]) {
                  $a .= '<a class="dropdown-item" href="cmsgo.php?do=articles&amp;cut='.$struct[$key]["acat_id"].'"><i class="fa fa-cut disabled fa-fw" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_cut_level'].' ['.$an.'] </a>';
                }
                //sort structur up
                if($sort_up) {
                ...
                '%7C'.(($key+1)*10).'"><i class="fa fa-caret-up fa-fw" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_sort_up'].'</a>';
                }
                //sort structur down
                if($sort_down) {
                ...
                '%7C'.(($key+1)*10).'"><i class="fa fa-caret-down fa-fw" aria-hidden="true"></i> '.$GLOBALS['BL']['be_func_struct_sort_down'].'</a>';
                }
                //delete structur
                if($struct[$key]["acat_id"]) {
    ...
    if(count($data['error'])) {
        ...
        if($rid) {
            ...
            if(isset($_POST['delete_'.md5($rid)])) {
                $data['data']['active'] = 9;
                $result = _dbQuery('DELETE FROM '.DB_PREPEND.'cmsgo_redirect WHERE rid='.$rid, 'DELETE');
            } else {
                $result = _dbUpdate('cmsgo_redirect', $data['data'], 'rid='.$rid);
            }
        } else {
            $result = _dbInsert('cmsgo_redirect', $data['data']);
