...
//if(isset($_POST['cmap_location_x']) && isset($_POST['cmap_location_edited']) && intval($_POST['cmap_location_edited']) && $content['id']) {
    ...
    if(!$content["location"]['title']) {
        ...
        if(!$content["location"]['id']) {
            // INSERT
            $query_mode = 'INSERT';
            $content["location"]['sql']  = "INSERT INTO ".DB_PREPEND."cmsgo_map SET ".$content["location"]['sql'];
        } else {
            // UPDATE
            $query_mode = 'UPDATE';
            $content["location"]['sql']  = "UPDATE ".DB_PREPEND."cmsgo_map SET ".$content["location"]['sql']." ";
