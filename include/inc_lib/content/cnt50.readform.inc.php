...
if(is_array($content['reference']["list"]) && count($content['reference']["list"])) {
    ...
    $img_sql = "SELECT * FROM ".DB_PREPEND."cmsgo_file WHERE (";
    $imgx = 0;
    ...
    if(isset($img_result[0]['f_id'])) {
        ...
        if((!RESPONSIVE_MODE && $content['reference']["width"] > $cmsgo["content_width"]) || $content['reference']["width"] == '') {
            $content['reference']["width"] = $cmsgo["content_width"];
        }
    ...
    if($temp_list_count > 1) {
        ...
        if(!$content['reference']["blockwidth"]) {
            $content['reference']["blockwidth"]  = $cmsgo["content_width"];
        }
        if(!$content['reference']["blockheight"]) {
            $content['reference']["blockheight"] = $cmsgo["img_prev_height"];
        }
