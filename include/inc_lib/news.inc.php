    ...
    public function listBackend() {
        ...
        if(count($this->news)) {
            ...
            foreach($this->news as $news) {
                ...
                $list[] = '<td class="column collast text-nowrap text-end">
                    <div class="btn-group btn-group-sm" role="group" aria-label="news-actions-'.$news['cnt_id'].'">
                    <button id="abtncontent'.$news["cnt_id"].'" class="btn fa btn-sm visible '.($news["cnt_status"]==0 ? "btn-danger" : "btn-success").'" data-id="'.$news["cnt_id"].'" data-type="content" data-table="content" data-field="cnt_status" data-fieldid="cnt_id" data-bs-toggle="tooltip" title="aktivieren/deaktivieren"><i class="fa-solid '.($news["cnt_status"]==0 ? "fa-eye-slash" : "fa-eye").' fa-fw" aria-hidden="true"></i></button>
                    ...
                    '<a class="btn btn-sm btn-danger ms-1" href="'.$this->base_url.'&amp;cntid='.$news['cnt_id'].'&amp;status=9" data-bs-toggle="tooltip" title="'.$this->BL['be_delete_dataset'].' '.html($news['cnt_name']).'" data-confirm-type="danger" data-confirm-action="'.$this->BL['be_delete_dataset'].'" data-confirm="'.$this->BL['be_delete_dataset'].' \\n'.js_singlequote($news['cnt_name']).'">
