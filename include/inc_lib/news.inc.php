...
class cmsgoNews {
    ...
    public function getNews() {
        ...
        $sql  = 'SELECT '.$this->select.' FROM '.DB_PREPEND.'cmsgo_content WHERE ';
        $sql .= "cnt_module='news'";
    ...
    public function filter() {
        ...
        if($keyword) {
            ...
            $this->where['cat'] .=  'FROM '.DB_PREPEND.'cmsgo_categories pcat WHERE ';
    ...
    public function getNewsCategories() {
        ...
        $where = '(SELECT COUNT(*) FROM '.DB_PREPEND."cmsgo_content WHERE  cnt_id=cat_pid AND cnt_status != 9 AND cnt_module='news')";
        $result = _dbGet('cmsgo_categories', 'cat_name', "cat_type='news' AND ".$where, 'cat_name', 'cat_name');
        $categories = array();
    ...
    public function edit() {
        ...
        if(isset($_GET['status'])) {
            ...
            switch($status) {
                ...
                case 1:
                case 9:
                    _dbUpdate('cmsgo_content', array('cnt_status'=>$status), 'cnt_id='.$this->newsId);
                    set_status_message(
        ...
        $this->data = array(
            ...
            'cnt_category'          => '',
            'cnt_sort'              => 0,
            'cnt_opengraph'         => empty($this->cmsgo['set_sociallink']['news']) ? 0 : 1,
        ...
        if( isset($_POST['cnt_name']) ) {
            ...
            if($post_error === false) {
                ...
                if($this->newsId == 0) {
                    ...
                    $result = _dbInsert('cmsgo_content', $values);
                ...
                // update existing dataset
                    ...
                    $result = _dbUpdate('cmsgo_content', $values, 'cnt_id='.$this->newsId);
                    if($result != false) {
                        $success = true;
    ...
    public function getPostData() {
        ...
        if(empty($this->cmsgo['allow_empty_alias']) && $post['cnt_alias'] == '') {
