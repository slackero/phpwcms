...
function change_articledate($article_id=0) {
	...
	if(($article_id = intval($article_id))) {
		$sql = "UPDATE ".DB_PREPEND."cmsgo_article SET article_tstamp=NOW() WHERE article_id='".$article_id."'";
		_dbQuery($sql, 'UPDATE');
	}
}
...
function struct_select_list($counter, $struct_id, $selected_id, $add_alias=false) {
	...
	$struct = _dbGet('cmsgo_articlecat', 'acat_id,acat_name,acat_alias,acat_aktiv,acat_regonly', 'acat_trash=0 AND acat_struct='.$struct_id);
...
function struct_checkbox_list($counter=0, $struct_id=0, $selected_id=array(), $add_alias=false, $checkbox_name='structlist') {
	...
	$struct = _dbGet('cmsgo_articlecat', 'acat_id,acat_name,acat_alias,acat_aktiv,acat_regonly', 'acat_trash=0 AND acat_struct='.intval($struct_id));
