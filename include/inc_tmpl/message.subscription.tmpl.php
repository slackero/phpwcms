  ...
  <div class="card-header"><h2><i class="fa fa-list" aria-hidden="true"></i> <?php echo $BL['be_cnt_title_overview'] ?> <?php echo $BL['be_newsletter_title'] ?></h2></div>
    ...
    if(isset($result[0]['subscription_id'])) {
        ...
        foreach($result as $row) {
            ...
            echo '<a class="btn btn-sm btn-blue" role="button" aria-disabled="true" title="'.$BL['be_tt_edit'].'" data-toggle="tooltip" href="phpwcms.php?do=messages&amp;p=2&amp;s='.$row["subscription_id"].'&amp;edit=1"><i class="fa fa-pencil-alt"></i></a>';
            ...
            echo '</div>';
            if ($subscribers>0) {
              echo '<div class="btn btn-sm btn-danger disabled ml-1" role="button" aria-disabled="true" title="'.$BL['be_mailinglist_cannotdelete_list'].': '.html_specialchars($row["subscription_name"]).'" data-toggle="tooltip" href="#"><i class="far fa-trash-alt"></i></div>';
            } else {
              echo '<a class="btn btn-sm btn-danger ml-1" role="button" aria-disabled="true" title="'.$BL['be_mailinglist_delete_list'].': '.html_specialchars($row["subscription_name"]).'" data-toggle="tooltip" href="phpwcms.php?do=messages&amp;p=2&amp;s='.$row["subscription_id"].'&amp;del='.$row["subscription_id"]. '" onclick="return confirm(\''.$BL['be_mailinglist_delete_list'].' '.js_singlequote($row["subscription_name"]).'\');"><i class="far fa-trash-alt"></i></a>';
            }
