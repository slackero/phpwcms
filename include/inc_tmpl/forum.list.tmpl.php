...
$sql = "SELECT * FROM ".DB_PREPEND."cmsgo_forum WHERE forum_entry=0 AND forum_deleted=0 ORDER BY forum_changed DESC";
$result = _dbQuery($sql);
...
?>
    ...
    <tr><td colspan="3"><form action="cmsgo.php?do=messages&amp;p=6&amp;s=0" method="post"><input name="addforum" type="submit" class="button" value="<?php echo $BL['be_forum_add'] ?>"></form></td></tr>
</table>
<?php
...
// should the edit forum dialog
    ...
    // read the create or edit forum form data
        ...
        if($forum["id"]) {
            ...
            $query_mode = 'UPDATE';
            $sql  = "UPDATE ".DB_PREPEND."cmsgo_forum SET " . $sqla;
            ...
            $query_mode = 'INSERT';
            $sql  = "INSERT INTO ".DB_PREPEND."cmsgo_forum SET ";
            $sql .= "forum_entry='0', ";
    ...
    // read the given subscription datas from db
        $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_forum WHERE forum_id=".$forum["id"]." LIMIT 1";
        $result = _dbQuery($sql);
...
<table width="538" border="0" cellpadding="0" cellspacing="0" summary="">
    ...
    <tr>
        ...
        <td><input name="Submit" type="submit" class="button" value="<?php echo $BL['be_save_btn'] ?>">&nbsp;&nbsp;<input type="button" class="button" value="<?php echo $BL['be_newsletter_button_cancel'] ?>" onclick="location.href='cmsgo.php?do=messages&p=6';"></td>
    </tr>
    <tr><td colspan="2"><img src="img/leer.gif" alt="" width="1" height="15"></td></tr>
</table>
</form>
