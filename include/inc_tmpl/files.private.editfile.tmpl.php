...
if(isset($_POST["file_aktion"]) && intval($_POST["file_aktion"]) === 2) {
    ...
    if ($file_alias != '') {
      ...
      $f_count  = "SELECT COUNT(f_alias) FROM ".DB_PREPEND."cmsgo_file WHERE ";
    ...
    if ($file_alias != $file_alias_old && $file_alias_old != '') {
      $files = glob(CMSGO_ROOT.'/content/images/*-'.$file_alias_old.'.'.$file_ext);
    ...
    if(empty($file_error)) {
        $sql =  "UPDATE ".DB_PREPEND."cmsgo_file SET ".
...
if($file_id) {
    $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_file WHERE f_id=".$file_id;
    if(empty($_SESSION["wcs_user_admin"])) {
        $sql .= " AND f_uid=".intval($_SESSION["wcs_user_id"]);
    }
        ...
        if(isset($row["f_hash"])) {
            ...
            if(empty($row["f_svg"])) {
                ...
                $thumb_image = get_cached_image(array(
                    ...
                    "thumb_name"    => md5($row["f_hash"].'420420'.$cmsgo["sharpen_level"].$cmsgo['colorspace'])
                ));

                if($thumb_image !== false) {
                    $file_thumb_small = '<img class="img-fluid" src="'.CMSGO_IMAGES . $thumb_image[0] .'" '.$thumb_image[3].' alt="" style="border:1px solid #9BBECA;background:#F5F8F9;" />';
                    $file_image_size = getimagesize(CMSGO_STORAGE . $row["f_hash"] . '.' . $row["f_ext"], $file_image_info);
                ...
                $file_thumb_small = '<img class="img-fluid" src="'.CMSGO_RESIZE_IMAGE.'/420x420/'.$row['f_hash'].'.'.$row['f_ext'].'" alt="" style="border:1px solid #9BBECA;background:#F5F8F9;max-width:420px;height:auto;" />';
            }
        }
        $ja = 1;
    }
    ...
    if(isset($result[0]['fcat_id'])) {
        foreach($result as $row) {
            if(get_filecat_childcount($row["fcat_id"])) {
                ...
                $ke = isset($file_error["keywords"][$row["fcat_id"]]) ? '<img class="img-fluid" src="img/symbole/error.gif" width="8" height="9" alt="" />&nbsp;' : '';
                $k .= "<tr>\n<td>".$ke.html($row["fcat_name"])."</td>\n";
