...
<div class="form-group align-items-center row g-2">
  ...
  <div class="col-sm-4">
    <div class="input-group input-group-sm">
      ...
      <button class="modalButton btn btn-blue folder-open" type="button" data-bs-toggle="modal" data-bs-target="#browserModal" data-src="filebrowser.php?opt=0&amp;target=nolist" title="<?php echo $BL['be_cnt_openimagebrowser']; ?>"><i class="fa-solid fa-folder-open fa-fw" aria-hidden="true"></i></button>
      <input name="cimage_name" type="text" id="cimage_name" class="form-control" value="<?php echo isset($content['image_name']) ? html($content['image_name']) : ''; ?>" maxlength="250" onfocus="this.blur()" />
      <a href="#" id="cimage_delete_button" class="btn btn-danger trash<?php echo empty($content['image_id']) ? ' disabled' : '' ?>"<?php echo empty($content['image_id']) ? ' aria-disabled="true"' : '' ?> data-bs-toggle="tooltip" title="<?php echo $BL['be_cnt_delimage']; ?>" onclick="if ($(this).hasClass('disabled')) return false; bsConfirmDanger('<?php echo js_singlequote($BL['be_image_delete_js']); ?>' + (document.articlecontent.cimage_name.value ? '\n[' + document.articlecontent.cimage_name.value + ']' : ''), function() { document.articlecontent.cimage_name.value='';document.articlecontent.cimage_id.value='0'; if (typeof onImageSelected === 'function') onImageSelected('_', '0', ''); }, '<?php echo js_singlequote($BL['be_yes']); ?>', '<?php echo js_singlequote($BL['be_no']); ?>'); this.blur();return false;"><i class="fa-solid fa-trash-alt fa-fw" aria-hidden="true"></i></a>
...
<div class="form-group align-items-center row g-2">
  ...
  <div class="col">
    ...
    ?>
    <input type="hidden" name="cimage_pos" id="cimage_pos" value="<?php echo $content["image_pos"] ?>">
    <div class="input-group input-group-sm">
...
<div class="form-group align-items-center row g-2">
  ...
  <div class="col-sm-auto">
    <div class="input-group input-group-sm">
			...
			<input name="cimage_width" type="text" class="form-control" id="cimage_width" style="width: 50px;" maxlength="4" onkeyup="if(!parseInt(this.value,10)) this.value='';" value="<?php echo empty($content["image_width"]) ? $template_default['article']['image_default_width'] : $content["image_width"] ?>" />
  ...
  <div class="col-sm-auto">
    <div class="input-group input-group-sm">
			...
			<input name="cimage_height" type="text" class="form-control" id="cimage_height" style="width: 50px;" maxlength="4" onkeyup="if(!parseInt(this.value,10)) this.value='';" value="<?php echo empty($content["image_height"]) ? $template_default['article']['image_default_height'] : $content["image_height"] ?>" />			
				<span class="input-group-text">px</span>
...
<legend><?php echo $BL['be_cnt_caption'] ?></legend>
    ...
    <div class="col">
      <input name="cimage_caption_url" type="text" id="cimage_caption_url" class="form-control" value="<?php echo  isset($image_caption_link[0]) ? html_specialchars($image_caption_link[0]) : '' ?>" maxlength="250" />
    </div>
  </div>
