...
$img_val = isset($_GET['v']) ? explode(',', $_GET['v']) : [1, 7, 7, 'FFFFFF', 'FF4000'];
$img_val[0] = (int)$img_val[0]; // border in px
$img_val[1] = empty($img_val[1]) ? 0 : (int)$img_val[1]; // width in px
$img_val[2] = empty($img_val[2]) ? 0 : (int)$img_val[2]; // height in px
    ...
    if (isset($_GET['xy'])) {
        ...
        if (count($points)) {
            foreach ($points as $value) {
                ...
                imagecopymerge($img_source, $img_point, (int)$point[0] - (int)$img_val[5], (int)$point[1] - (int)$img_val[6], 0, 0, $img_val[1], $img_val[2], 100);
            }
        }
    }
