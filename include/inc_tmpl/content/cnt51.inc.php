...
$tmpllist = get_tmpl_files(CMSGO_TEMPLATE.'inc_cntpart/map');
...
$map_selected = 0;
$map_name = '';
$imglist = get_tmpl_files(CMSGO_TEMPLATE.'inc_cntpart/map/map_img', 'jpg,gif,png,jpeg');
if(is_array($imglist) && count($imglist)) {
    foreach($imglist as $val) {
        ...
        if($val == $content["map"]['image']) {
            ...
            $map_info = getimagesize(CMSGO_TEMPLATE.'inc_cntpart/map/map_img/'.$map_name);
        }
        $val = html($val);
        $g .= '<tr><td>';
        ...
        $gp = trim($cmsgo["templates"], '/').'/inc_cntpart/map/map_img/'.$val;
...
if($map_selected) {
    ...
    if(isset($_GET['dellocid']) && intval($_GET['dellocid'])) {
        ...
        _dbQuery("UPDATE ".DB_PREPEND."cmsgo_map SET map_deleted=9 WHERE map_cid=".intval($content["id"])." AND map_id=".intval($_GET['dellocid']), 'UPDATE');

    }

    $map_current = (isset($_GET['locid'])) ? intval($_GET['locid']) : 0;
    $map_sql  = "SELECT * FROM ".DB_PREPEND."cmsgo_map WHERE map_deleted=0 ";
    ...
    if(isset($map_result[0]['map_id'])) {
        foreach($map_result as $map_row) {
            ...
            $map_list .= '<td align="right" class="v09"><a href="cmsgo.php?do=articles&amp;p=2&amp;s=1&amp;aktion=2&amp;';
            ...
            $map_list .= '';
            $map_list .= '<a href="cmsgo.php?do=articles&amp;p=2&amp;s=1&amp;aktion=2&amp;';
