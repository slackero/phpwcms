...
if($file_action['action'] === 1 && $file_action["mark"]) {
    if(is_array($file_action["mark"]) && count($file_action["mark"])) {
        foreach($file_action["mark"] as $key => $value) {
            $key = intval($key);
            if($key) {
                $sql = 'UPDATE '.DB_PREPEND.'cmsgo_file SET f_trash=1 WHERE f_id='.$key;
                @_dbQuery($sql, 'UPDATE');
            } else {
                unset($key);
            }
    ...
    if(is_array($file_action["mark"]) && count($file_action["mark"])) {
        foreach($file_action["mark"] as $key => $value) {
            $key = intval($key);
            if($key) {
                $sql  = 'UPDATE '.DB_PREPEND.'cmsgo_file SET f_pid='.$newdir.' WHERE f_id='.$key;
                @_dbQuery($sql, 'UPDATE');
            } else {
                unset($key);
            }
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <div class="card">
      <div class="card-body">
        <form action="cmsgo.php?do=files&amp;p=4" method="post" name="folderform" id="folderform">
    ...
    <form action="cmsgo.php?do=files&amp;p=4" method="post" name="filetakeover" id="filetakeover" class="mt-3">
    ...
    <div class="card">
      <div class="card-body">
        ...
        $fx = 0;
        $file_sql = "SELECT * FROM ".DB_PREPEND."cmsgo_file WHERE f_pid=" . $file_action["file_dir"] .
      ...
      <div class="card-body">
        ...
        <div id="div_user" style="display: none;">
        	<div class="form-group form-row align-items-center">
              ...
              <?php
                $sql = "SELECT usr_id, usr_name FROM ".DB_PREPEND."cmsgo_user WHERE usr_aktiv=1 AND usr_id !=".intval($_SESSION["wcs_user_id"])." ORDER BY usr_name";
                $result = _dbQuery($sql);
