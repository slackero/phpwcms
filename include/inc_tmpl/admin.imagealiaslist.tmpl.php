...
$result = _dbQuery("SHOW COLUMNS FROM ".DB_PREPEND."cmsgo_file LIKE 'f_alias'", 'COUNT_SHOW');

if(empty($result)) {
    _dbQuery("ALTER TABLE ".DB_PREPEND."cmsgo_file ADD f_alias VARCHAR(255) NOT NULL DEFAULT ''", 'ALTER');
    echo $BL['f_alias'];
}

$sql =  "SELECT f_name, f_id FROM ".DB_PREPEND."cmsgo_file WHERE f_alias = '' AND f_hash <> '' AND f_trash=0 AND (f_ext like 'jpg' OR f_ext like 'gif' OR f_ext like 'png' OR f_ext like 'svg') ";
...
if(isset($_POST['keyword'])) {
    ...
    echo $BL['f_alias'];
        foreach($result as $files) {
            ...
            $filename[0] = cmsgo_remove_accents($filename[0]);
            ...
            $sql_count  = "SELECT COUNT(f_alias) FROM ".DB_PREPEND."cmsgo_file WHERE ";
            ...
            $sql_alias =  "UPDATE ".DB_PREPEND."cmsgo_file SET f_alias = "._dbEscape($filename[0])." WHERE f_id = ".intval($files['f_id']);
...
<div class="card">
  ...
  <div class="card-body">
  <?php if ($emptyalias > 0) { ?>
  <form action="cmsgo.php?do=admin&amp;p=12" method="post" name="aliasform">
...
if(isset($result[0]['f_id'])) {
  foreach($result as $files) {
    ...
    echo $files['f_alias'];
    $sql_count  = "SELECT COUNT(acontent_form) FROM ".DB_PREPEND."cmsgo_articlecontent WHERE acontent_form LIKE '%".$files['f_hash']."%'";
    $f_count1 = _dbCount($sql_count);
    $sql_count  = "SELECT COUNT(acontent_form) FROM ".DB_PREPEND."cmsgo_articlecontent WHERE acontent_image LIKE '%".$files['f_hash']."%'";
    $f_count2 = _dbCount($sql_count);
    $sql_count  = "SELECT COUNT(cnt_object) FROM ".DB_PREPEND."cmsgo_content WHERE cnt_object LIKE '%".$files['f_hash']."%'";
    $f_count3 = _dbCount($sql_count);
    echo '</div><div class="col-sm-auto"><a class="btn btn-sm btn-blue" href="#" onClick="'."AjaxLink('#alias-".$files['f_id']."', '".$files['f_id']."');".'"><i class="fa fa-pencil" aria-hidden="true"></i></a></div>';
