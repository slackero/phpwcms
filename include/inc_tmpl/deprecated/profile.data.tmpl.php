...
$sql = 'SELECT COUNT(*) FROM '.DB_PREPEND.'cmsgo_userdetail WHERE detail_pid='.intval($_SESSION["wcs_user_id"]);

if(_dbQuery($sql, 'COUNT')) {

	//Es sind bereits Daten hinterlegt - diese jetzt auslesen
	$sql = 'SELECT * FROM '.DB_PREPEND.'cmsgo_userdetail WHERE detail_pid='.intval($_SESSION['wcs_user_id']).' LIMIT 1';
	$detail = _dbQuery($sql);
...
<form action="cmsgo.php?do=profile&amp;p=1" method="post" name="formprofiledetail" id="formprofiledetail">
...
?>
	...
	<tr>
		...
		<td colspan="2"><select name="form_country" id="form_country" class="custom-select v12 width250">
	...
	<tr>
		...
		<td colspan="2"><select name="form_prof" id="select2" class="custom-select v12 width250">
