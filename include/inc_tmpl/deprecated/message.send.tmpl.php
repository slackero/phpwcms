...
if(isset($_GET["msg"]) && intval($_GET["msg"]) && empty($_POST['msg_send_aktion'])) {
    ...
    if($msg) {
        ...
        $sql =  "SELECT *, DATE_FORMAT(cmsgo_message.msg_tstamp, '%b %e, %Y (%H:%i)') AS send_date ".
                "FROM ".DB_PREPEND."cmsgo_message INNER JOIN ".DB_PREPEND."cmsgo_user ON ".
                DB_PREPEND."cmsgo_message.msg_from=".DB_PREPEND."cmsgo_user.usr_id WHERE ".DB_PREPEND."cmsgo_message.msg_uid=".$_SESSION["wcs_user_id"].
                " AND ".DB_PREPEND."cmsgo_message.msg_id=".$msg." LIMIT 1";
        $result = _dbQuery($sql);
...
$result = _dbQuery("SELECT detail_signature FROM ".DB_PREPEND."cmsgo_userdetail WHERE detail_pid=".$_SESSION["wcs_user_id"]." LIMIT 1");
...
if(isset($_POST['msg_send_aktion']) && intval($_POST['msg_send_aktion'])) {
    ...
    if(str_empty($msg_err)) {
        ...
        foreach($msg_receivers as $value) {
            $sql =  "INSERT INTO ".DB_PREPEND."cmsgo_message (".
...
if($msg_send_ok) {
    ...
    echo $BL['be_msg_fwd']." <br /><a href='cmsgo.php?do=messages&p=1'>".$BL['be_msg_create']."</a>.";
...
?>
    ...
    <tr valign="top">
        <td class="v09"><select name="msg_send_to" size="10" multiple="multiple" class="custom-select width250" onDblClick="opt.transferRight()">
<?php
    ...
    if(!empty($msg_to)) {
        ...
        $sql = "SELECT usr_id, usr_login, usr_name FROM ".DB_PREPEND."cmsgo_user WHERE ".$where." ORDER BY usr_name ASC";
        $result = _dbQuery($sql);
...
?>
    ...
    <td class="v09"><select name="msg_send_list" size="10" multiple="multiple" id="msg_send_list" class="custom-select width250" onChange="opt.transferLeft()">
<?php
    //Create the list of possible recipients
    $sql = "SELECT usr_id, usr_login, usr_name FROM ".DB_PREPEND."cmsgo_user ".$where1." ORDER BY usr_name ASC";
    $result = _dbQuery($sql);
