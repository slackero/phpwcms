...
if(isset($_GET["u"]) && intval($_GET["u"])) {
    ...
    if(empty($_POST["form_aktion"]) || $_POST["form_aktion"] != "edit_account") {
        ...
        $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_user WHERE usr_id=".$new_user_id." AND usr_aktiv<>9";
        $result = _dbQuery($sql);
    ...
    if(isset($_POST["form_aktion"]) && $_POST["form_aktion"] == "edit_account") {
        ...
        if(empty($new_login)) {
            ...
            $sql = "SELECT usr_id, usr_vars, COUNT(*) AS anzahl FROM ".DB_PREPEND."cmsgo_user WHERE usr_login='".aporeplace($new_login)."' GROUP BY usr_id";
            $result = _dbQuery($sql);
        ...
        if(empty($user_err)) { //Insert new User
            ...
            $sql =  "UPDATE ".DB_PREPEND."cmsgo_user SET usr_login='".aporeplace($new_login)."', ";
            if($new_password) {
                $bcrypt_pass = password_hash(makeCharsetConversion($new_password, CMSGO_CHARSET, 'utf-8'), PASSWORD_DEFAULT);
    ...
    ?><form action="cmsgo.php?do=admin&amp;s=2&amp;u=<?php echo $new_user_id ?>" method="post" name="edituser">
    ...
    ?>
      ...
      <div class="tab-content my-3">
        ...
        if (isset($new_user_id)) {
          ...
          //group access
          $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_usergroup WHERE group_trash=0 AND group_active = 1 ORDER BY group_name";
          $result = _dbQuery($sql);
          if(isset($result[0]['group_id'])) {
            foreach ($result AS $row) {
              $group["member"]    = empty($row["group_member"]) ? array() : explode(',', $row["group_member"]);
              $url = (in_array($_SESSION["wcs_user_id"], $group["member"])) ? 'cmsgo.php?do=admin&amp;p=1&amp;u='.$row["group_id"] : '#';
          ...
          $sql = 'SELECT * FROM '.DB_PREPEND."cmsgo_userlog WHERE logged_user like '".$new_login."' ORDER BY logged_start DESC LIMIT 0,100";
          $result = _dbQuery($sql);
