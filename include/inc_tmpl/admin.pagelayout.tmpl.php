...
<?php
    ...
    if(isset($result[0]['pagelayout_id'])) {
        foreach($result as $row) {
            ...
            echo '<td class="dir"><a href="cmsgo.php?do=admin&amp;p=8&amp;s='.$row["pagelayout_id"];
...
<form action="cmsgo.php?do=admin&p=8&s=0" method="post"><input type="submit" value="<?php echo $BL['be_admin_page_add'] ?>" class="btn btn-blue"></form>
</div>
</div>
<?php
    ...
    if(isset($_POST["layout_id"])) {
        ...
        $pagelayout["layout_customblocks"]      = cmsgo_remove_accents(str_replace(' ', ',', strtoupper(clean_slweg($_POST['layout_customblocks']))));
        ...
        if($pagelayout["id"]) {
            ...
            $query_mode = 'INSERT';
            $sql =  "INSERT INTO ".DB_PREPEND."cmsgo_pagelayout (".
        ...
        if($pagelayout["layout_default"]) {
            _dbQuery("UPDATE ".DB_PREPEND."cmsgo_pagelayout SET pagelayout_default=0 WHERE pagelayout_id != ".$pagelayout["id"], 'UPDATE');
        }
    ...
    if($pagelayout["id"]) {
        ...
        $sql = "SELECT * FROM ".DB_PREPEND."cmsgo_pagelayout WHERE pagelayout_id=".$pagelayout["id"]." LIMIT 1";
        $result = _dbQuery($sql);
        if(isset($result[0]['pagelayout_id'])) {
...
?>
<form action="cmsgo.php?do=admin&p=8&s=<?php echo $pagelayout["id"] ?>" method="post" name="pagelayout" target="_self">
...
<div class="col-12 col-sm text-center text-sm-right mb-3">
	 <div class="form-group">
		...
		<input type="button" class="btn btn-sm btn-blue" value="<?php echo $BL['be_admin_struct_close'] ?>" onclick="location.href='cmsgo.php?do=admin&p=8';">
     </div>
</div>
</div>
...
<hr class="pagelayout-editable" <?php echo  $pagelayout['editable_hidden']; ?>/>
    ...
    <div class="form-group form-row align-items-center pagelayout-editable"<?php echo  $pagelayout['editable_hidden']; ?>>
      <label class="col-sm-2 col-form-label text-right"><?php echo $BL['be_admin_page_main']."&nbsp;[".$cmsgo["content_width"]?>]</label>
        <div class="col">
