...
if(isset($_GET["s"])) {
    ...
    include_once CMSGO_ROOT.'/include/inc_lib/newsletter.form.inc.php';

    if(isset($_GET['edit'])) {
        include_once CMSGO_ROOT.'/include/inc_tmpl/newsletter.form.tmpl.php';
    }

    if(isset($_GET['send']) && $show_nl_send) {

        include_once CMSGO_ROOT.'/include/inc_tmpl/newsletter.send.tmpl.php';
    ...
    if(isset($_GET['duplicate_nl'])) {
        @_dbDuplicateRow(   'cmsgo_newsletter', 'newsletter_id', intval($_GET['duplicate_nl']),
...
$_newsletter['count_total'] = _dbQuery("SELECT COUNT(*) FROM ".DB_PREPEND."cmsgo_newsletter WHERE newsletter_trashed=0", 'COUNT');
...
<div class="row align-items-center">
  ...
  <div class="col-12 col-sm text-center text-sm-right mb-3">
    <div class="form-group align-items-center">
      <a class="btn btn-sm btn-blue mr-1" role="button" aria-disabled="true" href="cmsgo.php?do=messages&amp;p=3&amp;s=0&amp;edit=1"><i class="fa fa-plus"></i> <?php echo $BL['be_newsletter_new'] ?></a>
    </div>
  </div>
</div>
...
<div class="card">
    ...
    <div class="row align-items-center">
			...
			if($_newsletter['pages_total'] > 1) {
				...
				if($_SESSION['newsletter_page'] > 1) {
						...
						echo '<div class="input-group-prepend">';
						echo '<a class="btn btn-blue disabled" href="cmsgo.php?do=messages&amp;p=3&amp;page='.($_SESSION['newsletter_page']-1).'">';
						echo '<i class="fa fa-angle-left fa-fw"></i></a>';
						echo '</div>';
				}
      ...
      <div class="col-12 col-sm-auto text-right">
          <select class="form-control form-control-sm custom-select">
              ...
              <option <?php echo ($_SESSION['list_newsletter_count'] == '5') ? 'selected ' : ''; ?>onClick="window.location = 'cmsgo.php?do=messages&amp;p=3&amp;c=5'">5</option>
              <option <?php echo ($_SESSION['list_newsletter_count'] == '10') ? 'selected ' : ''; ?>onClick="window.location = 'cmsgo.php?do=messages&amp;p=3&amp;c=10'">10</option>
              <option <?php echo ($_SESSION['list_newsletter_count'] == '25') ? 'selected ' : ''; ?>onClick="window.location = 'cmsgo.php?do=messages&amp;p=3&amp;c=25'">25</option>
              <option <?php echo ($_SESSION['list_newsletter_count'] == '50') ? 'selected ' : ''; ?>onClick="window.location = 'cmsgo.php?do=messages&amp;p=3&amp;c=50'">50</option>
              <option <?php echo ($_SESSION['list_newsletter_count'] == '100') ? 'selected ' : ''; ?>onClick="window.location = 'cmsgo.php?do=messages&amp;p=3&amp;c=100'">100</option>
              <option <?php echo ($_SESSION['list_newsletter_count'] == '99999') ? 'selected ' : ''; ?>onClick="window.location = 'cmsgo.php?do=messages&amp;p=3&amp;c=all'"><?php echo $BL['be_ftptakeover_all'] ?></option>
          </select>
      </div>
    </div>
    ...
    <?php
      ...
      $sql  = "SELECT *, UNIX_TIMESTAMP(newsletter_pub) AS cdate, UNIX_TIMESTAMP(newsletter_lastsending) AS lastsend FROM ".DB_PREPEND."cmsgo_newsletter WHERE newsletter_trashed=0 ORDER BY newsletter_pub DESC, newsletter_changed DESC";
      ...
      if(isset($result[0]['newsletter_id'])) {
        ...
        foreach($result as $row) {
          ...
          // sent/queue status
          $count_sent       = _dbQuery('SELECT COUNT(*) FROM '.DB_PREPEND.'cmsgo_newsletterqueue WHERE queue_status=1 AND queue_pid='.$row["newsletter_id"], 'COUNT');
          ...
          if(empty($row["newsletter_active"]) || !$count_queue) {
            ...
            echo '<a href="cmsgo.php?do=messages&amp;p=3&amp;s='.$row["newsletter_id"];
