...
if(isset($_POST["file_search"])) {
	...
	if(sizeof($search_string)) {
		...
		//check for AND or OR
			...
			$file_key = get_list_of_file_keywords(); //Auslesen der File Schl�sselw�rter
			...
			if($result = mysql_query($sql, $db) or die ("error while running search")) {
				while($row = mysql_fetch_array($result)) {
					...
					$search["string"]  = str_replace("\n", " ", $search["string"]);
					$search["string"] .= add_keywords_to_search ($file_key, $row["f_keywords"]); //f�gt freie Keywords zum Suchstring hinzu
				...
				if(isset($search["result"]) && sizeof($search["result"]) && $search["andor"]) {
					//Pr�fen, ob die AND bedingung erf�llt ist
