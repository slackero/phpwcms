<?php
/**
 * cmsGO! content management system
 *
...
if (isset($_POST['adcampaign_title'])) {
    ...
    if ($plugin['data']['adcampaign_id'] && !empty($_FILES['adcampaign_upload_image']['name'])) {
        // image upload
        $plugin['data']['upload'] = saveUploadedFile('adcampaign_upload_image', CMSGO_CONTENT . CMSGO_ADS_DIR . '/' . $plugin['data']['adcampaign_id'] . '/', '', '1,2,3,18', '1,4');
    ...
    if (!isset($plugin['error'])) {
        ...
        if ($plugin['data']['adcampaign_id']) {
            ...
            // INSERT
            $sql = 'INSERT INTO ' . DB_PREPEND . 'cmsgo_ads_campaign (';
...
if ($plugin['id'] && !isset($plugin['error'])) {
    ...
    $sql .= 'FROM ' . DB_PREPEND . 'cmsgo_ads_campaign WHERE adcampaign_id=' . $plugin['id'];
