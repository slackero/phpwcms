...
function renderAds($match) {
	...
	$sql  = 'SELECT * FROM '.DB_PREPEND.'cmsgo_ads_campaign ac ';
	...
	if(is_array($ads) && count($ads) ) {
		...
		if(empty($_COOKIE['cmsgoAdsUserId'])) {

			$ad = $ads[array_rand($ads)];

		} else {

			$ads_userid = $_COOKIE['cmsgoAdsUserId'];
			while(count($ads)) {
				...
				if($ad['adcampaign_maxviewuser']) {
					...
					$sql  = 'SELECT COUNT(*) FROM '.DB_PREPEND.'cmsgo_ads_tracking WHERE ';
	...
	$ad_urldata	= '&amp;u='.CMSGO_USER_KEY.'&amp;r='.(empty($_SERVER['HTTP_REFERER']) ? '' : urlencode($_SERVER['HTTP_REFERER'])).'&amp;c='.$GLOBALS['aktion'][0].'&amp;a='.$GLOBALS['aktion'][1].'&amp;k='.$ad_random;
	...
	switch($ad['adcampaign_type']) {
		...
		case 3:	//Flash Layer
				...
				if(!empty($ad['adcampaign_data']['flash']) && is_file($ad['dir'].$ad['adcampaign_data']['flash'])) {
					...
					$ad_flash	.= '	var flashvars_'.$ad_so.'	= {clickTag: "'.urlencode('index.php?adclickval='.$ad['adcampaign_id'].'&current='.$ad_random.'&u='.CMSGO_USER_KEY.'&url='.$ad['adcampaign_data']['url']).'", ';
					$ad_flash	.= 'clickTarget: "'.urlencode($ad['adcampaign_data']['target']).'"};' . LF;
