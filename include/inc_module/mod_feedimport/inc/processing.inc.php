...
$plugin['id'] = isset($_GET['edit']) ? (int)$_GET['edit'] : 0;
$plugin['fields_cnt_object-structure_level_id'] = ['-2' => $BLM['cnt_object-structure_empty'], '-1' => ' ', 0 => $BL['be_admin_struct_index']] + struct_select_menu(0, 0, 0, 'array');
$plugin['fields_cnt_object-article_template_detail'] = feedimport_article_templates(CMSGO_TEMPLATE . 'inc_cntpart/articlesummary/article');
$plugin['fields_cnt_object-article_template_list'] = feedimport_article_templates(CMSGO_TEMPLATE . 'inc_cntpart/articlesummary/list');
...
$plugin['fields_cnt_object-feed_cache_timeout'] = [
...
if (isset($_POST['cnt_name'])) {
    ...
    foreach ($plugin['fields'] as $key => $value) {
        ...
        switch ($value) {
            ...
            case 'FLOAT':
                $plugin['data'][$key] = empty($_POST[$key]) ? 0 : (float)$_POST[$key];
                break;
            ...
            case 'FILE':
                $plugin['file_' . $key] = empty($_POST['file_' . $key]) ? '' : clean_slweg($_POST['file_' . $key]);
            case 'HIDDENINT':
            case 'DATESELECT':
            case 'INT':
                $plugin['data'][$key] = empty($_POST[$key]) ? 0 : (int)$_POST[$key];
                break;

            case 'DECIMAL':
                $plugin['data'][$key] = empty($_POST[$key]) ? 0 : clean_preformatted_number($_POST[$key], 'DEC2MET');
                break;
    ...
    if (!isset($plugin['error'])) {
        ...
        if ($plugin['data']['id']) {
            ...
            // UPDATE
            $result = _dbUpdate('cmsgo_content', $data, 'cnt_id=' . $plugin['data']['id'] . ' AND cnt_module=' . _dbEscape(MODULE_KEY));

        } else {

            // INSERT
            $data['cnt_created'] = now();
            $result = _dbInsert('cmsgo_content', $data);
...
if ($plugin['id'] && !isset($plugin['error'])) {
    ...
    if (isset($plugin['data'][0])) {
        ...
        $plugin['data']['cnt_object-feed_import_trigger_url'] = CMSGO_URL . 'index.php?feedimport=' . md5($plugin['data']['cnt_id'] . $plugin['data']['cnt_text']);
...
if (empty($plugin['data']['cnt_object-author_name'])) {
    [$plugin['data']['cnt_object-author_name']] = explode(' (', current($plugin['fields_cnt_object-author_id']));
}
if (empty($plugin['data']['cnt_object-source_link_text'])) {
    [$plugin['data']['cnt_object-source_link_text']] = '@@Source@@';
}
if (empty($plugin['data']['cnt_object-import_status_email'])) {
    [$plugin['data']['cnt_object-import_status_email']] = '';
}
