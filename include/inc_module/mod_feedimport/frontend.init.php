...
if(!empty($_getVar['feedimport'])) {
	...
	$feedimport_result = _dbGet('cmsgo_content', 'cnt_id,cnt_name,cnt_text,cnt_object', $feedimport_where);
	...
	if(isset($feedimport_result['cnt_object']['structure_level_id'])) {
		...
		if( $rss_obj->data ) {
			...
			include_once CMSGO_ROOT.'/include/inc_lib/backend.functions.inc.php';

			// set import sort counter
			$article_sort_counter = _dbGet('cmsgo_article', 'article_sort', 'article_cid='._dbEscape($feedimport_result['cnt_object']['structure_level_id']), '', 'article_sort DESC', 1);
			...
			foreach($rss_obj->get_items() as $rssvalue) {
				...
				$sql  = 'SELECT * FROM '.DB_PREPEND.'cmsgo_crossreference c ';
				$sql .= 'LEFT JOIN '.DB_PREPEND.'cmsgo_article a ';
                ...
                $article_title          = html_entity_decode($rssvalue->get_title(), ENT_COMPAT, CMSGO_CHARSET);
                ...
                $article_description    = preg_replace('/\s+/', ' ', html_entity_decode(strip_tags($article_summary), ENT_COMPAT, CMSGO_CHARSET));
				...
				if($article_author) {
                    $article_author     = html_entity_decode($article_author->get_name(), ENT_COMPAT, CMSGO_CHARSET);
				} else {
				...
				if($article_enclosure = $rssvalue->get_enclosure()) {
					...
					if(!$article_thumbnail && $article_enclosure->get_link()) {
						...
						if($article_thumbnail && ($article_thumbnail_ext = which_ext($article_thumbnail)) && in_array($article_thumbnail_ext, array('jpg', 'jpeg', 'png', 'gif'))) {
							...
							$article_thumbnail_store = CMSGO_STORAGE . $article_thumbnail_hash . '.' . $article_thumbnail_ext;

                            $oldmask    = umask(0);
                            $insert     = false;

							if($dir = @opendir(CMSGO_STORAGE) && copy($article_thumbnail, $article_thumbnail_store)) {
								...
								if($article_thumbnail_size = filesize($article_thumbnail_store)) {
									...
									if(isset($insert['INSERT_ID'])) {
                                        ...
                                        $feedimport_result['image']['width']            = $cmsgo["content_width"];
				...
				if(isset($result['INSERT_ID'])) {
					...
					if($article_content) {
						...
						// Inset CP Data
						$insert = _dbInsert('cmsgo_articlecontent', $cpdata);
					...
					$feedimport_result['status'][] = date('Y-m-d, H:i:s', $article_begin) . LF . $article_title . LF . $rssvalue->get_permalink() . LF . CMSGO_URL . 'cmsgo.php?do=articles&p=2&s=1&id='.$result['INSERT_ID'];
					...
					_dbInsert('cmsgo_crossreference', $data);
