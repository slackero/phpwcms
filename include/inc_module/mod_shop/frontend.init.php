...
function get_cart_data() {
    ...
    $sql  = 'SELECT * FROM '.DB_PREPEND.'cmsgo_shop_products WHERE shopprod_status=1 AND ';
...
function shop_image_tag($img=array(), $counter=0, $title='') {
    ...
    $thumb_image = get_cached_image(array(
        ...
        "max_width"		=>	$width,
        "max_height"	=>	$height,
        "thumb_name"	=>	md5($img['f_hash'].$width.$height.$GLOBALS['cmsgo']["sharpen_level"].$crop.$GLOBALS['cmsgo']['colorspace']),
        'crop_image'	=>	$crop
    ));
    ...
    if($thumb_image) {
        ...
        $list_img_temp .= CMSGO_LAZY_LOADING . HTML_TAG_CLOSE;

        // use lightbox effect
        if($config['image_'.$config['mode'].'_lightbox']) {

            $a  = '<a href="'.CMSGO_RESIZE_IMAGE.'/';
...
function get_shop_category_name($id=0, $subid=0) {
    ...
    $sql  = 'SELECT cat_name FROM '.DB_PREPEND.'cmsgo_categories WHERE ';
