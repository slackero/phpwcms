...
class ModuleShopSearch {
	...
	function search() {
		...
		$sql .= 'FROM '.DB_PREPEND.'cmsgo_shop_products WHERE shopprod_status=1';
		if($shop_lang_support && !empty($GLOBALS['cmsgo']['default_lang'])) {
			$sql .= " AND (shopprod_lang='' OR shopprod_lang="._dbEscape($GLOBALS['cmsgo']['default_lang']).')';
		}
		$data = _dbQuery($sql);
