	...
	if(isset($_SESSION[CART_KEY]['distance']) && $_SESSION[CART_KEY]['distance'] === false && !empty($_SESSION[CART_KEY]['delivery_address'])) {
		...
		if($subtotal['shop_zone_base']) {
			...
			if(CMSGO_CHARSET !== 'utf-8') {
				$_base_address = mb_convert_encoding($subtotal['shop_zone_base'], 'utf-8', CMSGO_CHARSET);
				$_delivery_address = mb_convert_encoding($_SESSION[CART_KEY]['delivery_address'], 'utf-8', CMSGO_CHARSET);
			} else {
			...
			if($_response = @file_get_contents($_query)) {
				...
				if(isset($_response->status) && $_response->status == 'OK' && isset($_response->routes[0]->legs[0]->distance->value)) {
					...
					$_query = sprintf('http://maps.googleapis.com/maps/api/geocode/json?address=%s&sensor=false&language=%s', rawurlencode($_delivery_address), $cmsgo['default_lang']);
					if($_response = @file_get_contents($_query)) {
						...
						if(isset($_response->status) && $_response->status == 'OK' && isset($_response->results[0]->address_components)) {
							foreach($_response->results[0]->address_components as $_component) {
								if(isset($_component->types[0])) {
									if($_component->types[0] === 'locality') {
										$_SESSION[CART_KEY]['distance_details']['city'] = CMSGO_CHARSET == 'utf-8' ? $_component->long_name : mb_convert_encoding($_component->long_name, CMSGO_CHARSET, 'utf-8');
									} elseif($_component->types[0] === 'country') {
										$_SESSION[CART_KEY]['distance_details']['country'] = CMSGO_CHARSET == 'utf-8' ? $_component->long_name : mb_convert_encoding($_component->long_name, CMSGO_CHARSET, 'utf-8');
										$_SESSION[CART_KEY]['distance_details']['country_code'] = strtolower($_component->short_name);
									} elseif($_component->types[0] === 'postal_code') {
										$_SESSION[CART_KEY]['distance_details']['postcode'] = CMSGO_CHARSET == 'utf-8' ? $_component->long_name : mb_convert_encoding($_component->long_name, CMSGO_CHARSET, 'utf-8');
									}
								}
							}
							...
							$_query = sprintf('http://maps.googleapis.com/maps/api/geocode/json?address=%s&sensor=false&language=%s', rawurlencode($_base_address), $cmsgo['default_lang']);
