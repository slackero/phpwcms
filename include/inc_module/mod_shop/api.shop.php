...
if(is_file($path.'/include/config/conf.inc.php')) {
    ...
    require $path.'/config/cmsgo/conf.inc.php';
}
require $path.'/include/inc_lib/default.inc.php';
require CMSGO_ROOT.'/include/inc_lib/helper.session.php';
...
if(CMSGO_CHARSET !== 'utf-8') {
    function _convert_charset($string) {
        return mb_convert_encoding($string, 'utf-8', CMSGO_CHARSET);
    }
} else {
    function _convert_charset($string) {
        return $string;
...
if($shop_api_access) {
    ...
    if($shop_api_action === 'getorders') {
        ...
        $data = _dbGet('cmsgo_shop_orders', '*', "order_status='NEW-ORDER'", '', 'order_date ASC');
