...
if(_getConfig( 'shop_pref_felang' )) {
	...
	define('SHOP_FELANG_SQL', " AND (shopprod_lang='' OR shopprod_lang="._dbEscape($cmsgo['default_lang']).')');
	define('CART_KEY', 'shopping_cart_'.$cmsgo['default_lang']);
} else {
...
if( $_shop_load_cat !== false || $_shop_load_list !== false || $_shop_load_order !== false || $_shop_load_cart_small !== false) {
	...
	if(is_file($cmsgo['modules']['shop']['path'].'template/'.$cmsgo['default_lang'].'.html')) {
		$_tmpl['source'] = @file_get_contents($cmsgo['modules']['shop']['path'].'template/'.$cmsgo['default_lang'].'.html');
	} else {
		$_tmpl['source'] = @file_get_contents($cmsgo['modules']['shop']['path'].'template/default.html');
	}

	if($_tmpl['source']) {

		if(CMSGO_CHARSET !== 'utf-8' && cmsgo_seems_utf8($_tmpl['source'])) {
			$_tmpl['source'] = mb_convert_encoding($_tmpl['source'], CMSGO_CHARSET, 'utf-8');
		}
	...
	if( isset($_POST['shop_action']) && $_POST['shop_action'] == 'add') {
		...
		if(!empty($shop_prod_id) && !empty($shop_prod_amount)) {
			...
			if(!isset($_POST['prod_opt1']) && !isset($_POST['prod_opt2'])) {
				...
				$data = _dbGet('cmsgo_shop_products', 'shopprod_size,shopprod_color', 'shopprod_status=1 AND shopprod_id='.$shop_prod_id);
...
if( $_shop_load_list !== false ) {
	...
	$sql  = "SELECT * FROM ".DB_PREPEND.'cmsgo_shop_products WHERE ';
	...
	if( isset($data[0]) ) {
		...
		foreach($data as $row) {
			...
			if($row['shopprod_size'] && $_cart_opt_1 = explode(LF, $row['shopprod_size'])) {
				...
				$_cart_prod_opt1 = '<select name="prod_opt1" id="prod_opt1_'.$row['shopprod_id'].'" class="custom-select '.trim($_tmpl['config']['class_product_option_1'].' '.$_tmpl['config']['class_prefix_shop_mode']).'">' . $_cart_prod_opt1 . '</select>';
			}
			...
			if($row['shopprod_color'] && $_cart_opt_2 = explode(LF, $row['shopprod_color'])) {
				...
				$_cart_prod_opt2 = '<select name="prod_opt2" id="prod_opt2_'.$row['shopprod_id'].'" class="custom-select '.trim($_tmpl['config']['class_product_option_2'].' '.$_tmpl['config']['class_prefix_shop_mode']).'">' . $_cart_prod_opt2 . '</select>';
			}
			...
			if($shop_detail_id) {
				...
				if($row['shopprod_opengraph']) {
					...
					$content['opengraph']['url'] = abs_url(array('shop_detail'=>$shop_detail_id), array('shop_cat', 'shop_cart', 'cmsgo_output_action', 'print', 'cmsgo-preview', 'unsubscribe', 'subscribe'));
				...
				$sql = 'UPDATE LOW_PRIORITY '.DB_PREPEND.'cmsgo_shop_products SET shopprod_track_view=shopprod_track_view+1 WHERE shopprod_id='.$shop_detail_id;
				_dbQuery($sql, 'UPDATE');
...
if( $_shop_load_order !== false ) {
	...
	if(empty($cart_data)) {
		...
		$cart_mode = 'terms';
		include $cmsgo['modules']['shop']['path'].'inc/cart.items.inc.php';
		...
		if($_tmpl['config']['order_number_style'] == 'RANDOM') {
			...
			$order_num = _dbCount('SELECT COUNT(*) FROM '.DB_PREPEND.'cmsgo_shop_orders') + 1;
		...
		if(empty($send_order_to[0]) || !is_valid_email($send_order_to[0])) {
			$email_to = $cmsgo['SMTP_FROM_EMAIL'];
		} else {
