...
if(isset($cmsgo['modules'][$module]['path'])) {
	...
	if(empty($_SESSION['shop_db_proof'])) {
		...
		if(!empty($result)) {
			...
			if(!isset($result[0])) {
				$result = _dbQuery("ALTER TABLE ".DB_PREPEND."cmsgo_shop_products ADD shopprod_track_view INT(11) NOT NULL DEFAULT '0', ADD INDEX (shopprod_track_view)", 'ALTER');
			}
			$result = _dbQuery("SHOW COLUMNS FROM ".DB_PREPEND."cmsgo_shop_products LIKE 'shopprod_lang'");
			if(!isset($result[0])) {
				$result = _dbQuery("ALTER TABLE ".DB_PREPEND."cmsgo_shop_products ADD shopprod_lang VARCHAR(255) NOT NULL DEFAULT '', ADD INDEX (shopprod_lang)", 'ALTER');
			}
	...
	include_once $cmsgo['modules'][$module]['path'].'inc/functions.backend.inc.php';
	include_once $cmsgo['modules'][$module]['path'].'inc/functions.global.inc.php';
	...
	$BE['HEADER']['module.shop.css'] = '	<link href="'.$cmsgo['modules'][$module]['dir'].'template/module.shop.css" rel="stylesheet" type="text/css" />';
	...
	// processing
	if( $action ) {
		include_once $cmsgo['modules'][$module]['path'].'inc/processing.' . $controller . '.inc.php';
	}
	...
	// listing
	if($action) {
		include_once $cmsgo['modules'][$module]['path'].'inc/'.$action.'.' . $controller . '.inc.php';
	} else {
		include_once $cmsgo['modules'][$module]['path'].'inc/listing.' . $controller . '.inc.php';
	}
	
	// footer
	include_once $cmsgo['modules'][$module]['path'].'inc/footer.inc.php';
