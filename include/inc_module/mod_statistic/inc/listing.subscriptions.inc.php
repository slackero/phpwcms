    ...
    <div class="table-responsive mb-3">
      <table class="table table-sm table-striped table-hover table-valign-middle mb-0">
        ...
        foreach($result2 as $data) {
          ...
          echo '      <a class="btn btn-sm btn-danger ms-1" href="'.$_controller_link.'&amp;delete='.$data["log_id"].'" title="delete: '.html_specialchars($data["log_msg"]).'" data-confirm-type="danger" data-confirm-action="'.$BLM['log_delete_entry'].'" data-confirm="'.$BLM['log_delete_entry'].' '.js_singlequote($data["log_msg"]).'"><i class="fa-solid fa-trash-alt"></i></a>';

          if (stristr($data["log_msg"], '::')) {
            $email_array = explode('::', $data["log_msg"]);
            echo '    <a class="btn btn-sm btn-dark ms-1" href="'.$_controller_link.'&amp;blacklist='.$email_array[1].'" title="Add to blacklist: '.html_specialchars($email_array[1]).'" data-confirm-type="warning" data-confirm-action="'.$BLM['log_insert_blacklist'].'" data-confirm="'.html_specialchars($email_array[1]).' '.$BLM['log_insert_blacklist'].'"><i class="fa-solid fa-ban"></i></a>';
          }
          echo '    </td></tr>' . LF;
          $x++;
        }
