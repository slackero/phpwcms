...
$result = _dbGet(
    ...
    'detail_regkey='._dbEscape(CMSGO_ADDRESS_KEY).' AND detail_aktiv=1 AND detail_public=1'
);
...
if(isset($result[0])) {
    ...
    $sql  = 'SELECT * FROM '.DB_PREPEND.'cmsgo_userdetail pu ';
    $sql .= 'LEFT JOIN '.DB_PREPEND.'cmsgo_country pc ON pu.detail_country=pc.country_iso ';
    $sql .= "WHERE pu.detail_regkey="._dbEscape(CMSGO_ADDRESS_KEY)." AND ";
    ...
    $order_by_country_suffix = $cmsgo['default_lang'] === 'de' ? '_de' : '';
    $result = _dbGet(
        'cmsgo_country',
        '*',
    ...
    if($_adr_count) {
        ...
        foreach($_adr as $_akey => $arow) {
            ...
            $result[] = '        <div class="card '.( $arow['detail_varchar2'] == 1 ? 'branch-office' : 'cmsgo-office' ).'">'; // 3
            ...
            $contacts = _dbGet(
                ...
                'detail_aktiv=1 AND detail_regkey='._dbEscape(CMSGO_CONTACTS_KEY).' AND detail_int1='.$arow['detail_id'].' AND '.$contact_where,
                '',
            ...
            if(isset($contacts[0])) {
                ...
                */
                ...
                //$result_contacts[] = '       <div class="collapse" id="cmsgo-contact-section-'.$_akey.'"><div class="row">';
            ...
            if(count($result_contacts)) { // && !isset($related_contacts[$arow['detail_id']])
                ...
                $result[] = '       <button type="button" class="btn btn-readmore-gray" data-toggle="collapse" data-target="#cmsgo-contact-section-'.$_akey.'">';
            ...
            if(count($result_contacts)) {
                ...
                $result[] = '<div class="collapse" id="cmsgo-contact-section-'.$_akey.'">';
                $result[] = '<div class="row">';
        ...
        */
        ...
        if(count($maps)) {
            ...
            require_once( CMSGO_ROOT.'/include/inc_ext/GoogleMapsAPI/cmsgoGoogleMapAPIv3.class.php' );

            // JavaScripts laden
            $block['custom_htmlhead']['google_maps1'] = '  <script src="//maps.googleapis.com/maps/api/js?sensor=false&amp;h1='.$cmsgo['default_lang'].'" type="text/javascript"></script>'; //key='.MAPS_API_KEY.'&amp;
            //$block['custom_htmlhead']['google_maps2'] = '  <script src="'.TEMPLATE_PATH.'inc_script/cmsgo/js/cmsgo.address.js" type="text/javascript"></script>';
            ...
            foreach($maps as $row) {
                ...
                $map->addMarkerIcon($icon_path.'cmsgo-arrow.png',$icon_path.'cmsgo-arrow.shadow.png', 12, 30, 15, 15);
...
$content['all'] = str_replace('{CMSGO-ADDRESS-LOCATOR}', LF . implode(LF, $dealer), $content['all']);
