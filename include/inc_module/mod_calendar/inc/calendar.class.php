...
class cmsgoCalendar {
    ...
    function parse_match($match='') {
        ...
        if($match !== '' && strpos($match, '=') !== FALSE ) {
            ...
            // oh yes fix, in case LF was converted to <br /> by cmsgo
        ...
        if(!empty($match['date_start'])) {
            ...
            if(strtoupper($match['date_start']) == 'TODAY') {
                ...
                $match['date_start'] = cmsgo_strtotime($match['date_start']);
                if($match['date_start']) {
        ...
        if(!empty($match['date_end'])) {
            ...
            if(is_intval($match['date_end'])) {
                ...
                $match['date_end'] = cmsgo_strtotime($match['date_end']);
                if($match['date_end']) {
        ...
        if($default['template'] !== '') {
            ...
            if(is_file(CMSGO_TEMPLATE.'calendar/'.$default['template'])) {
                $default['template'] = file_get_contents(CMSGO_TEMPLATE.'calendar/'.$default['template']);
                if($default['template']) {
        ...
        if($default['lang'] !== '') {
            ...
            $default['lang']    = array_intersect( convertStringToArray( $default['lang'], ' '), $GLOBALS['cmsgo']['allowed_lang']);
        ...
        if($default['tag'] !== '') {
            ...
            if(count($default['tag'])) {
                ...
                if(count($tag_where)) {
                    ...
                    $this->join_on  = 'LEFT JOIN '.DB_PREPEND.'cmsgo_categories ON cat_pid=calendar_id';
    ...
    function getDate($s=NULL, $e=NULL, $o='', $l='', $g='', $w='') {
        ...
        $sql .= ' FROM '.DB_PREPEND.'cmsgo_calendar pc ';
        $sql .= $this->join_on;
        $sql .= ' WHERE ';
    ...
    function getDateSelect($name='', $year_min_max=NULL, $day=NULL, $month=NULL, $year=NULL) {
        ...
        $_day  = '<select name="'.$name.$bind.'day" id="'.$name.$bind.'day" class="custom-select cal-day">' . LF;
        for($x=1; $x<=31; $x++) {
            $_day .= '  <option value="'.$x.'"';
            if($x==$day) {
        ...
        $_month = '<select name="'.$name.$bind.'month" id="'.$name.$bind.'month" class="custom-select cal-month">';
        for($x=1; $x<=12; $x++) {
        ...
        $_year = '<select name="'.$name.$bind.'year" id="'.$name.$bind.'year" class="custom-select cal-year">';
