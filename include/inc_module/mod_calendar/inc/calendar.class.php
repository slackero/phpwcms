...
class cmsgoCalendar {
    ...
    public function parse_match($match='') {
        ...
        if ($default['template'] !== '') {
            ...
            if (!is_file($template_path)) {
                $template_path =  CMSGO_TEMPLATE . 'calendar/' . $default['template'];
                if (!is_file($template_path)) {
    ...
    public function renderDateImage($date, $template) {
        ...
        $image_WxHxC  = $this->width ?: $GLOBALS['cmsgo']['img_list_width'];
        $image_WxHxC .= 'x';
        $image_WxHxC .= $this->height ?: $GLOBALS['cmsgo']['img_list_height'];
        $image_WxHxC_zoom  = $this->width ?: $GLOBALS['cmsgo']['img_prev_width'];
        $image_WxHxC_zoom .= 'x';
        $image_WxHxC_zoom .= $this->height ?: $GLOBALS['cmsgo']['img_prev_height'];
        if ($this->crop) {
            $image_WxHxC .= 'x1';
            $image_WxHxC_zoom .= 'x1';
        }
