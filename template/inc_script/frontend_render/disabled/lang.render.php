...
if(is_array($cmsgo['allowed_lang']) && count($cmsgo['allowed_lang']) > 1) {
	...
	foreach($cmsgo['allowed_lang'] as $lang) {
		...
		if( $content['auto_lang']['is_left'] ) {
			...
			if($content['auto_lang']['match_left'][1] == $lang) {
				...
				$lang_active_class	= 'active';
				$content_replace['current_lang'] = $lang;
			} else {
    ...
    $qstring = '';
    if (isset($_GET['profile_register'])) {
      $qstring = '?profile_register='.$_GET['profile_register'];
    }

		if(CMSGO_REWRITE) {
			$content['auto_lang']['opposite'][$lang] = '<a href="'.$content['auto_lang']['opposite'][$lang].CMSGO_REWRITE_EXT.$qstring;
		} else {
			$content['auto_lang']['opposite'][$lang] = '<a href="index.php?'.$content['auto_lang']['opposite'][$lang];
		}

		$content['auto_lang']['opposite'][$lang] .= '" class="' . $lang_active_class . '">';
	...
	$content_replace['switch_lang'] = '<div class="languageselector" id="languageselector">' . implode(' / ', $content['auto_lang']['opposite']) . '</div>';
