...
$redirect       = CMSGO_URL.'index.php'.$redirect;

if(!empty($_SERVER['HTTP_REFERER']) && strpos(strtolower($_SERVER['HTTP_REFERER']), strtolower(CMSGO_URL)) !== FALSE) {
...
if(!empty($_SESSION['cmsgoAgree']) || ( isset($_COOKIE['cmsgoAgree']) && $_COOKIE['cmsgoAgree'] == 1)) {
    ...
    $ACCESS['source_data'] = @file_get_contents(CMSGO_TEMPLATE.'inc_script/access/'.$ACCESS['source_data']);
