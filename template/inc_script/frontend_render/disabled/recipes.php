...
// define neccessary functions only when RT is in use
	...
	function showRecipeSeach()
	{
		...
		$search = file_get_contents(CMSGO_TEMPLATE . 'inc_cntpart/recipe/search/search.html');
	...
	{
		$cat = _dbQuery('SELECT acontent_text FROM ' . DB_PREPEND . 'cmsgo_articlecontent WHERE acontent_type=26 AND acontent_trash=0');
		$cat_all = '';
		if($cat) {
			foreach($cat as $temp) {
	...
	function listRecipes($alt = null)
	{
		...
		$sql .= "INNER JOIN " . DB_PREPEND . "cmsgo_article ON ";
		$sql .= DB_PREPEND . "cmsgo_article.article_id = " . DB_PREPEND . "cmsgo_articlecontent.acontent_aid ";
		...
		if(!empty($_getVar['recipetext'])) {
			...
			foreach($text as $value) {
				...
				$t[] = '( CONCAT(acontent_newsletter, ' . DB_PREPEND . "cmsgo_article.article_title, acontent_title) LIKE '%" . aporeplace($value) . "%' )";
		...
		$sql .= DB_PREPEND . "cmsgo_article.article_deleted=0 ";
		if(!PREVIEW_MODE) {
			$sql .= 'AND ';
			$sql .= '(' . DB_PREPEND . 'cmsgo_article.article_begin IS NULL OR ' . DB_PREPEND . 'cmsgo_article.article_begin < NOW()) ';
			$sql .= 'AND ';
			$sql .= '(' . DB_PREPEND . 'cmsgo_article.article_end > NOW() OR ' .DB_PREPEND . 'cmsgo_article.article_end IS NULL)';
		}
