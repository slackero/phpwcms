...
function createSocialBookmark($matches) {
    ...
    if(strpos($matches[1], '<!--DETAIL_ONLY-->') === false) {
        ...
        if(empty($GLOBALS['aktion'][1])) {
            $bm['url']  = CMSGO_URL.'index.php?id='.$GLOBALS['aktion'][0];
        } else {
            $bm['url']  = CMSGO_URL.'index.php?aid='.$GLOBALS['aktion'][1];
        }
    ...
    $bm['service']['technorati']['url']  = 'http://technorati.com/faves?add='.rawurlencode(CMSGO_URL);
    ...
    $bm['service']['yahoo']['url']       = 'http://myweb2.search.yahoo.com/myresults/bookmarklet?t='.$bm['title'].'&amp;u='.$bm['url'].'&amp;d='.$bm['bodytext'].'&amp;ei='.CMSGO_CHARSET;
