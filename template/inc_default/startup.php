	...
	<div class="card-body">
		...
		if (!empty($be_support['phone']) || !empty($be_support['email'])) {
			...
			if (!empty($be_support['phone'])) {
				echo 'Tel ' . html_specialchars($be_support['phone']) . '<br>';
			}
			if (!empty($be_support['email'])) {
				echo '<a href="mailto:' . html_specialchars($be_support['email']) . '">' . html_specialchars($be_support['email']) . '</a>';
			}
			echo '</p>';
		}
		?>
...
<div class="text-right text-muted mt-3" style="font-size: 0.8em;">
